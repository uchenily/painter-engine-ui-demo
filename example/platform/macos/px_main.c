...
void PX_glutDisplayFunction(void) {
    ...
    glTexImage2D(GL_TEXTURE_2D,
                 0,
                 GL_RGBA,
                 pRenderSurface->width,
                 pRenderSurface->height,
                 0,
                 GL_RGBA,
                 GL_UNSIGNED_BYTE,
                 pRenderSurface->surfaceBuffer);
...
px_void PX_glutOnMouseEvent(int button, int state, int x, int y) {
    PX_Object_Event e;
    px_float cursorx_scale
        = App.runtime.surface_width * 1.0f / App.runtime.window_width;
    px_float cursory_scale
        = App.runtime.surface_height * 1.0f / App.runtime.window_height;
    switch (button) {
...
int main(int argc, char **argv) {
    ...
    // PX_srand(314159);
    if (!PX_ApplicationInitialize(
            &App, PX_GetScreenWidth(), PX_GetScreenHeight()))
        return 0;
    if (!PX_CreateWindow(App.runtime.surface_width,
                         App.runtime.surface_height,
                         App.runtime.window_width,

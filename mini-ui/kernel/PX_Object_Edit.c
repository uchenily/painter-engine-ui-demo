...
px_void PX_Object_EditGetCursorXY(PX_Object *pObject,
                                  px_int *cx,
                                  px_int *cy,
                                  px_int *height) {
    ...
    while (PX_TRUE) {
        ...
        if (cursor == pEdit->cursor_index) {
            *cx = x;
            *cy = y;
            *height = pEdit->fontModule ? pEdit->fontModule->max_Height
                                        : __PX_FONT_HEIGHT;
            return;
        }
        ...
        if (pEdit->fontModule) {
            px_dword code;
            px_int width, height;
            fsize = PX_FontModuleGetCharacterDesc(
                pEdit->fontModule, Text + cursor, &code, &width, &height);
            if (!fsize) {
                break;
            }
            if (code == '\r') {
    ...
    *cy = y;
    *height
        = pEdit->fontModule ? pEdit->fontModule->max_Height : __PX_FONT_HEIGHT;
}
...
px_void PX_Object_EditOnMouseMove(PX_Object *Object,
                                  PX_Object_Event e,
                                  px_void *user_ptr) {
...
px_void
PX_Object_EditUpdateCursorOnDown(PX_Object *pObject, px_int cx, px_int cy);
px_void PX_Object_EditOnMouseLButtonDown(PX_Object *Object,
                                         PX_Object_Event e,
                                         px_void *user_ptr) {
    PX_Object_Edit *pEdit = PX_Object_GetEdit(Object);
    px_float x, y;
    ...
    if (pEdit) {
        if (PX_ObjectIsPointInRegion(Object, (px_float) x, (px_float) y)) {
            ...
            PX_Object_EditUpdateCursorOnDown(
                Object, (px_int) (x - objx), (px_int) (y - objy));
        } else
            PX_Object_EditSetFocus(Object, PX_FALSE);
    }
}

px_void PX_Object_EditOnKeyboardString(PX_Object *Object,
                                       PX_Object_Event e,
                                       px_void *user_ptr) {
    PX_Object_Edit *pEdit = PX_Object_GetEdit(Object);
...
PX_Object *PX_Object_EditCreate(px_memorypool *mp,
                                PX_Object *Parent,
                                px_int x,
                                px_int y,
                                px_int Width,
    ...
    PX_Object_Edit *pEdit
        = (PX_Object_Edit *) MP_Malloc(mp, sizeof(PX_Object_Edit));
    ...
    PX_ObjectRegisterEvent(pObject,
                           ...
                           PX_Object_EditOnMouseMove,
                           PX_NULL);
    PX_ObjectRegisterEvent(pObject,
                           PX_OBJECT_EVENT_CURSORDOWN,
                           PX_Object_EditOnMouseLButtonDown,
                           PX_NULL);
    PX_ObjectRegisterEvent(pObject,
                           PX_OBJECT_EVENT_STRING,
                           PX_Object_EditOnKeyboardString,
                           PX_NULL);
        ...
        if (pEdit->fontModule) {
            ...
            if (code == '\r') {
                ...
                if (cx <= x_draw_oft + width / 2 + pEdit->xFontSpacing / 2
                    && cy <= y_draw_oft + pEdit->fontModule->max_Height
                                 + pEdit->yFontSpacing / 2) {
            ...
            if (Text[cursor] == '\r') {
                ...
                if (cx <= x_draw_oft + __PX_FONT_ASCSIZE / 2
                              + pEdit->xFontSpacing / 2
                    && cy <= y_draw_oft + __PX_FONT_HEIGHT
                                 + pEdit->yFontSpacing / 2) {
                    pEdit->cursor_index = cursor;
                    return;
                } else {
...
px_void PX_Object_EditRender(px_surface *psurface,
                             PX_Object *pObject,
                             px_uint elapsed) {
    ...
    if (pEdit->EditSurface.width != (px_int) objWidth
        || pEdit->EditSurface.height != (px_int) objHeight) {
        PX_SurfaceFree(&pEdit->EditSurface);
        if (!PX_SurfaceCreate(pObject->mp,
                              (px_int) objWidth,
                              (px_int) objHeight,
                              &pEdit->EditSurface)) {
            return;
    ...
    if (pEdit->style == PX_OBJECT_EDIT_STYLE_RECT) {
        // clear
        PX_SurfaceClear(&pEdit->EditSurface,
                        0,
                        0,
                        (px_int) objWidth - 1,
                        (px_int) objHeight - 1,
        ...
        if (pEdit->Border) {
            if (pEdit->state == PX_OBJECT_EDIT_STATE_NORMAL)
                PX_GeoDrawBorder(&pEdit->EditSurface,
                                 0,
                                 0,
                                 (px_int) objWidth - 1,
                                 (px_int) objHeight - 1,
        ...
        PX_SurfaceClear(&pEdit->EditSurface,
                        ...
                        (px_int) objHeight - 1,
                        PX_COLOR(0, 0, 0, 0));
        PX_GeoDrawSolidRoundRect(&pEdit->EditSurface,
                                 0,
                                 0,
                                 (px_int) objWidth - 1,
                                 (px_int) objHeight - 1,
        ...
        if (pEdit->Border) {
            if (pEdit->state == PX_OBJECT_EDIT_STATE_NORMAL)
                PX_GeoDrawRoundRect(&pEdit->EditSurface,
                                    0,
                                    0,
                                    (px_int) objWidth - 1,
                                    (px_int) objHeight - 1,
    ...
    while (PX_TRUE) {
        ...
        if (pEdit->onFocus && pEdit->cursor_index == cursor) {
            ...
            if ((pEdit->elapsed / 200) & 1) {
                if (pEdit->fontModule) {
                    PX_GeoDrawRect(&pEdit->EditSurface,
                                   x_draw_oft + 1,
                                   y_draw_oft,
                                   x_draw_oft,
                                   y_draw_oft + pEdit->fontModule->max_Height
                                       - 2,
                                   pEdit->TextColor);
                } else {
                    PX_GeoDrawRect(&pEdit->EditSurface,
        ...
        if (pEdit->fontModule) {
            ...
            if (code == '\r') {
                ...
                PX_FontModuleDrawCharacter(&pEdit->EditSurface,
                                           pEdit->fontModule,
                                           x_draw_oft,
                                           y_draw_oft,
                                           code,
            ...
            if (Text[cursor] == '\r') {
                ...
                if (pEdit->Password) {
                    PX_FontDrawChar(&pEdit->EditSurface,
                                    x_draw_oft,
                                    y_draw_oft,
                                    '*',
                                    pEdit->TextColor);
            ...
            if (*Text != 8) {
                ...
                if (pEdit->max_length > 0) {
                    if (PX_StringLen(&pEdit->text) < pEdit->max_length) {
                        ...
                        PX_StringInsertChar(
                            &pEdit->text, pEdit->cursor_index, ch);
                        pEdit->cursor_index++;
                    }
                } else {
                    px_char ch = *Text;
            ...
            case PX_FONTMODULE_CODEPAGE_UTF8: {
                ...
                for (i = 0; i < 6; i++) {
                    ...
                    if ((pText[pEdit->cursor_index - 1] & 0x80) == 0x00) {
                        PX_StringRemoveChar(&pEdit->text,
...
px_void PX_Object_EditAutoNewLine(PX_Object *pObject,
                                  px_bool b,
...
px_void PX_Object_EditSetOffset(PX_Object *pObject,
                                px_int TopOffset,
                                px_int LeftOffset) {
    PX_Object_Edit *pEdit = PX_Object_GetEdit(pObject);
    if (pObject != PX_NULL) {
...
px_void
PX_Object_EditSetXYOffset(PX_Object *pObject, px_int XOffset, px_int YOffset) {
    PX_Object_Edit *pEdit = PX_Object_GetEdit(pObject);
    if (pEdit != PX_NULL) {
...
px_void PX_Object_EditSetInputMode(PX_Object *pObject,
                                   PX_OBJECT_EDIT_INPUT_MODE mode) {
    PX_Object_Edit *pEdit = PX_Object_GetEdit(pObject);
    if (pEdit != PX_NULL) {
...
// edit
//////////////////////////////////////////////////////////////////////////
PX_Object *PX_Designer_EditCreate(px_memorypool *mp,
                                  PX_Object *pparent,
                                  px_float x,
                                  px_float y,
                                  px_float width,
    ...
    return PX_Object_EditCreate(
        mp, pparent, (px_int) x, (px_int) y, 128, 28, fm);
}

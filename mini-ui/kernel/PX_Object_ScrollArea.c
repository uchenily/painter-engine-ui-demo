...
px_void PX_Object_ScrollArea_EventDispatcher(PX_Object *Object,
                                             PX_Object_Event e,
                                             px_void *user_ptr) {
    ...
    if (e.Event == PX_OBJECT_EVENT_EXECUTE
...
px_void PX_Object_ScrollAreaHSliderChanged(PX_Object *pObject,
                                           PX_Object_Event e,
                                           px_void *ptr) {
...
px_void PX_Object_ScrollAreaVSliderChanged(PX_Object *pObject,
                                           PX_Object_Event e,
                                           px_void *ptr) {
    px_int value = PX_Object_SliderBarGetValue(pObject);
    PX_Object_ScrollArea *pSA = (PX_Object_ScrollArea *) ptr;
...
PX_Object *PX_Object_ScrollAreaCreate(px_memorypool *mp,
                                      PX_Object *Parent,
                                      px_int x,
                                      px_int y,
                                      px_int Width,
    ...
    PX_Object_ScrollArea *pSA
        = (PX_Object_ScrollArea *) MP_Malloc(mp, sizeof(PX_Object_ScrollArea));
    ...
    pSA->vscroll = PX_Object_SliderBarCreate(mp,
                                             pObject,
                                             0,
                                             0,
                                             16,
    ...
    PX_ObjectRegisterEvent(pObject,
                           PX_OBJECT_EVENT_ANY,
                           PX_Object_ScrollArea_EventDispatcher,
                           PX_NULL);
    PX_ObjectRegisterEvent(pSA->hscroll,
                           PX_OBJECT_EVENT_VALUECHANGED,
                           PX_Object_ScrollAreaHSliderChanged,
                           pSA);
    PX_ObjectRegisterEvent(pSA->vscroll,
                           PX_OBJECT_EVENT_VALUECHANGED,
                           PX_Object_ScrollAreaVSliderChanged,
                           pSA);
...
PX_Object *PX_Object_ScrollAreaSetBackgroundColor(PX_Object *pObj,
                                                  px_color color) {
    PX_Object_ScrollArea *pSA;
    pSA = PX_Object_GetScrollArea(pObj);
    if (pSA) {
...
px_void PX_Object_ScrollAreaGetRegion(PX_Object *pObject,
                                      px_float *left,
                                      px_float *top,
                                      px_float *right,
                                      px_float *bottom) {
    ...
    if (pObject->Visible == PX_FALSE) {
        PX_Object_ScrollAreaGetRegion(
            pObject->pNextBrother, left, top, right, bottom);
        return;
    }
    ...
    do {
        ...
        if (rWidth > pObject->Width) {
            if (pObject->Width != 0) {
                ...
                PX_Object_SliderBarSetSliderButtonLength(pSA->hscroll,
                                                         (px_int) btnW);
            }
            PX_Object_SliderBarSetRange(
                pSA->hscroll, (px_int) left, (px_int) (right - objWidth));
    ...
    do {
        ...
        if (rHeight > pObject->Height) {
            if (pObject->Height != 0) {
                ...
                PX_Object_SliderBarSetSliderButtonLength(pSA->vscroll,
                                                         (px_int) btnH);
            }
            PX_Object_SliderBarSetRange(
                pSA->vscroll, (px_int) top, (px_int) (bottom - objHeight));
...
px_void PX_Object_ScrollAreaRender(px_surface *psurface,
                                   PX_Object *pObject,
                                   px_uint elapsed) {
    PX_Object_ScrollArea *pSA;
    px_float objx, objy, objWidth, objHeight;
    ...
    if (pSA->surface.height != (px_int) objHeight
        || pSA->surface.width != (px_int) objWidth) {
...
px_void PX_Object_ScrollAreaSetBorderColor(PX_Object *pObj,

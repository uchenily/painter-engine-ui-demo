...
px_void PX_Object_RingProcessBarRender(px_surface *psurface,
                                       PX_Object *pObject,
                                       px_uint elapsed) {
    px_int x, y, r;
    px_char content[8] = {0};
    ...
    PX_Object_RingProcessBar *pDesc
        = (PX_Object_RingProcessBar *) pObject->pObject;
    ...
    PX_sprintf1(content,
                sizeof(content),
                "%1.2%",
                PX_STRINGFORMAT_FLOAT(process * 100));
    PX_GeoDrawCircle(
        psurface, x, y, r, pDesc->linewidth + 6, pDesc->BackgroundColor);
    PX_GeoDrawRing(psurface,
                   x,
                   y,
                   (px_float) r,
                   (px_float) pDesc->linewidth,
                   pDesc->Color,
                   -90,
                   -90 + (px_int) (360 * process));
    PX_FontModuleDrawText(
        psurface, pDesc->fm, x, y, PX_ALIGN_CENTER, content, pDesc->Color);
}

PX_Object *PX_Object_RingProcessBarCreate(px_memorypool *mp,
                                          PX_Object *Parent,
                                          px_int x,
                                          px_int y,
                                          px_int r,
    ...
    pObject = PX_ObjectCreateEx(mp,
                                ...
                                PX_NULL,
                                PX_Object_RingProcessBarRender,
                                PX_NULL,
                                &Desc,
...
px_void PX_Object_RingProcessBarSetColor(PX_Object *pRingProcessBar,
                                         px_color Color) {
    PX_Object_RingProcessBar *pProcess
        = PX_Object_GetRingProcessBar(pRingProcessBar);
    if (pProcess != PX_NULL) {
...
px_void PX_Object_RingProcessBarSetBackgroundColor(PX_Object *pRingProcessBar,
                                                   px_color Color) {
    PX_Object_RingProcessBar *pProcess
        = PX_Object_GetRingProcessBar(pRingProcessBar);
    if (pProcess != PX_NULL) {
...
px_void PX_Object_RingProcessBarSetValue(PX_Object *pRingProcessBar,
                                         px_int Value) {
    PX_Object_RingProcessBar *pProcess
        = PX_Object_GetRingProcessBar(pRingProcessBar);
    if (pProcess != PX_NULL) {

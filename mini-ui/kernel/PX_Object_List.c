...
px_void PX_Object_ListOnCursorMove(PX_Object *pObject,
                                   PX_Object_Event e,
                                   px_void *ptr) {
    ...
    if (pList->pArrayData) {
        if (pList->ArrayDataCount * pList->ItemHeight > pObject->Height) {
            if (PX_Object_Event_GetCursorX(e)
                > objx + objWidth - PX_OBJECT_LIST_SLIDERBAR_WIDTH) {
                return;
            }
        }
    } else {
    ...
    pList->currentCursorIndex
        = (px_int) (PX_Object_Event_GetCursorY(e) + pList->offsety - objy)
          / pList->ItemHeight;

    if (pList->pArrayData) {
        if (pList->currentCursorIndex < 0
            || pList->currentCursorIndex >= pList->ArrayDataCount) {
            pList->currentCursorIndex = -1;
        }
    } else {
        if (pList->currentCursorIndex < 0
            || pList->currentCursorIndex >= pList->pData.size) {
            pList->currentCursorIndex = -1;
        }
    }
}

px_void PX_Object_ListOnCursorDown(PX_Object *pObject,
                                   PX_Object_Event e,
                                   px_void *ptr) {
    PX_Object_List *pList = PX_Object_GetList(pObject);
    px_float objx, objy, objWidth, objHeight;
    ...
    if (pList->currentSelectedIndex != currentSelectedIndex) {
        ...
        if (pList->pArrayData) {
            if (pList->currentSelectedIndex < 0
                || pList->currentSelectedIndex >= pList->ArrayDataCount) {
                pList->currentSelectedIndex = -1;
            }
        } else {
            if (pList->currentSelectedIndex < 0
                || pList->currentSelectedIndex >= pList->pData.size) {
                pList->currentSelectedIndex = -1;
            }
        }
        ...
        if (pList->doubleCursorCancel && pList->currentSelectedIndex != -1) {
            ...
            PX_Object_Event_SetIndex(&ne, pList->currentSelectedIndex);
            PX_ObjectExecuteEvent(pObject, ne);
        } else if (pList->currentSelectedIndex != -1
                   && pList->click_elapsed < 260) {
            PX_memset(&e, 0, sizeof(ne));
...
px_void PX_Object_ListOnCursorRDown(PX_Object *pObject,
                                    PX_Object_Event e,
                                    px_void *ptr) {
    PX_Object_List *pList = PX_Object_GetList(pObject);
    if (!PX_ObjectIsCursorInRegion(pObject, e)) {
...
px_void
PX_Object_ListOnWheel(PX_Object *pObject, PX_Object_Event e, px_void *ptr) {
    PX_Object_List *pList = PX_Object_GetList(pObject);
    px_float objx, objy, objWidth, objHeight;
    px_float inheritX, inheritY;
    ...
    if (pList->pArrayData) {
        if (pList->offsety
            > (pList->ItemHeight) * pList->ArrayDataCount - objHeight) {
            pList->offsety = (pList->ItemHeight) * pList->ArrayDataCount
                             - (px_int) objHeight;
        }
    } else {
        if (pList->offsety
            > (pList->ItemHeight) * pList->pData.size - objHeight) {
            pList->offsety
                = (pList->ItemHeight) * pList->pData.size - (px_int) objHeight;
        }
    }
...
px_void PX_Object_ListMoveToBottom(PX_Object *pObject) {
    ...
    if (pList->ItemHeight * pList->pData.size > pObject->Height) {
        pList->offsety = (px_int) (pList->ItemHeight * pList->pData.size
                                   - pObject->Height);
        PX_Object_SliderBarSetRange(
            pList->SliderBar,
            0,
            (px_int) (pList->ItemHeight * (pList->pData.size)
                      - pObject->Height));
        PX_Object_SliderBarSetValue(
            pList->SliderBar,
...
px_void PX_Object_ListRender(px_surface *psurface,
                             PX_Object *pObject,
                             px_uint elapsed) {
    ...
    if (pList->Items.size != (px_int) (objHeight / (pList->ItemHeight) + 2)) {
        ...
        for (i = 0; i < pList->Items.size; i++) {
            PX_Object *pItemObject
                = *PX_VECTORAT(PX_Object *, &pList->Items, i);
        ...
        for (i = 0; i < (px_int) (objHeight / pList->ItemHeight + 2); i++) {
            Item.pList = pObject;
            Item.pdata = PX_NULL;
            pItemObject = PX_ObjectCreateEx(pList->mp,
                                            0,
                                            0,
                                            0,
                                            0,
                                            (px_float) pObject->Width,
                                            (px_float) pList->ItemHeight,
                                            0,
            ...
            if (!pList->CreateFunctions(pList->mp, pItemObject, pList->userptr))
                return;
            if (!PX_VectorPushback(&pList->Items, &pItemObject))
                return;
        }
    }

    if (pList->ItemHeight * pList->pData.size > objHeight) {
        PX_Object_SliderBar *pSliderBar
            = PX_Object_GetSliderBar(pList->SliderBar);
        ...
        pSliderBar->SliderButtonLength
            = (px_int) (pList->SliderBar->Height * objHeight
                        / (pList->ItemHeight * pList->pData.size));
        if (pSliderBar->SliderButtonLength < 32)
            pSliderBar->SliderButtonLength = 32;
        pSliderBar->Max
            = (pList->ItemHeight) * pList->pData.size - (px_int) objHeight;
        PX_Object_SliderBarSetBackgroundColor(pList->SliderBar,
                                              pList->BackgroundColor);
    ...
    if ((px_int) pList->renderSurface.width != (px_int) pObject->Width
        ...
        if (!PX_SurfaceCreate(pList->mp,
                              (px_int) pObject->Width,
    ...
    PX_SurfaceClear(&pList->renderSurface,
                    0,
                    0,
                    pList->renderSurface.width - 1,
                    pList->renderSurface.height - 1,
                    pList->BackgroundColor);
    for (i = 0; i < pList->Items.size; i++) {
        px_int index = i + pList->offsety / pList->ItemHeight;
        pItemObject = *PX_VECTORAT(PX_Object *, &pList->Items, i);
        pItem = PX_Object_GetListItem(pItemObject);
        if (pItemObject->y + pItemObject->Height > 0
            && pItemObject->y < pObject->Height && pItem->pdata) {

            if (pList->showCursor) {
                if (index == pList->currentCursorIndex
                    && index != pList->currentSelectedIndex) {
                    PX_GeoDrawRect(
                        &pList->renderSurface,
                        (px_int) pItemObject->x,
                        (px_int) pItemObject->y,
                        (px_int) (pItemObject->x + pItemObject->Width - 1),
                        (px_int) (pItemObject->y + pItemObject->Height - 1),
                        pList->CursorColor);
                }
            }
    ...
    PX_GeoDrawBorder(&pList->renderSurface,
                     0,
                     0,
                     pList->renderSurface.width - 1,
                     pList->renderSurface.height - 1,
...
px_void PX_Object_ListOnSliderValueChanged(PX_Object *Object,
                                           PX_Object_Event e,
                                           px_void *ptr) {
...
px_void
PX_Object_ListOnEvent(PX_Object *Object, PX_Object_Event e, px_void *ptr) {
    px_int i;
    PX_Object_List *pList = PX_Object_GetList(Object);

    switch (e.Event) {
        case PX_OBJECT_EVENT_CURSORWHEEL:
            if (!PX_ObjectIsPointInRegion(Object,
        ...
        case PX_OBJECT_EVENT_CURSORMOVE:
            ...
            PX_Object_Event_SetCursorX(
                &e, PX_Object_Event_GetCursorX(e) - Object->x);
            PX_Object_Event_SetCursorY(
                &e, PX_Object_Event_GetCursorY(e) - Object->y);
            break;
    }
...
PX_Object *PX_Object_ListCreate(px_memorypool *mp,
                                PX_Object *Parent,
                                px_int x,
                                px_int y,
                                px_int Width,
                                px_int Height,
                                px_int ItemHeight,
                                PX_Object_ListItemOnCreate _CreateFunc,
                                px_void *userptr) {
    ...
    if (!PX_SurfaceCreate(mp, Width, Height, &List.renderSurface))
        return PX_NULL;
    if (!(pListObject = PX_ObjectCreateEx(mp,
                                          Parent,
                                          (px_float) x,
                                          (px_float) y,
                                          0,
                                          ...
                                          PX_OBJECT_TYPE_LIST,
                                          PX_NULL,
                                          PX_Object_ListRender,
                                          PX_Object_ListFree,
                                          &List,
                                          sizeof(PX_Object_List))))
        return PX_NULL;

    pList = PX_Object_GetList(pListObject);
    pList->SliderBar = PX_Object_SliderBarCreate(
        mp,
        pListObject,
        (px_int) pListObject->Width - PX_OBJECT_LIST_SLIDERBAR_WIDTH,
        0,
        PX_OBJECT_LIST_SLIDERBAR_WIDTH,
        (px_int) pListObject->Height,
        PX_OBJECT_SLIDERBAR_TYPE_VERTICAL,
        PX_OBJECT_SLIDERBAR_STYLE_BOX);

    PX_ObjectRegisterEvent(pListObject,
                           PX_OBJECT_EVENT_CURSORWHEEL,
                           PX_Object_ListOnWheel,
                           pListObject);
    PX_ObjectRegisterEvent(pListObject,
                           PX_OBJECT_EVENT_CURSORDOWN,
                           PX_Object_ListOnCursorDown,
                           pListObject);
    PX_ObjectRegisterEvent(pListObject,
                           PX_OBJECT_EVENT_CURSORRDOWN,
                           PX_Object_ListOnCursorRDown,
                           pListObject);
    PX_ObjectRegisterEvent(pListObject,
                           PX_OBJECT_EVENT_CURSORMOVE,
                           PX_Object_ListOnCursorMove,
                           pListObject);
    PX_ObjectRegisterEvent(pList->SliderBar,
                           PX_OBJECT_EVENT_VALUECHANGED,
                           PX_Object_ListOnSliderValueChanged,
                           pListObject);
    PX_ObjectRegisterEvent(
        pListObject, PX_OBJECT_EVENT_ANY, PX_Object_ListOnEvent, pListObject);
    return pListObject;
}

static px_void PX_Object_ListContentItemOnRender(px_surface *psurface,
                                                 PX_Object *pObject,
                                                 px_dword elapsed) {
    ...
    PX_FontModuleDrawText(psurface,
                          PX_Object_GetList(pListItem->pList)->fm,
                          (px_int) objx + 3,
                          (px_int) (objy + objHeight / 2),
                          PX_ALIGN_LEFTMID,
                          (const px_char *) pListItem->pdata,
                          PX_OBJECT_UI_DEFAULT_FONTCOLOR);
}

px_bool PX_Designer_ListContentItemOnCreate(px_memorypool *mp,
...
PX_Object *PX_Object_ListContentCreate(px_memorypool *mp,
                                       PX_Object *Parent,
                                       px_int x,
                                       px_int y,
                                       px_int Width,
    ...
    PX_Object *pObject;
    if (fm) {
        pObject = PX_Object_ListCreate(mp,
                                       Parent,
                                       x,
                                       y,
                                       Width,
                                       Height,
                                       fm->max_Height + 6,
                                       PX_Designer_ListContentItemOnCreate,
                                       PX_NULL);
        if (pObject) {
...
px_bool PX_Object_ListItemIsOnDisplayPresent(PX_Object *pObject, px_int index) {
    if (pObject->Type == PX_OBJECT_TYPE_LIST) {
        PX_Object_List *pList = PX_ObjectGetDesc(PX_Object_List, pObject);
        PX_Object *pItemObject
            = *PX_VECTORAT(PX_Object *, &pList->Items, index);
...
px_void PX_Object_ListSetBackgroundColor(PX_Object *pListObject,
                                         px_color color) {
...
px_void PX_Object_ListViewFocus(PX_Object *pListObject, px_int index) {
    ...
    if (index >= 0 && index < pList->pData.size) {
        ...
        if (y < pList->offsety
            || y + pList->ItemHeight > pList->offsety + pListObject->Height) {
            if (y + pList->ItemHeight < pListObject->Height) {
                pList->offsety = 0;
            } else if (y > pList->pData.size * pList->ItemHeight
                               - pListObject->Height) {
                pList->offsety = (px_int) (pList->pData.size * pList->ItemHeight
                                           - pListObject->Height);
            } else {
...
px_void PX_Object_ListArrayRender(px_surface *psurface,
                                  PX_Object *pObject,
                                  px_uint elapsed) {
    PX_Object *pItemObject;
    px_int offsetX = 0, drawXCenter = 0;
    ...
    if (pList->ItemHeight * pList->ArrayDataCount > objHeight) {
        ...
        if (pSliderBar->SliderButtonLength < 32)
            pSliderBar->SliderButtonLength = 32;
        pSliderBar->Max
            = (pList->ItemHeight) * pList->ArrayDataCount - (px_int) objHeight;
        PX_Object_SliderBarSetBackgroundColor(pList->SliderBar,
                                              pList->BackgroundColor);
        PX_Object_SliderBarSetColor(pList->SliderBar, pList->BorderColor);
    } else {
    ...
    for (i = 0; i < pList->Items.size; i++) {
        ...
        if (pList->offsety < 0) {
            ...
            if (i + pList->offsety / pList->ItemHeight
                < pList->ArrayDataCount) {
                px_int index = i + pList->offsety / pList->ItemHeight;
                pItem->pdata = pList->pArrayData;
                pItem->index = index;
                PX_ObjectUpdate(pItemObject, elapsed);
...
px_bool PX_Designer_ListArrayItemOnCreate(px_memorypool *mp,
                                          PX_Object *ItemObject,
                                          px_void *userptr) {
    PX_Object_List *pList
        = (PX_Object_List *) PX_ObjectGetDesc(PX_Object_ListItem, ItemObject)
              ->pList->pObject;
    ItemObject->Func_ObjectRender = pList->ArrayRender;
    return PX_TRUE;
}
PX_Object *PX_Object_ListArrayCreate(px_memorypool *mp,
                                     PX_Object *Parent,
                                     px_int x,
                                     px_int y,
                                     px_int Width,
    ...
    pObject = PX_Object_ListCreate(mp,
                                   ...
                                   Height,
                                   ItemHeight,
                                   PX_Designer_ListArrayItemOnCreate,
                                   PX_NULL);
    if (pObject) {
        PX_Object_List *pList = PX_ObjectGetDesc(PX_Object_List, pObject);
...
px_void PX_Object_ListArraySetData(PX_Object *pListObject,
                                   px_void *ArrayData,
...
px_void *PX_Object_ListArrayItemGetData(PX_Object *pListItemObject) {
    if (pListItemObject->Type == PX_OBJECT_TYPE_LISTITEM) {
        PX_Object_ListItem *pItem
            = PX_ObjectGetDesc(PX_Object_ListItem, pListItemObject);
        return pItem->pdata;
    }
    return PX_NULL;
}
...
PX_Object *PX_Designer_ListCreate(px_memorypool *mp,
                                  PX_Object *pparent,
                                  px_float x,
                                  px_float y,
                                  px_float width,
    ...
    return PX_Object_ListContentCreate(
        mp, pparent, (px_int) x, (px_int) y, 256, 128, fm);
}

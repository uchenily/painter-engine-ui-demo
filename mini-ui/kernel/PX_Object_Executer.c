...
px_void PX_Object_ExecuterPrintText(PX_Object *pObject, const px_char *text) {
    if (pObject->Type == PX_OBJECT_TYPE_EXECUTER) {
        PX_Object_Executer *pExecuter
            = PX_ObjectGetDesc(PX_Object_Executer, pObject);
        PX_Object_PrinterPrintText(pExecuter->printer, text);
    }
}

PX_Object *PX_Object_ExecuterLastPrintText(PX_Object *pObject,
                                           const px_char *text) {
    if (pObject->Type == PX_OBJECT_TYPE_EXECUTER) {
        PX_Object_Executer *pExecuter
            = PX_ObjectGetDesc(PX_Object_Executer, pObject);
...
px_bool PX_ExecuterVM_LastPrint(PX_VM *Ins, px_void *userptr) {
    ...
    PX_Object_ExecuterLastPrintText(pObject,
                                    PX_VM_STACK(Ins, 0)._string.buffer);
    PX_VM_RET(Ins,
              PX_VM_Variable_int(
                  PX_Object_PrinterGetLastCreateId(pExecute->printer)));
    return PX_TRUE;
}
...
px_bool PX_ExecuterVM_Sin(PX_VM *Ins, px_void *userptr) {
    ...
    PX_VM_RET(
        Ins,
        PX_VM_Variable_float((px_float) PX_sind(PX_VM_STACK(Ins, 0)._float)));
    return PX_TRUE;
}
...
px_bool PX_ExecuterVM_Cos(PX_VM *Ins, px_void *userptr) {
    ...
    PX_VM_RET(
        Ins,
        PX_VM_Variable_float((px_float) PX_cosd(PX_VM_STACK(Ins, 0)._float)));
    return PX_TRUE;
}
...
px_bool PX_ExecuterVM_CreateThread(PX_VM *Ins, px_void *userptr) {
    ...
    if (!PX_VMBeginThreadFunction(&pExecute->vm,
                                  PX_VMGetFreeThreadId(Ins),
                                  PX_VM_STACK(Ins, 0)._string.buffer,
                                  PX_NULL,
                                  0)) {
        PX_VM_RET(Ins, PX_VM_Variable_int(0));
...
static px_bool PX_Object_ExecuterRunScript(PX_Object *pObject,
                                           const px_char *pshellstr) {
    ...
    if (!PX_CompilerInitialize(pObject->mp, &compiler))
        goto _ERROR;
    if (!PX_CompilerAddSource(&compiler, stdio))
        goto _ERROR;
    if (!PX_CompilerAddSource(&compiler, pshellstr))
        goto _ERROR;
    if (!PX_CompilerCompile(&compiler, &bin, &pExecute->debugmap, "main"))
        goto _ERROR;

    // Load
    if (!PX_VMInitialize(
            &pExecute->vm, pObject->mp, bin.buffer, bin.usedsize)) {
        goto _ERROR;
    }
    PX_MemoryFree(&bin);
    ...
    PX_VMRegistHostFunction(
        &pExecute->vm, "print", PX_ExecuterVM_Print, pObject); // Print
    PX_VMRegistHostFunction(
        &pExecute->vm, "gets", PX_ExecuterVM_Gets, pObject); // Gets
    PX_VMRegistHostFunction(
        &pExecute->vm, "clear", PX_ExecuterVM_Clear, pObject); // Clear
    PX_VMRegistHostFunction(
        &pExecute->vm, "sleep", PX_ExecuterVM_Sleep, pObject); // Sleep
    PX_VMRegistHostFunction(
        &pExecute->vm, "rand", PX_ExecuterVM_Rand, pObject); // Rand
    PX_VMRegistHostFunction(
        &pExecute->vm, "sin", PX_ExecuterVM_Sin, pObject); // Sin
    PX_VMRegistHostFunction(
        &pExecute->vm, "cos", PX_ExecuterVM_Cos, pObject); // Cos
    PX_VMRegistHostFunction(&pExecute->vm,
                            "lastprint",
                            PX_ExecuterVM_LastPrint,
                            pObject); // lastprint
    PX_VMRegistHostFunction(&pExecute->vm,
                            "createthread",
                            PX_ExecuterVM_CreateThread,
...
px_bool PX_ExecuterRegistryHostFunction(PX_Object *pObject,
                                        const px_char Name[],
                                        PX_VM_Function_Modules function,
                                        px_void *userptr) {
    if (pObject->Type != PX_OBJECT_TYPE_EXECUTER) {
        PX_ASSERT();
        return PX_FALSE;
...
px_void PX_Object_ExecuterRender(px_surface *psurface,
                                 PX_Object *pObject,
                                 px_dword elapsed) {
    PX_Object_Executer *pdesc = PX_ObjectGetDesc(PX_Object_Executer, pObject);
...
PX_Object *PX_Object_ExecuterCreate(px_memorypool *mp,
                                    PX_Object *Parent,
                                    px_int x,
                                    px_int y,
                                    px_int width,
    ...
    pObject = PX_ObjectCreateEx(mp,
                                ...
                                PX_Object_ExecuterUpdate,
                                0,
                                PX_Object_ExecuterFree,
                                &desc,
                                sizeof(desc));
    pdesc = PX_ObjectGetDesc(PX_Object_Executer, pObject);
    pdesc->printer
        = PX_Object_PrinterCreate(mp, pObject, x, y, width, height, PX_NULL);

...
static px_void PX_Designer_RefreshProperties(PX_Object *pObject) {
    px_int i;
    PX_Object_Designer *pDesignerDesc
        = PX_ObjectGetDesc(PX_Object_Designer, pObject);
    ...
    if (PX_VectorCheckIndex(&pDesignerDesc->Objects,
                            pDesignerDesc->selectObjectIndex)) {
        PX_Designer_Object *pdobject
            = PX_VECTORAT(PX_Designer_Object,
                          &pDesignerDesc->Objects,
                          pDesignerDesc->selectObjectIndex);
        if (PX_VectorCheckIndex(&pDesignerDesc->ObjectDesc,
                                pdobject->descIndex)) {
            px_int j;
            PX_Designer_ObjectDesc *pObjectDesc
                = PX_VECTORAT(PX_Designer_ObjectDesc,
                              &pDesignerDesc->ObjectDesc,
                              pdobject->descIndex);
            for (j = 0; j < PX_COUNTOF(pObjectDesc->properties); j++) {
                ...
                PX_Object_LabelSetText(pDesignerDesc->label_propertys[j],
                                       pObjectDesc->properties[j].Name);

                if (pObjectDesc->properties[j].getbool) {
                    px_bool v
                        = pObjectDesc->properties[j].getbool(pdobject->pObject);
                    if (v) {
                ...
                if (pObjectDesc->properties[j].getfloat) {
                    px_float v = pObjectDesc->properties[j].getfloat(
                        pdobject->pObject);
                    px_char content[32];
                ...
                if (pObjectDesc->properties[j].getint) {
                    px_int v
                        = pObjectDesc->properties[j].getint(pdobject->pObject);
                    px_char content[32];
                ...
                if (pObjectDesc->properties[j].getstring) {
                    ...
                    ;
                    if (pObjectDesc->properties[j].getstring(pdobject->pObject,
                                                             &str)) {
                        PX_Object_EditSetText(pDesignerDesc->edit_propertys[j],
                                              str.buffer);
                    } else {
        ...
        if (PX_VectorCheckIndex(&pDesignerDesc->ObjectDesc,
            ...
            for (j = 0; j < PX_COUNTOF(pObjectDesc->properties); j++) {
                ...
                if (pObjectDesc->properties[j].setbool) {
                    const px_char *pContent = PX_Object_EditGetText(
                        pDesignerDesc->edit_propertys[j]);
                    ;
                    if (pContent[0] == '0') {
                        pObjectDesc->properties[j].setbool(pdobject->pObject,
                                                           PX_FALSE);
                    } else {
                        pObjectDesc->properties[j].setbool(pdobject->pObject,
                                                           PX_TRUE);
                ...
                if (pObjectDesc->properties[j].setfloat) {
                    ...
                    pObjectDesc->properties[j].setfloat(pdobject->pObject,
                                                        PX_atof(pContent));
                ...
                if (pObjectDesc->properties[j].setint) {
                    const px_char *pContent = PX_Object_EditGetText(
                        pDesignerDesc->edit_propertys[j]);
                    ;
                    pObjectDesc->properties[j].setint(pdobject->pObject,
                                                      PX_atoi(pContent));
                    continue;
                }
                ...
                if (pObjectDesc->properties[j].setstring) {
                    const px_char *pContent = PX_Object_EditGetText(
                        pDesignerDesc->edit_propertys[j]);
                    ;
                    pObjectDesc->properties[j].setstring(pdobject->pObject,
                                                         pContent);
                    continue;
                }
            }
...
static px_void PX_Designer_UpdateObjectsList(PX_Object *pObject) {
    ...
    for (i = 0; i < pDesignerDesc->Objects.size; i++) {
        PX_Object_ListAdd(
            pDesignerDesc->list_Objects,
            PX_VECTORAT(PX_Designer_Object, &pDesignerDesc->Objects, i));
    }
}
px_void PX_DesignerOnDesignerBoxValueChanged(PX_Object *pObject,
                                             PX_Object_Event e,
                                             px_void *ptr) {
    PX_Object_DesignerBox *pDesignerBox
        = (PX_Object_DesignerBox *) pObject->pObject;
    PX_Object_Designer *pDesc
        = (PX_Object_Designer *) ((PX_Object *) ptr)->pObject;

    if (pDesc->selectObjectIndex >= 0
        && pDesc->selectObjectIndex < pDesc->Objects.size) {
        PX_Designer_Object *pdobject = PX_VECTORAT(
            PX_Designer_Object, &pDesc->Objects, pDesc->selectObjectIndex);
        ...
        switch (pdobject->type) {
            ...
            case PX_DESIGNER_OBJECT_TYPE_GAME: {
                ...
                pdobject->pObject->x = pDesc->designerbox->x
                                       + pDesc->pLinkWorld->offsetx
                                       + pDesc->designerbox->Width / 2;
                pdobject->pObject->y = pDesc->designerbox->y
                                       + pDesc->pLinkWorld->offsety
...
px_void PX_DesignerUpdate(PX_Object *pObject, px_dword elapsed) {
    ...
    // delete button
    if (pDesc->selectObjectIndex != -1) {
        pDesc->button_delete->x = pDesc->designerbox->x
                                  + pDesc->designerbox->Width
                                  - pDesc->button_delete->Width;
        pDesc->button_delete->y
            = pDesc->designerbox->y - pDesc->button_delete->Height - 8;
    ...
    for (i = 0; i < count; i++) {
        PX_Designer_ObjectDesc *pController
            = PX_VECTORAT(PX_Designer_ObjectDesc, &pDesc->ObjectDesc, i);
        PX_Object_ListAdd(pDesc->list_ObjectDesc, pController);
    }
}

px_void PX_Designer_ControllersListItemOnRender(px_surface *psurface,
                                                PX_Object *pObject,
                                                px_dword elapsed) {
    PX_Designer_ObjectDesc *pDesc
        = (PX_Designer_ObjectDesc *) PX_Object_ListItemGetData(pObject);
    PX_Object_Designer *pDesignerDesc
        = PX_ObjectGetDesc(PX_Object_Designer, (PX_Object *) pObject->User_ptr);
    ...
    switch (pDesc->type) {
        case PX_DESIGNER_OBJECT_TYPE_UI:
            PX_GeoDrawBorder(psurface,
                             ...
                             (px_int) objy + 1,
                             (px_int) objx + 64,
                             (px_int) objy + PX_DESIGNER_CONTROLLER_ITEM_HEIGHT
                                 - 2,
                             1,
            ...
            PX_FontModuleDrawText(psurface,
                                  ...
                                  (px_int) objx + 32,
                                  (px_int) objy
                                      + PX_DESIGNER_CONTROLLER_ITEM_HEIGHT / 2,
                                  PX_ALIGN_CENTER,
                                  "UI",
                                  PX_COLOR(255, 192, 64, 255));
            PX_FontModuleDrawText(psurface,
...
px_bool PX_Designer_ControllersListItemOnCreate(px_memorypool *mp,
                                                PX_Object *ItemObject,
                                                px_void *userptr) {
...
px_void PX_Designer_ObjectsListItemOnRender(px_surface *psurface,
                                            PX_Object *pObject,
                                            px_dword elapsed) {
    PX_Designer_Object *pDesc
        = (PX_Designer_Object *) PX_Object_ListItemGetData(pObject);
    PX_Object_Designer *pDesignerDesc
        = PX_ObjectGetDesc(PX_Object_Designer, (PX_Object *) pObject->User_ptr);
    px_float objx, objy, objWidth, objHeight;
    px_float inheritX, inheritY;
...
px_bool PX_Designer_ObjectsListItemOnCreate(px_memorypool *mp,
                                            PX_Object *ItemObject,
                                            px_void *userptr) {
...
px_void
PX_DesignerRender(px_surface *psurface, PX_Object *pObject, px_dword elapsed) {
    ...
    if (pDesc->pLinkWorld && pDesc->showsliderbar) {
        if (pDesc->pLinkWorld->world_width > pDesc->pLinkWorld->surface_width) {
            ...
            pDesc->world_hscroll->Width
                = pDesc->pLinkWorld->surface_width - 24.f;
        } else {
            pDesc->world_hscroll->Visible = PX_FALSE;
        }

        if (pDesc->pLinkWorld->world_height
            > pDesc->pLinkWorld->surface_height) {
            ...
            pDesc->world_vscroll->Height
                = pDesc->pLinkWorld->surface_height - 24.f;
        } else {
...
px_void PX_Designer_OnPropertyChanged(PX_Object *pObject,
                                      PX_Object_Event e,
                                      px_void *ptr) {
    PX_Object_Designer *pDesignerDesc
        = PX_ObjectGetDesc(PX_Object_Designer, (PX_Object *) ptr);
    PX_Designer_UpdateProperties((PX_Object *) ptr);
}

px_void PX_Designer_OnObjectsListChanged(PX_Object *pObject,
                                         PX_Object_Event e,
                                         px_void *ptr) {
    PX_Object_Designer *pDesignerDesc
        = PX_ObjectGetDesc(PX_Object_Designer, (PX_Object *) ptr);
    pDesignerDesc->selectObjectIndex
        = PX_Object_ListGetCurrentSelectIndex(pDesignerDesc->list_Objects);
    PX_Designer_RefreshProperties((PX_Object *) ptr);
}

px_void PX_Designer_OnPropertyRefresh(PX_Object *pObject,
                                      PX_Object_Event e,
                                      px_void *ptr) {
    PX_Object_Designer *pDesignerDesc
        = PX_ObjectGetDesc(PX_Object_Designer, (PX_Object *) ptr);
...
px_void PX_Designer_OnDeleteCurrentObject(PX_Object *pObject,
                                          PX_Object_Event e,
                                          px_void *ptr) {
    PX_Object_Designer *pDesignerDesc
        = PX_ObjectGetDesc(PX_Object_Designer, (PX_Object *) ptr);
    if (PX_VectorCheckIndex(&pDesignerDesc->Objects,
        ...
        switch (pdobject->type) {
            ...
            case PX_DESIGNER_OBJECT_TYPE_GAME: {
                ...
                PX_WorldRemoveObject(pDesignerDesc->pLinkWorld,
                                     pdobject->pObject);
            }
            default:
                break;
        }
        PX_VectorErase(&pDesignerDesc->Objects,
                       pDesignerDesc->selectObjectIndex);
    }
...
px_void
PX_Designer_OnCursorDown(PX_Object *pObject, PX_Object_Event e, px_void *ptr) {
    PX_Object_Designer *pDesignerDesc
        = PX_ObjectGetDesc(PX_Object_Designer, pObject);
    ...
    if (e.Event == PX_OBJECT_EVENT_CURSORDOWN) {
        ...
        if (pDesignerDesc->selectObjectIndex != -1) {
            if (PX_isXYInRegion(x,
                                y,
                                pDesignerDesc->designerbox->x - 6,
                                pDesignerDesc->designerbox->y - 6,
                                pDesignerDesc->designerbox->Width + 12,
                                pDesignerDesc->designerbox->Height + 12)) {
                return;
            }
        ...
        for (i = 0; i < pDesignerDesc->Objects.size; i++) {
            ...
            if (pDesignerDesc->pLinkWorld
                && pdobject->type == PX_DESIGNER_OBJECT_TYPE_GAME) {
                px_rect rect;
                rect.x = pdobject->pObject->x
                         - pDesignerDesc->pLinkWorld->offsetx
                         - pdobject->pObject->Width / 2;
                rect.y = pdobject->pObject->y
                         - pDesignerDesc->pLinkWorld->offsety
                         - pdobject->pObject->Height / 2;
            ...
            if (pDesignerDesc->pLinkObject
                && pdobject->type == PX_DESIGNER_OBJECT_TYPE_UI) {
        ...
        if (selectObjectIndex == -1 && pDesignerDesc->selectObjectIndex == -1) {
            px_int selectDescIndex = PX_Object_ListGetCurrentSelectIndex(
                pDesignerDesc->list_ObjectDesc);
            ...
            if (selectDescIndex != -1) {
                ...
                switch (pObjectDesc->type) {
                    case PX_DESIGNER_OBJECT_TYPE_UI: {
                        if (pDesignerDesc->pLinkObject) {
                            PX_Object *pNewObject = pObjectDesc->createfunc(
                                pDesignerDesc->mp_ui,
                                pDesignerDesc->pLinkObject,
                                x,
                                y,
                                100,
                                100,
                            ...
                            PX_sprintf2(
                                pNewObject->id,
                                sizeof(pNewObject->id),
                                "%1%2",
                                PX_STRINGFORMAT_STRING(pObjectDesc->Name),
                                PX_STRINGFORMAT_INT(pDesignerDesc->allocID++));

                            dobject.pObject = pNewObject;
                            dobject.type = PX_DESIGNER_OBJECT_TYPE_UI;
                            dobject.descIndex = selectDescIndex;
                            PX_VectorPushback(&pDesignerDesc->Objects,
                                              &dobject);
                    ...
                    case PX_DESIGNER_OBJECT_TYPE_GAME: {
                        if (pDesignerDesc->pLinkWorld) {
                            PX_Object *pNewObject = pObjectDesc->createfunc(
                                pDesignerDesc->mp_game,
                                PX_NULL,
                                x + pDesignerDesc->pLinkWorld->offsetx,
                                y + pDesignerDesc->pLinkWorld->offsety,
                                100,
                                100,
                                pDesignerDesc->userptr);
                            PX_sprintf2(
                            ...
                            PX_WorldAddObject(pDesignerDesc->pLinkWorld,
                                              pNewObject);
                            dobject.pObject = pNewObject;
                    ...
                    case PX_DESIGNER_OBJECT_TYPE_FUNCTION: {
                        PX_Object *pNewObject
                            = pObjectDesc->createfunc(pDesignerDesc->mp,
                                                      PX_NULL,
                                                      x,
                                                      y,
                                                      100,
        ...
        if (selectObjectIndex != -1) {
            PX_Object_ListSetCurrentSelectIndex(pDesignerDesc->list_Objects,
                                                selectObjectIndex);
            if (pDesignerDesc->selectObjectIndex != selectObjectIndex) {
                PX_Designer_Object *pdobject
                    = PX_VECTORAT(PX_Designer_Object,
                                  &pDesignerDesc->Objects,
                                  selectObjectIndex);
                PX_Object_DesignerBox *pDesignerBox = PX_ObjectGetDesc(
                    PX_Object_DesignerBox, pDesignerDesc->designerbox);
                ...
                switch (pdobject->type) {
                    case PX_DESIGNER_OBJECT_TYPE_UI:
                        ...
                        pDesignerDesc->designerbox->Width
                            = pdobject->pObject->Width;
                        pDesignerDesc->designerbox->Height
                            = pdobject->pObject->Height;
                        break;
                    case PX_DESIGNER_OBJECT_TYPE_GAME:
                    case PX_DESIGNER_OBJECT_TYPE_FUNCTION:
                        pDesignerDesc->designerbox->x
                            = pdobject->pObject->x
                              - pDesignerDesc->pLinkWorld->offsetx
                              - pdobject->pObject->Width / 2;
                        pDesignerDesc->designerbox->y
                            = pdobject->pObject->y
                              - pDesignerDesc->pLinkWorld->offsety
                              - pdobject->pObject->Height / 2;
                        pDesignerDesc->designerbox->Width
...
//////////////////////////////////////////////////////////////////////////
/*
typedef PX_Object *(*px_designer_createfunc)(px_memorypool *mp,PX_Object
*pparent,px_float x,px_float y,px_float width,px_float height,PX_FontModule
*fm); typedef px_float (*px_designer_getproperty_float)(PX_Object *pObject);
...
typedef px_void (*px_designer_setproperty_string)(PX_Object *pObject,const
px_char v[]); typedef px_void (*px_designer_setproperty_bool)(PX_Object
*pObject,px_bool v); typedef struct
*/
...
//////////////////////////////////////////////////////////////////////////
/*
PX_Object *PX_Designer_LabelCreate(px_memorypool *mp,PX_Object *pparent,px_float
x,px_float y,px_float width,px_float height,PX_FontModule *fm)
{
    return
PX_Object_LabelCreate(mp,pparent,(px_int)x,(px_int)y,128,32,"Label",fm,PX_OBJECT_UI_DEFAULT_FONTCOLOR);
}
...
px_void PX_DesignerOnMenuExit(px_void *userPtr) {
    PX_Object *pObject = (PX_Object *) userPtr;
    PX_ObjectExecuteEvent(pObject,
                          PX_OBJECT_BUILD_EVENT(PX_OBJECT_EVENT_CLOSE));
...
px_void PX_Designer_OnScrollVChanged(PX_Object *pObject,
                                     PX_Object_Event e,
                                     px_void *ptr) {
    PX_Object *pDesignerObject = (PX_Object *) ptr;
    PX_Object_Designer *pDesc
        = PX_ObjectGetDesc(PX_Object_Designer, pDesignerObject);
    if (pDesc->pLinkWorld) {
...
px_void PX_Designer_OnScrollHChanged(PX_Object *pObject,
                                     PX_Object_Event e,
                                     px_void *ptr) {
    PX_Object *pDesignerObject = (PX_Object *) ptr;
    PX_Object_Designer *pDesc
...
PX_Object *PX_Object_DesignerCreate(px_memorypool *mp,
    ...
    pdesc->widget_property = PX_Object_WidgetCreate(
        mp, pObject, 5, 36 + 256, 192, 192, "properties", PX_NULL);
    if (!pdesc->widget_property)
        return PX_FALSE;
    if (pdesc->pLinkWorld)
        pdesc->widget_property->x = pdesc->pLinkWorld->surface_width
                                    - pdesc->widget_property->Width - 25;

    pdesc->widget_propertyscrollarea = PX_Object_ScrollAreaCreate(
        mp, pdesc->widget_property, 0, 0, 192, 162);

    do {
        PX_Object_Menu_Item *pItem;
        pdesc->menu = PX_Object_MenuCreate(mp, pObject, 0, 0, 64, PX_NULL);

        pItem = PX_Object_MenuAddItem(pdesc->menu,
                                      PX_Object_MenuGetRootItem(pdesc->menu),
                                      "designer",
                                      PX_NULL,
                                      PX_NULL);
        PX_Object_MenuAddItem(
            pdesc->menu, pItem, "open", PX_DesignerOnMenuOpen, pObject);
        PX_Object_MenuAddItem(
            pdesc->menu, pItem, "save", PX_DesignerOnMenuSave, pObject);
        PX_Object_MenuAddItem(
            pdesc->menu, pItem, "exit", PX_DesignerOnMenuExit, pObject);
        ...
        pItem = PX_Object_MenuAddItem(pdesc->menu,
                                      ...
                                      PX_NULL,
                                      PX_NULL);
        PX_Object_MenuAddItem(pdesc->menu,
                              pItem,
                              "controllers",
                              PX_DesignerOnMenuWindowControllers,
                              pObject);
        PX_Object_MenuAddItem(pdesc->menu,
                              pItem,
                              "properties",
                              PX_DesignerOnMenuWindowProperties,
                              pObject);
        PX_Object_MenuAddItem(pdesc->menu,
                              pItem,
                              "objects",
                              PX_DesignerOnMenuWindowObjects,
                              pObject);
        ...
        PX_Object_MenuAddItem(pdesc->menu,
                              pItem,
                              "ScrollBar",
                              PX_DesignerOnMenuViewScrollbar,
                              pObject);

    } while (0);

    for (i = 0; i < PX_DESIGNER_MAX_PROPERTYS; i++) {
        pdesc->label_propertys[i]
            = PX_Object_LabelCreate(mp,
                                    pdesc->widget_propertyscrollarea,
                                    5,
                                    3 + i * 24,
                                    64,
                                    24,
        ...
        pdesc->edit_propertys[i] = PX_Object_EditCreate(
            mp, pdesc->widget_propertyscrollarea, 74, 3 + i * 24, 96, 24, fm);
        PX_Object_GetEdit(pdesc->edit_propertys[i])->multiLines = PX_TRUE;
        pdesc->edit_propertys[i]->Visible = PX_FALSE;
        PX_ObjectRegisterEvent(pdesc->edit_propertys[i],
                               PX_OBJECT_EVENT_VALUECHANGED,
                               PX_Designer_OnPropertyChanged,
                               pObject);
        // PX_ObjectRegisterEvent(pdesc->edit_propertys[i],PX_OBJECT_EVENT_LOSTFOCUS,PX_Designer_OnPropertyRefresh,pObject);
    }

    pdesc->widget_controllers = PX_Object_WidgetCreate(
        mp, pObject, 5, 25, 192, 256, "controllers", PX_NULL);
    if (!pdesc->widget_controllers)
        return PX_FALSE;
    if (pdesc->pLinkWorld)
        pdesc->widget_controllers->x = pdesc->pLinkWorld->surface_width
                                       - pdesc->widget_controllers->Width - 25;

    pdesc->widget_objects = PX_Object_WidgetCreate(
        mp, pObject, 205, 25, 192, 256, "objects", PX_NULL);
    if (!pdesc->widget_objects)
        return PX_FALSE;

    if (!PX_VectorInitialize(
            mp, &pdesc->ObjectDesc, sizeof(PX_Designer_ObjectDesc), 16))
        return PX_FALSE;
    if (!PX_VectorInitialize(
            mp, &pdesc->Objects, sizeof(PX_Designer_Object), 16))
        return PX_FALSE;
    pdesc->mp = mp;
    pdesc->mp_ui = mp_ui;
    ...
    pdesc->list_ObjectDesc = PX_Object_ListCreate(
        mp,
        PX_Object_WidgetGetRoot(pdesc->widget_controllers),
        0,
        1,
        (px_int) PX_Object_GetWidget(pdesc->widget_controllers)
                ->renderTarget.width
            - 1,
        (px_int) PX_Object_GetWidget(pdesc->widget_controllers)
                ->renderTarget.height
    ...
    pdesc->list_Objects = PX_Object_ListCreate(
        mp,
        PX_Object_WidgetGetRoot(pdesc->widget_objects),
        0,
        1,
        (px_int) PX_Object_GetWidget(pdesc->widget_objects)->renderTarget.width
            - 1,
        (px_int) PX_Object_GetWidget(pdesc->widget_objects)->renderTarget.height
            - 1,
        PX_DESIGNER_CONTROLLER_ITEM_HEIGHT,
        PX_Designer_ObjectsListItemOnCreate,
        pObject);
    PX_ObjectRegisterEvent(pdesc->list_Objects,
                           PX_OBJECT_EVENT_VALUECHANGED,
                           PX_Designer_OnObjectsListChanged,
                           pObject);

    pdesc->designerbox
        = PX_Object_DesignerBoxCreate(mp, pObject, 0, 0, 100, 100);
    pdesc->designerbox->Visible = PX_FALSE;

    pdesc->button_delete
        = PX_Object_PushButtonCreate(mp, pObject, 0, 0, 16, 16, "x", PX_NULL);
    pdesc->button_delete->Visible = PX_FALSE;
    PX_ObjectRegisterEvent(pdesc->button_delete,
                           PX_OBJECT_EVENT_EXECUTE,
                           PX_Designer_OnDeleteCurrentObject,
                           pObject);

    pdesc->world_hscroll
        = PX_Object_SliderBarCreate(mp,
                                    pObject,
                                    0,
                                    0,
                                    10,
    ...
    if (pdesc->pLinkWorld
        && pdesc->pLinkWorld->world_width > pdesc->pLinkWorld->surface_width) {
        PX_Object_SliderBarSetRange(
            pdesc->world_hscroll,
            (px_int) (pdesc->pLinkWorld->surface_width / 2) - 1,
            (px_int) (pdesc->pLinkWorld->world_width
                      - pdesc->pLinkWorld->surface_width / 2)
                + 1);
    } else {
        pdesc->world_hscroll->Visible = PX_FALSE;
    }
    PX_ObjectRegisterEvent(pdesc->world_hscroll,
                           PX_OBJECT_EVENT_VALUECHANGED,
                           PX_Designer_OnScrollHChanged,
                           pObject);
    ...
    pdesc->world_vscroll
        = PX_Object_SliderBarCreate(mp,
                                    ...
                                    PX_OBJECT_SLIDERBAR_STYLE_BOX);
    if (pdesc->pLinkWorld
        && pdesc->pLinkWorld->world_height
               > pdesc->pLinkWorld->surface_height) {
        PX_Object_SliderBarSetRange(
            pdesc->world_vscroll,
            (px_int) (pdesc->pLinkWorld->surface_height / 2) - 1,
            (px_int) (pdesc->pLinkWorld->world_height
                      - pdesc->pLinkWorld->surface_height / 2)
                + 1);
    } else {
        pdesc->world_vscroll->Visible = PX_FALSE;
    }
    PX_ObjectRegisterEvent(pdesc->world_vscroll,
                           PX_OBJECT_EVENT_VALUECHANGED,
                           PX_Designer_OnScrollVChanged,
                           pObject);

    PX_ObjectRegisterEvent(
        pObject, PX_OBJECT_EVENT_CURSORDOWN, PX_Designer_OnCursorDown, pObject);
    PX_ObjectRegisterEvent(pdesc->designerbox,
                           PX_OBJECT_EVENT_VALUECHANGED,
                           PX_DesignerOnDesignerBoxValueChanged,
                           pObject);
...
px_bool PX_Object_DesignerExport(PX_Object *pObject, px_string *pText) {
    ...
    for (i = 0; i < designer->Objects.size; i++) {
        PX_Designer_Object *pdobject
            = PX_VECTORAT(PX_Designer_Object, &designer->Objects, i);
        PX_Designer_ObjectDesc *pobjectdesc = PX_VECTORAT(
            PX_Designer_ObjectDesc, &designer->ObjectDesc, pdobject->descIndex);
        px_int j;
        if (!pobjectdesc) {
            PX_ASSERT();
            goto _ERROR;
        ...
        if (!PX_StringCatEx(pText, "\""))
            goto _ERROR;
        if (!PX_StringCatEx(pText, pobjectdesc->Name))
            goto _ERROR;
        if (!PX_StringCatEx(pText, "\":{\n"))
            goto _ERROR;
        ...
        for (j = 0; j < PX_COUNTOF(pobjectdesc->properties); j++) {
            ...
            if (pobjectdesc->properties[j].getbool) {
                if (pobjectdesc->properties[j].getbool(pdobject->pObject)) {
                    if (!PX_StringCatEx(pText, "\""))
                        goto _ERROR;
                    if (!PX_StringCatEx(pText, pobjectdesc->properties[j].Name))
                        goto _ERROR;
                    if (!PX_StringCatEx(pText, "\":true"))
                        goto _ERROR;
                } else {
                    if (!PX_StringCatEx(pText, "\""))
                        goto _ERROR;
                ...
                if (!PX_StringCatEx(pText, "\":"))
                    goto _ERROR;
                ret = PX_ftos(
                    pobjectdesc->properties[j].getfloat(pdobject->pObject), 2);
                if (!PX_StringCatEx(pText, ret.data))
                    goto _ERROR;
                ...
                if (!PX_StringCatEx(pText, "\":"))
                    goto _ERROR;
                ret = PX_itos(
                    pobjectdesc->properties[j].getint(pdobject->pObject), 10);
                if (!PX_StringCatEx(pText, ret.data))
                    goto _ERROR;
...
px_bool PX_Object_DesignerImportToUIObject(px_memorypool *mp,
    ...
    for (i = 0; i < Json.rootValue._object.values.size; i++) {
        PX_Json_Value *pJsonValue
            = PX_JsonGetObjectValueByIndex(&Json.rootValue, i);
        PX_Designer_ObjectDesc *pObjectDesc;
        ...
        for (j = 0; j < descvec.size; j++) {
            PX_Designer_ObjectDesc *pEnumObjectDesc
                = PX_VECTORAT(PX_Designer_ObjectDesc, &descvec, j);
        ...
        if (pObjectDesc) {
            PX_Json_Value *TypeValue
                = PX_JsonGetObjectValue(pJsonValue, "type");
            ...
            if (TypeValue && TypeValue->type == PX_JSON_VALUE_TYPE_STRING) {
                ...
                switch (type) {
                    case PX_DESIGNER_OBJECT_TYPE_UI: {
                        pNewObject = pObjectDesc->createfunc(
                            mp, pRootObject, 0, 0, 100, 100, fm);
                    } break;
                    default:
                        break;
                }
            ...
            if (pNewObject) {
                for (j = 0; j < PX_COUNTOF(pObjectDesc->properties); j++) {
                    ...
                    if (pObjectDesc->properties[j].setbool) {
                        PX_Json_Value *pvalue = PX_JsonGetObjectValue(
                            pJsonValue, pObjectDesc->properties[j].Name);
                        if (pvalue
                            && pvalue->type == PX_JSON_VALUE_TYPE_BOOLEAN) {
                            pObjectDesc->properties[j].setbool(
                                pNewObject, pvalue->_boolean);
                        }
                        ...
                        if (pvalue
                            ...
                            pObjectDesc->properties[j].setfloat(
                                pNewObject, (px_float) pvalue->_number);
                        }
                        ...
                        if (pvalue
                            && pvalue->type == PX_JSON_VALUE_TYPE_NUMBER) {
                            pObjectDesc->properties[j].setint(
                                pNewObject, (px_int) pvalue->_number);
                        }
                        ...
                        if (pvalue
                            ...
                            pObjectDesc->properties[j].setstring(
                                pNewObject, pvalue->_string.buffer);
                        }
                    }
                }
            }
...
px_bool PX_Object_DesignerImport(PX_Object *pObject, const px_char *pText) {
    PX_Json Json;
    px_int i;
    PX_Object_Designer *pdesigner
        = PX_ObjectGetDesc(PX_Object_Designer, pObject);
    if (!PX_JsonInitialize(pdesigner->mp, &Json))
        return PX_FALSE;
    if (!PX_JsonParse(&Json, pText)) {
        goto _ERROR;
    }
    for (i = 0; i < Json.rootValue._object.values.size; i++) {
        ...
        for (j = 0; j < pdesigner->ObjectDesc.size; j++) {
            PX_Designer_ObjectDesc *pEnumObjectDesc = PX_VECTORAT(
                PX_Designer_ObjectDesc, &pdesigner->ObjectDesc, j);
            if (PX_strequ(pJsonValue->name.buffer, pEnumObjectDesc->Name)) {
                pObjectDesc = pEnumObjectDesc;
                descindex = j;
                break;
            ...
            if (TypeValue && TypeValue->type == PX_JSON_VALUE_TYPE_STRING) {
                ...
                switch (type) {
                    case PX_DESIGNER_OBJECT_TYPE_UI: {
                        PX_Designer_Object dobject;
                        pNewObject
                            = pObjectDesc->createfunc(pdesigner->mp_ui,
                                                      pdesigner->pLinkObject,
                                                      0,
                                                      0,
                                                      100,
                                                      100,
                    ...
                    case PX_DESIGNER_OBJECT_TYPE_GAME: {
                        PX_Designer_Object dobject;
                        pNewObject
                            = pObjectDesc->createfunc(pdesigner->mp_game,
                                                      PX_NULL,
                                                      0,
                                                      0,
                                                      100,

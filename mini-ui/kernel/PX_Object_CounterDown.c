...
px_void PX_Object_CounterDownUpdate(PX_Object *pObject, px_dword elapsed) {
    PX_Object_CounterDown *pdesc
        = PX_ObjectGetDesc(PX_Object_CounterDown, pObject);
    if (pdesc->ms_down > (px_int) elapsed) {
        ...
        if (pdesc->ms_down) {
            ...
            PX_ObjectExecuteEvent(
                pObject, PX_OBJECT_BUILD_EVENT(PX_OBJECT_EVENT_EXECUTE));
        }
    }
}

px_void PX_Object_CounterDownRender(px_surface *psurface,
                                    PX_Object *pObject,
                                    px_dword elapsed) {
    px_char n[16];
    ...
    do {
        ...
        PX_GeoDrawCircle(
            ...
            (px_int) objx,
            (px_int) objy,
            (px_int) (pObject->diameter / 2 + (pObject->diameter * (a))),
            (px_int) (16 * a),
            clr);
        ...
        PX_GeoDrawSolidCircle(psurface,
                              (px_int) objx,
                              (px_int) objy,
                              (px_int) (a * pObject->diameter / 2),
                              pdesc->backgroundcolor);
        PX_GeoDrawSector(psurface,
                         (px_int) objx,
                         (px_int) objy,
                         (a * (pObject->diameter / 2 - 5)),
                         (a * (pObject->diameter / 2 - 1)),
                         pdesc->ringcolor,
                         -90,
...
PX_Object *PX_Object_CounterDownCreate(px_memorypool *mp,
                                       PX_Object *Parent,
                                       px_float x,
                                       px_float y,
                                       px_float radius,
    ...
    pObject = PX_ObjectCreateEx(mp,
                                ...
                                PX_Object_CounterDownUpdate,
                                PX_Object_CounterDownRender,
                                0,
                                &Desc,
                                sizeof(Desc));
...
px_void PX_Object_CounterDownSetBackgroundColor(PX_Object *pObject,
                                                px_color clr) {
    PX_Object_CounterDown *pdesc = PX_Object_GetCounterDown(pObject);
    if (pdesc) {

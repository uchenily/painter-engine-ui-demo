...
px_void PX_Object_PancRender(px_surface *pSurface,
                             PX_Object *pObject,
                             px_dword elapsed) {
    ...
    PX_GeoDrawSolidCircle(pSurface,
                          (px_int) pDesc->currentX,
                          (px_int) pDesc->currentY,
                          8,
                          renderColor);

    PX_GeoDrawSolidCircle(
        pSurface, (px_int) (objx), (px_int) (objy), 5, renderColor);
    PX_GeoDrawSolidCircle(
        pSurface, (px_int) (objx + objWidth), (px_int) (objy), 5, renderColor);
    PX_GeoDrawSolidCircle(
        pSurface, (px_int) (objx), (px_int) (objy + objHeight), 5, renderColor);
    PX_GeoDrawSolidCircle(pSurface,
                          (px_int) (objx + objWidth),
                          (px_int) (objy + objHeight),
                          5,
...
px_void PX_Object_PancOnCursorDown(PX_Object *pObject,
                                   PX_Object_Event e,
                                   px_void *ptr) {
    px_float x, y;
    px_float objx, objy, objWidth, objHeight;
    ...
    if ((x - pDesc->currentX) * (x - pDesc->currentX)
            + (y - pDesc->currentY) * (y - pDesc->currentY)
        < 64) {
    ...
    if ((x - objx - objWidth) * (x - objx - objWidth) + (y - objy) * (y - objy)
        < 25) {
    ...
    if ((x - objx) * (x - objx)
            + (y - objy - objHeight) * (y - objy - objHeight)
        < 25) {
        pDesc->bselect = PX_Object_Panc_bselect_leftbottom;
        return;
    }

    if ((x - objx - objWidth) * (x - objx - objWidth)
            + (y - objy - objHeight) * (y - objy - objHeight)
        < 25) {
...
px_void PX_Object_PancOnCursorRDown(PX_Object *pObject,
                                    PX_Object_Event e,
                                    px_void *ptr) {
    px_float x, y;
    PX_Object_Panc *pDesc = PX_ObjectGetDesc(PX_Object_Panc, pObject);
...
px_void PX_Object_PancOnCursorDrag(PX_Object *pObject,
                                   PX_Object_Event e,
                                   px_void *ptr) {
    px_float x, y;
    PX_Object_Panc *pDesc = PX_ObjectGetDesc(PX_Object_Panc, pObject);
...
PX_Object *PX_Object_PancCreate(px_memorypool *mp,
                                PX_Object *Parent,
                                px_float x,
                                px_float y,
                                px_float widht,
...
px_void PX_Object_PancReset(PX_Object *pObject,
                            px_float x,
                            px_float y,
                            px_float width,
                            px_float height) {

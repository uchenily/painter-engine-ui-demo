...
px_void PX_Object_ModbusVarBoxOnConfirm(PX_Object *pObject,
                                        PX_Object_Event e,
                                        px_void *ptr) {
    PX_Object_ModbusVarBox *pdesc
        = PX_ObjectGetDesc(PX_Object_ModbusVarBox, ((PX_Object *) ptr));
    ...
    if (pdesc->EditMode)
        ...
        PX_ObjectExecuteEvent(((PX_Object *) ptr),
                              PX_OBJECT_BUILD_EVENT(PX_OBJECT_EVENT_EXECUTE));
    PX_Object_ModbusVarBoxClose((PX_Object *) ptr);
}

px_void PX_Object_ModbusVarBoxOnKeyDown(PX_Object *pObject,
                                        PX_Object_Event e,
                                        px_void *ptr) {
    if (e.Event == PX_OBJECT_EVENT_KEYDOWN
        && PX_Object_Event_GetKeyDown(e) == '\r') {
        PX_Object_ModbusVarBoxOnConfirm(
            pObject, PX_OBJECT_BUILD_EVENT(PX_OBJECT_EVENT_EXECUTE), ptr);
    }
}

px_void PX_Object_ModbusVarBoxOnCancel(PX_Object *pObject,
                                       PX_Object_Event e,
                                       px_void *ptr) {
    PX_Object_ModbusVarBoxClose((PX_Object *) ptr);
}

px_void PX_Object_ModbusVarBoxOnTypeChanged(PX_Object *pObject,
                                            PX_Object_Event e,
                                            px_void *ptr) {
    PX_Object_ModbusVarBox *pDesc = (PX_Object_ModbusVarBox *) ptr;
}

PX_Object *PX_Object_ModbusVarBoxCreate(px_memorypool *mp,
                                        PX_Object *Parent,
                                        px_int x,
                                        px_int y,
                                        px_int width,
    ...
    pdesc->widget = PX_Object_WidgetCreate(
        mp, pObject, 0, 0, width, height, "", fontmodule);
    PX_Object_WidgetShowHideCloseButton(pdesc->widget, PX_FALSE);
    pdesc->various_name = PX_Object_VariousCreate(mp,
                                                  pdesc->widget,
                                                  width / 2 - 128,
                                                  32,
                                                  256,
                                                  ...
                                                  fontmodule);
    PX_Object_VariousSetEditStyle(pdesc->various_name,
                                  PX_OBJECT_VARIOUS_EDIT_TYPE_STRING);
    pdesc->various_address
        = PX_Object_VariousCreate(mp,
                                  pdesc->widget,
                                  width / 2 - 128,
                                  72,
                                  256,
                                  ...
                                  PX_OBJECT_VARIOUS_TYPE_EDIT,
                                  fontmodule);
    PX_Object_VariousSetEditStyle(pdesc->various_address,
                                  PX_OBJECT_VARIOUS_EDIT_TYPE_HEX);

    pdesc->various_value = PX_Object_VariousCreate(mp,
                                                   pdesc->widget,
                                                   width / 2 - 128,
                                                   112,
                                                   256,
                                                   ...
                                                   PX_OBJECT_VARIOUS_TYPE_EDIT,
                                                   fontmodule);
    PX_Object_VariousSetEditStyle(pdesc->various_value,
                                  PX_OBJECT_VARIOUS_EDIT_TYPE_HEX);

    pdesc->selectbar_type = PX_Object_SelectBarCreate(
        mp, pdesc->widget, width / 2 - 128, 152, 256, 32, fontmodule);
    ...
    PX_ObjectRegisterEvent(pdesc->selectbar_type,
                           PX_OBJECT_EVENT_VALUECHANGED,
                           PX_Object_ModbusVarBoxOnTypeChanged,
                           pdesc);

    pdesc->checkbox_readonly = PX_Object_CheckBoxCreate(mp,
                                                        pdesc->widget,
                                                        width / 2 - 128,
                                                        192,
                                                        256,
                                                        ...
                                                        "Read Only",
                                                        fontmodule);
    pdesc->btn_ok = PX_Object_PushButtonCreate(
        mp, pdesc->widget, width - 256, height - 68, 96, 32, "OK", fontmodule);
    pdesc->btn_close = PX_Object_PushButtonCreate(mp,
                                                  pdesc->widget,
                                                  width - 128,
                                                  height - 68,
                                                  96,
    ...
    PX_ObjectRegisterEvent(pdesc->btn_ok,
                           PX_OBJECT_EVENT_EXECUTE,
                           PX_Object_ModbusVarBoxOnConfirm,
                           pObject);
    PX_ObjectRegisterEvent(pdesc->btn_close,
                           PX_OBJECT_EVENT_EXECUTE,
                           PX_Object_ModbusVarBoxOnCancel,
                           pObject);
    PX_ObjectRegisterEvent(pdesc->widget,
                           PX_OBJECT_EVENT_KEYDOWN,
                           PX_Object_ModbusVarBoxOnKeyDown,
                           pObject);
...
px_void PX_Object_ModbusVarBoxShow(PX_Object *pObject) {
    ...
    PX_Object_ModbusVarBox *pDesc
        = PX_ObjectGetDesc(PX_Object_ModbusVarBox, pObject);
    if (pObject->Type != PX_OBJECT_TYPE_MODBUSVARBOX) {
        PX_ASSERT();
        return;
    }
...
px_void PX_Object_ModbusVarBoxShowEdit(PX_Object *pObject,
                                       const px_char name[],
                                       px_dword address,
                                       px_int type,
                                       px_word value) {

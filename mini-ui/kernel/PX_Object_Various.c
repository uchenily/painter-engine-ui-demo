...
px_void PX_Object_VariousOnEditChange(PX_Object *pEObject,
                                      PX_Object_Event e,
                                      px_void *ptr) {
...
px_void PX_Object_VariousRender(px_surface *psurface,
                                PX_Object *pObject,
                                px_dword elapsed) {
    PX_Object_Various *pDesc = PX_ObjectGetDesc(PX_Object_Various, pObject);

    pDesc->label->Width = pObject->Width * 0.25f;
    if (pDesc->label->Width
        < PX_Object_LabelGetTextRenderWidth(pDesc->label) + 16) {
        pDesc->label->Width
            = PX_Object_LabelGetTextRenderWidth(pDesc->label) + 16.f;
    }
...
PX_Object *PX_Object_VariousCreate(px_memorypool *mp,
                                   PX_Object *Parent,
                                   px_int x,
                                   px_int y,
                                   px_int Width,
    ...
    switch (type) {
        ...
        case PX_OBJECT_VARIOUS_TYPE_SELECTBAR: {
            pdesc->various
                = PX_Object_SelectBarCreate(mp, pObject, 0, 0, 1, 1, fm);
            PX_ObjectRegisterEvent(pdesc->various,
                                   PX_OBJECT_EVENT_VALUECHANGED,
                                   PX_Object_VariousOnEditChange,
                                   pObject);
...
px_void PX_Object_VariousSetEditStyle(PX_Object *pObject,
                                      PX_OBJECT_VARIOUS_EIDT_TYPE type) {
    if (pObject->Type != PX_OBJECT_TYPE_VARIOUS) {
        PX_ASSERT();
        return;
    } else {
        ...
        if (pDesc->type == PX_OBJECT_VARIOUS_TYPE_EDIT) {
            switch (type) {
                ...
                case PX_OBJECT_VARIOUS_EDIT_TYPE_HEX: {
                    PX_Object_EditSetLimit(pDesc->various,
                                           "-01234567890ABCDEFabcdef");
                    PX_Object_EditSetInputMode(
                        pDesc->various, PX_OBJECT_EDIT_INPUT_MODE_UPPERCASE);
                } break;
                default:
                    break;
            }
...
const px_char *PX_Object_VariousGetText(PX_Object *pObject) {
    if (pObject->Type != PX_OBJECT_TYPE_VARIOUS) {
        ...
        switch (pDesc->type) {
            case PX_OBJECT_VARIOUS_TYPE_EDIT: {
                return PX_Object_EditGetText(
                    PX_Object_GetVarious(pObject)->various);
            }
            case PX_OBJECT_VARIOUS_TYPE_LABEL: {
                return PX_Object_LabelGetText(
                    PX_Object_GetVarious(pObject)->various);
            }
            case PX_OBJECT_VARIOUS_TYPE_SELECTBAR: {
                return PX_Object_SelectBarGetCurrentText(
                    PX_Object_GetVarious(pObject)->various);
            }
            default:
                break;
    ...
    if (pObject->Type != PX_OBJECT_TYPE_VARIOUS) {
        ...
        switch (pDesc->type) {
            case PX_OBJECT_VARIOUS_TYPE_EDIT: {
                ...
                PX_Object_EditSetText(PX_Object_GetVarious(pObject)->various,
                                      Text);
                PX_Object_EditSetXYOffset(
                    PX_Object_GetVarious(pObject)->various, 0, 0);
            } break;
            case PX_OBJECT_VARIOUS_TYPE_LABEL: {
                PX_Object_LabelSetText(PX_Object_GetVarious(pObject)->various,
                                       Text);
            } break;
            case PX_OBJECT_VARIOUS_TYPE_SELECTBAR: {
                PX_ASSERT();
...
px_void PX_Object_VariousSetBackgroundColor(PX_Object *pObject,
                                            px_color Color) {
    if (pObject->Type != PX_OBJECT_TYPE_VARIOUS) {
        PX_ASSERT();
        return;
        ...
        switch (pDesc->type) {
            case PX_OBJECT_VARIOUS_TYPE_EDIT: {
                PX_Object_LabelSetBackgroundColor(
                    PX_Object_GetVarious(pObject)->label, Color);
                PX_Object_EditSetBackgroundColor(
                    PX_Object_GetVarious(pObject)->various, Color);
            } break;
            case PX_OBJECT_VARIOUS_TYPE_LABEL: {
                PX_Object_LabelSetBackgroundColor(
                    PX_Object_GetVarious(pObject)->label, Color);
            ...
            case PX_OBJECT_VARIOUS_TYPE_SELECTBAR: {
                PX_Object_LabelSetBackgroundColor(
                    PX_Object_GetVarious(pObject)->label, Color);
                PX_Object_SelectBarSetBackgroundColor(
                    PX_Object_GetVarious(pObject)->various, Color);
            } break;
            default:
                break;

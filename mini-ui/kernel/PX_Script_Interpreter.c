...
static px_char *PX_Script_Keywords[] = {"IF",
                                        "ELSE",
                                        "SWITCH",
                                        "CASE",
                                        "WHILE",
...
px_void PX_ScriptTranslatorError(PX_ScriptInterpreter *analysis,
                                 px_char *info) {
    if (analysis) {
        px_lexer *lexer = &analysis->lexer;
        PX_memset(analysis->PX_Script_InterpreterError,
                  0,
                  sizeof(analysis->PX_Script_InterpreterError));
        if (lexer->Sources) {
            lexer->Sources[lexer->SourceOffset] = '\0';
            analysis->PX_Script_InterpreterError[0] = '\0';
            if (lexer->SourceOffset > 200) {
                PX_strcpy(analysis->PX_Script_InterpreterError,
                          lexer->Sources + lexer->SourceOffset - 200,
                          200);
            } else {
                PX_strcpy(
                    analysis->PX_Script_InterpreterError, lexer->Sources, 200);
            }
        }
...
px_void PX_ScriptParserClearStack(PX_ScriptInterpreter *analysis) {
    ...
    for (i = 0; i < analysis->v_variablesStackTable.size; i++) {
        pvar = PX_VECTORAT(
            PX_SCRIPT_VARIABLES, &analysis->v_variablesStackTable, i);
...
px_void PX_ScriptCompilerFree(PX_SCRIPT_LIBRARY *lib) {
    ...
    for (i = 0; i < lib->codeLibraries.size; i++) {
        PX_StringFree(
            &PX_VECTORAT(PX_SCRIPT_CODE, &lib->codeLibraries, i)->name);
        PX_StringFree(
            &PX_VECTORAT(PX_SCRIPT_CODE, &lib->codeLibraries, i)->code);
    }
...
px_bool PX_ScriptCompilerLoad(PX_SCRIPT_LIBRARY *lib, const px_char *code) {
    ...
    for (i = 0; i < lib->codeLibraries.size; i++) {
        if (PX_strequ(scode.name.buffer,
                      PX_VECTORAT(PX_SCRIPT_CODE, &lib->codeLibraries, i)
                          ->name.buffer)) {
            PX_StringFree(&scode.name);
            PX_StringFree(&scode.code);
...
static px_bool PX_ScriptParseInclude(PX_ScriptInterpreter *analysis,
                                     px_string *codes,
    ...
    while (PX_TRUE) {
        ...
        if (type == PX_LEXER_LEXEME_TYPE_TOKEN) {
            if (PX_strequ(lexer.CurLexeme.buffer, "#INCLUDE")) {
                if (PX_ScriptTranslatorNextToken(&lexer)
                    == PX_LEXER_LEXEME_TYPE_CONATINER) {
                    if (PX_LexerGetCurrentContainerType(&lexer) != quotes) {
                        PX_ScriptTranslatorError(
                            analysis,
                            "syntactic error: include \"name\" expected but "
                            "not found.");
                        goto _ERROR;
                    }
                    ...
                    bfound = PX_FALSE;
                    for (i = 0; i < lib->codeLibraries.size; i++) {
                        if (PX_strequ(lexer.CurLexeme.buffer,
                                      PX_VECTORAT(PX_SCRIPT_CODE,
                                                  &lib->codeLibraries,
                                                  i)
                                          ->name.buffer)) {
                            if (PX_VECTORAT(
                                    PX_SCRIPT_CODE, &lib->codeLibraries, i)
                                    ->bInclude
                                != PX_FALSE) {
                                PX_ScriptTranslatorError(
                                    analysis,
                                    "syntactic error: reduplicate "
                                    "included-name.");
                                goto _ERROR;
                            }
                            PX_VECTORAT(PX_SCRIPT_CODE, &lib->codeLibraries, i)
                            ...
                            if ((type = PX_ScriptTranslatorNextToken(&lexer))
                                != PX_LEXER_LEXEME_TYPE_NEWLINE) {
                                if (type == PX_LEXER_LEXEME_TYPE_END) {
                                    PX_ScriptTranslatorError(
                                        analysis,
                                        "syntactic error:include on last line "
                                        "without newline");
                                    goto _ERROR;
                                } else {
                                    PX_ScriptTranslatorError(
                                        analysis,
                                        "syntactic error:new line expected but "
                                        "not found.");
                                    goto _ERROR;
                                }
                            }
                            allocsize
                                = lexerState.offset
                                  + PX_strlen(PX_VECTORAT(PX_SCRIPT_CODE,
                                                          &lib->codeLibraries,
                                                          i)
                                                  ->code.buffer)
                                  + PX_strlen(lexer.Sources
                                              + PX_LexerGetState(&lexer).offset
                                              - 1)
                                  + 1;
                            exchangeBuffer
                                = (px_char *) MP_Malloc(lib->mp, allocsize);
                            ...
                            PX_memcpy(exchangeBuffer,
                                      codes->buffer,
                                      lexerState.offset);
                            exchangeBuffer[lexerState.offset] = 0;
                            PX_strcat(exchangeBuffer,
                                      PX_VECTORAT(PX_SCRIPT_CODE,
                                                  &lib->codeLibraries,
                                                  i)
                                          ->code.buffer);
                    ...
                    if (!bfound) {
                        PX_ScriptTranslatorError(
                            analysis, "include error: source not found.");
                        goto _ERROR;
                    }
                } else {
                    PX_ScriptTranslatorError(
                        analysis,
                        "syntactic error:include-name expected but not found.");
                    goto _ERROR;
                }
            }
        }
...
static px_bool PX_ScriptParseDefine(PX_ScriptInterpreter *analysis,
                                    px_string *codes,
                                    PX_SCRIPT_LIBRARY *lib,
                                    const px_char *name) {
    px_lexer lexer;
    ...
    PX_VectorInitialize(
        lib->mp, &defines, sizeof(PX_SCRIPT_TRANSLATOR_DEFINE_ST), 32);
    ...
    while (PX_TRUE) {
        ...
        if (type == PX_LEXER_LEXEME_TYPE_TOKEN) {
            if (PX_strequ(lexer.CurLexeme.buffer, "#DEFINE")) {
                ...
                if (PX_LexerGetNextLexeme(&lexer)
                    != PX_LEXER_LEXEME_TYPE_SPACER) {
                    PX_ScriptTranslatorError(
                        analysis,
                        "syntactic error:spacer expected but not found.");
                    goto _ERROR;
                }
                ...
                PX_StringReplaceRange(
                    codes, startIndex, lexer.SourceOffset, "");
                PX_LexerSetSourcePointer(&lexer, codes->buffer);
    ...
    while (PX_TRUE) {
        ...
        if (type == PX_LEXER_LEXEME_TYPE_TOKEN) {
            for (i = 0; i < defines.size; i++) {
                if (PX_strequ(
                        lexer.CurLexeme.buffer,
                        PX_VECTORAT(PX_SCRIPT_TRANSLATOR_DEFINE_ST, &defines, i)
                            ->name.buffer)) {
                    startIndex = lexer.SourceOffset
                                 - PX_strlen(lexer.CurLexeme.buffer);
                    PX_StringReplaceRange(
                        codes,
                        startIndex,
                        lexer.SourceOffset - 1,
                        PX_VECTORAT(PX_SCRIPT_TRANSLATOR_DEFINE_ST, &defines, i)
                            ->token.buffer);
                    PX_LexerSetSourcePointer(&lexer, codes->buffer);
                    lexer.SourceOffset
                        = startIndex
                          + PX_strlen(
                              PX_VECTORAT(
                                  PX_SCRIPT_TRANSLATOR_DEFINE_ST, &defines, i)
                                  ->token.buffer);
                    break;
                }
            }
    ...
    for (i = 0; i < defines.size; i++) {
        PX_StringFree(
            &PX_VECTORAT(PX_SCRIPT_TRANSLATOR_DEFINE_ST, &defines, i)->name);
        PX_StringFree(
            &PX_VECTORAT(PX_SCRIPT_TRANSLATOR_DEFINE_ST, &defines, i)->token);
    }
...
static px_bool PX_ScriptParsePretreatment(PX_ScriptInterpreter *analysis,
                                          px_string *codes,
                                          PX_SCRIPT_LIBRARY *lib,
                                          const px_char *name) {
...
static px_bool PX_ScriptParseIsValidToken(px_char *token) {
    ...
    while (*token) {
        if ((*token >= 'A' && *token <= 'Z') || (*token >= 'a' && *token <= 'z')
            || *token == '_' || *token == ':' || PX_charIsNumeric(*token)) {
            token++;
            continue;
        }
        return PX_FALSE;
...
static PX_SCRIPT_STRUCT *
PX_ScriptParseGetStructByIndex(PX_ScriptInterpreter *analysis, px_int index) {
...
static px_bool PX_ScriptParse_AST_PushToken(PX_ScriptInterpreter *analysis,
                                            px_vector *op,
                                            px_vector *tk,
                                            PX_SCRIPT_EXPR_STREAM atom,
                                            PX_SCRIPT_STRUCT *_inset,
                                            PX_SCRIPT_STRUCT **_outset) {
    px_int i;
    PX_SCRIPT_VARIABLES *pvar;
                ...
                *_outset = PX_NULL;
                switch (psetmem->defvar.type) {
                    ...
                    case PX_SCRIPT_PARSER_VAR_TYPE_STRUCT: {
                        ...
                        operand._oft = psetmem->offset;
                        operand.pStruct = PX_ScriptParseGetStructByIndex(
                            analysis, psetmem->defvar.setIndex);
                    ...
                    case PX_SCRIPT_PARSER_VAR_TYPE_INT_ARRAY: {
                        operand.operandType
                            = PX_SCRIPT_AST_OPERAND_TYPE_INT_PTR_CONST;
                        operand._oft = psetmem->offset;
                        operand.pStruct = PX_NULL;
                        PX_VectorPushback(tk, &operand);
                        return PX_TRUE;
                    ...
                    case PX_SCRIPT_PARSER_VAR_TYPE_FLOAT_ARRAY: {
                        operand.operandType
                            = PX_SCRIPT_AST_OPERAND_TYPE_FLOAT_PTR_CONST;
                        operand._oft = psetmem->offset;
                        operand.pStruct = PX_NULL;
                        PX_VectorPushback(tk, &operand);
                        return PX_TRUE;
                    ...
                    case PX_SCRIPT_PARSER_VAR_TYPE_STRING_ARRAY: {
                        operand.operandType
                            = PX_SCRIPT_AST_OPERAND_TYPE_STRING_PTR_CONST;
                        operand._oft = psetmem->offset;
                        operand.pStruct = PX_NULL;
                        PX_VectorPushback(tk, &operand);
                        return PX_TRUE;
                    ...
                    case PX_SCRIPT_PARSER_VAR_TYPE_MEMORY_ARRAY: {
                        operand.operandType
                            = PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_PTR_CONST;
                        operand._oft = psetmem->offset;
                        operand.pStruct = PX_NULL;
                        PX_VectorPushback(tk, &operand);
                        return PX_TRUE;
                    ...
                    case PX_SCRIPT_PARSER_VAR_TYPE_SET_ARRAY: {
                        ...
                        operand.operandType
                            = PX_SCRIPT_AST_OPERAND_TYPE_STRUCT_PTR_CONST;
                        operand._oft = psetmem->offset;
                        operand.pStruct = PX_ScriptParseGetStructByIndex(
                            analysis, psetmem->defvar.setIndex);
                        *_outset = operand.pStruct;
                    ...
                    case PX_SCRIPT_PARSER_VAR_TYPE_INT_PTR: {
                        operand.operandType
                            = PX_SCRIPT_AST_OPERAND_TYPE_INT_PTR;
                        operand._oft = psetmem->offset;
                        operand.pStruct = PX_NULL;
                        PX_VectorPushback(tk, &operand);
                        return PX_TRUE;
                    ...
                    case PX_SCRIPT_PARSER_VAR_TYPE_FLOAT_PTR: {
                        operand.operandType
                            = PX_SCRIPT_AST_OPERAND_TYPE_FLOAT_PTR;
                        operand._oft = psetmem->offset;
                        operand.pStruct = PX_NULL;
                        PX_VectorPushback(tk, &operand);
                        return PX_TRUE;
                    ...
                    case PX_SCRIPT_PARSER_VAR_TYPE_STRING_PTR: {
                        operand.operandType
                            = PX_SCRIPT_AST_OPERAND_TYPE_STRING_PTR;
                        operand._oft = psetmem->offset;
                        operand.pStruct = PX_NULL;
                        PX_VectorPushback(tk, &operand);
                        return PX_TRUE;
                    ...
                    case PX_SCRIPT_PARSER_VAR_TYPE_MEMORY_PTR: {
                        operand.operandType
                            = PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_PTR;
                        operand._oft = psetmem->offset;
                        operand.pStruct = PX_NULL;
                        PX_VectorPushback(tk, &operand);
                        return PX_TRUE;
                    ...
                    case PX_SCRIPT_PARSER_VAR_TYPE_STRUCT_PTR: {
                        operand.operandType
                            = PX_SCRIPT_AST_OPERAND_TYPE_STRUCT_PTR;
                        operand._oft = psetmem->offset;
                        operand.pStruct = PX_ScriptParseGetStructByIndex(
                            analysis, psetmem->defvar.setIndex);
                        *_outset = operand.pStruct;
    ...
    if (atom.opclass == PX_SCRIPT_TRANSLATOR_OP_CLASS_VAR) {
        ...
        for (i = 0; i < analysis->v_variablesGlobalTable.size; i++) {
            pvar = PX_VECTORAT(
                PX_SCRIPT_VARIABLES, &analysis->v_variablesGlobalTable, i);
            if (PX_strequ(pvar->Mnemonic.buffer, atom.code.buffer)) {
    ...
    _CONTINUE:
        switch (pvar->type) {
            ...
            case PX_SCRIPT_PARSER_VAR_TYPE_STRUCT: {
                *_outset
                    = PX_ScriptParseGetStructByIndex(analysis, pvar->setIndex);
                operand.operandType = PX_SCRIPT_AST_OPERAND_TYPE_STRUCT;
                operand._oft = pvar->BeginIndex;
                operand.pStruct
                    = PX_ScriptParseGetStructByIndex(analysis, pvar->setIndex);
...
static px_bool PX_ScriptParseAST_PushOpcode(PX_ScriptInterpreter *analysis,
                                            px_vector *op,
                                            PX_SCRIPT_EXPR_STREAM atom) {
...
static PX_SCRIPT_STRUCT *
PX_ScriptParseGetStructInfo(PX_ScriptInterpreter *analysis, px_char *name) {
    px_int i;
...
static px_int PX_ScriptParseGetSetIndex(PX_ScriptInterpreter *analysis,
                                        px_char *name) {
    px_int i;
    PX_SCRIPT_STRUCT *pset;
    for (i = 0; i < analysis->v_struct.size; i++) {
...
static px_bool
PX_ScriptParseIsCompareAbleOperand(PX_SCRIPT_AST_OPERAND operand1,
                                   PX_SCRIPT_AST_OPERAND operand2) {
    if (!PX_ScriptParseIsOperandNumericType(operand1)
        || !PX_ScriptParseIsOperandNumericType(operand2)) {
            ...
            switch (operand.region) {
                ...
                case PX_SCRIPT_VARIABLE_REGION_GLOBAL: {
                    ...
                    if (operand.bAtomPopIndex) {
                        PX_StringCat(out, "POP R3\n");
                        PX_StringFormat1(&fmrString,
                                         "STRCHR R2,GLOBAL[%1],R3\n",
                                         PX_STRINGFORMAT_INT(operand._oft));
                        PX_StringCat(out, fmrString.buffer);
                    } else {
                        PX_StringFormat2(
                            &fmrString,
                            "STRCHR R2,GLOBAL[%1],%2\n",
                            PX_STRINGFORMAT_INT(operand._oft),
                            PX_STRINGFORMAT_INT(operand.bAtomPopIndex));
                        PX_StringCat(out, fmrString.buffer);
                    }
            ...
            switch (operand.region) {
                case PX_SCRIPT_VARIABLE_REGION_LOCAL: {
                    ...
                    if (operand.bAtomPopIndex) {
                        PX_StringCat(out, "POP R3\n");
                        PX_StringFormat1(&fmrString,
                                         "MEMBYTE R2,LOCAL[%1],R3\n",
                                         PX_STRINGFORMAT_INT(operand._oft));
                        PX_StringCat(out, fmrString.buffer);
                    } else {
                        PX_StringFormat2(
                            &fmrString,
                            "MEMBYTE R2,LOCAL[%1],%2\n",
                            PX_STRINGFORMAT_INT(operand._oft),
                            PX_STRINGFORMAT_INT(operand.bAtomPopIndex));
                        PX_StringCat(out, fmrString.buffer);
                    }
                ...
                case PX_SCRIPT_VARIABLE_REGION_GLOBAL: {
                    ...
                    if (operand.bAtomPopIndex) {
                        PX_StringCat(out, "POP R3\n");
                        PX_StringFormat1(&fmrString,
                                         "MEMBYTE R2,GLOBAL[%1],R3\n",
                                         PX_STRINGFORMAT_INT(operand._oft));
                        PX_StringCat(out, fmrString.buffer);
                    } else {
                        PX_StringFormat2(
                            &fmrString,
                            "MEMBYTE R2,GLOBAL[%1],%2\n",
                            PX_STRINGFORMAT_INT(operand._oft),
                            PX_STRINGFORMAT_INT(operand.bAtomPopIndex));
                        PX_StringCat(out, fmrString.buffer);
                    }
            ...
            switch (operand.region) {
                ...
                case PX_SCRIPT_VARIABLE_REGION_GLOBAL: {
                    ...
                    if (operand.bAtomPopIndex) {
                        PX_StringCat(out, "POP R3\n");
                        PX_StringFormat1(&fmrString,
                                         "STRCHR R1,GLOBAL[%1],R3\n",
                                         PX_STRINGFORMAT_INT(operand._oft));
                        PX_StringCat(out, fmrString.buffer);
                    } else {
                        PX_StringFormat2(
                            &fmrString,
                            "STRCHR R1,GLOBAL[%1],%2\n",
                            PX_STRINGFORMAT_INT(operand._oft),
                            PX_STRINGFORMAT_INT(operand.bAtomPopIndex));
                        PX_StringCat(out, fmrString.buffer);
                    }
            ...
            switch (operand.region) {
                case PX_SCRIPT_VARIABLE_REGION_LOCAL: {
                    ...
                    if (operand.bAtomPopIndex) {
                        PX_StringCat(out, "POP R3\n");
                        PX_StringFormat1(&fmrString,
                                         "MEMBYTE R1,LOCAL[%1],R3\n",
                                         PX_STRINGFORMAT_INT(operand._oft));
                        PX_StringCat(out, fmrString.buffer);
                    } else {
                        PX_StringFormat2(
                            &fmrString,
                            "MEMBYTE R1,LOCAL[%1],%2\n",
                            PX_STRINGFORMAT_INT(operand._oft),
                            PX_STRINGFORMAT_INT(operand.bAtomPopIndex));
                        PX_StringCat(out, fmrString.buffer);
                    }
                ...
                case PX_SCRIPT_VARIABLE_REGION_GLOBAL: {
                    ...
                    if (operand.bAtomPopIndex) {
                        PX_StringCat(out, "POP R3\n");
                        PX_StringFormat1(&fmrString,
                                         "MEMBYTE R1,GLOBAL[%1],R3\n",
                                         PX_STRINGFORMAT_INT(operand._oft));
                        PX_StringCat(out, fmrString.buffer);
                    } else {
                        PX_StringFormat2(
                            &fmrString,
                            "MEMBYTE R1,GLOBAL[%1],%2\n",
                            PX_STRINGFORMAT_INT(operand._oft),
                            PX_STRINGFORMAT_INT(operand.bAtomPopIndex));
                        PX_StringCat(out, fmrString.buffer);
                    }
...
static px_bool PX_ScriptParseLastInstr_DOT(PX_ScriptInterpreter *analysis,
                                           px_vector *op,
                                           px_vector *tk,
                                           px_string *out) {
    ...
    operand1 = *((PX_SCRIPT_AST_OPERAND *) PX_VECTORAT(
        PX_SCRIPT_AST_OPERAND, tk, tk->size - 2));
    operand2 = *((PX_SCRIPT_AST_OPERAND *) PX_VECTORAT(
        PX_SCRIPT_AST_OPERAND, tk, tk->size - 1));
    ...
    pTop = (PX_SCRIPT_AST_OPERAND *) PX_VECTORAT(
        PX_SCRIPT_AST_OPERAND, tk, tk->size - 1);
    *pTop = operand2;
    return PX_TRUE;
}
static px_bool PX_ScriptParseLastInstr_EQUAL(PX_ScriptInterpreter *analysis,
                                             px_vector *op,
                                             px_vector *tk,
                                             px_string *out) {
    ...
    operandLeft = *((PX_SCRIPT_AST_OPERAND *) PX_VECTORAT(
        PX_SCRIPT_AST_OPERAND, tk, tk->size - 2));
    operandRight = *((PX_SCRIPT_AST_OPERAND *) PX_VECTORAT(
        PX_SCRIPT_AST_OPERAND, tk, tk->size - 1));
    ...
    switch (operandRight.operandType) {
        ...
        case PX_SCRIPT_AST_OPERAND_TYPE_INT: {
            ...
            switch (operandLeft.operandType) {
                case PX_SCRIPT_AST_OPERAND_TYPE_FLOAT:
                case PX_SCRIPT_AST_OPERAND_TYPE_INT: {
                    if (operandLeft.operandType
                        == PX_SCRIPT_AST_OPERAND_TYPE_INT) {
                        pTop->operandType
                            = PX_SCRIPT_AST_OPERAND_TYPE_INT_CONST;
                        pTop->region = PX_SCRIPT_VARIABLE_REGION_POP;
                        pTop->_oft = 0;
                    } else {
                        pTop->operandType
                            = PX_SCRIPT_AST_OPERAND_TYPE_FLOAT_CONST;
                        pTop->region = PX_SCRIPT_VARIABLE_REGION_POP;
                        pTop->_oft = 0;
                    ...
                    switch (operandLeft.region) {
                        ...
                        case PX_SCRIPT_VARIABLE_REGION_POP: {
                            ...
                            if (operandLeft.bAtomPopIndex) {
                                PX_StringCat(out, "POP R1\n");
                                PX_StringCat(out, "POP R3\n");
                                PX_StringSet(&fmrString,
                                             "STRSET GLOBAL[R1],R3,R2\n");
                                PX_StringCat(out, fmrString.buffer);
                                PX_StringSet(&fmrString, "PUSH GLOBAL[R1]\n");
                                PX_StringCat(out, fmrString.buffer);
                            } else {
        ...
        case PX_SCRIPT_AST_OPERAND_TYPE_FLOAT: {
            ...
            switch (operandLeft.operandType) {
                ...
                case PX_SCRIPT_AST_OPERAND_TYPE_INT: {
                    if (operandLeft.operandType
                        == PX_SCRIPT_AST_OPERAND_TYPE_FLOAT) {
                        pTop->operandType
                            = PX_SCRIPT_AST_OPERAND_TYPE_FLOAT_CONST;
                        pTop->region = PX_SCRIPT_VARIABLE_REGION_POP;
                        pTop->_oft = 0;
        ...
        case PX_SCRIPT_AST_OPERAND_TYPE_STRUCT_PTR: // struct xx var[x]
        {
            ...
            } else if (operandLeft.operandType
                       == PX_SCRIPT_AST_OPERAND_TYPE_MEMORY) {
        ...
        case PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_PTR_CONST:
        case PX_SCRIPT_AST_OPERAND_TYPE_STRUCT_PTR_CONST: {
            if (operandRight.operandType
                    == PX_SCRIPT_AST_OPERAND_TYPE_INT_PTR_CONST
                && operandLeft.operandType
                       != PX_SCRIPT_AST_OPERAND_TYPE_INT_PTR) {
                return PX_FALSE;
            }
            if (operandRight.operandType
        ...
        case PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_CONST: {
            ...
            //////////////////////////////////////////////////////////////////////////
            if (operandRight.operandType
                    == PX_SCRIPT_AST_OPERAND_TYPE_STRING_CONST
                && operandLeft.operandType
...
static px_bool PX_ScriptParseLastInstr_IDX(PX_ScriptInterpreter *analysis,
                                           px_vector *op,
                                           px_vector *tk,
                                           px_string *out) {
...
static px_bool PX_ScriptParseLastInstr_OFT(PX_ScriptInterpreter *analysis,
                                           px_vector *op,
                                           px_vector *tk,
                                           px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_PTR(PX_ScriptInterpreter *analysis,
                                           px_vector *op,
                                           px_vector *tk,
                                           px_string *out) {
...
static px_bool PX_ScriptParseLastInstr_ADR(PX_ScriptInterpreter *analysis,
                                           px_vector *op,
                                           px_vector *tk,
                                           px_string *out) {
...
static px_bool PX_ScriptParseLastInstr_ADD(PX_ScriptInterpreter *analysis,
                                           px_vector *op,
                                           px_vector *tk,
                                           px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
    ...
    switch (operand1.operandType) {
        ...
        case PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_IDX: {
            switch (operand2.operandType) {
                ...
                case PX_SCRIPT_AST_OPERAND_TYPE_STRING_IDX:
                case PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_IDX: {
                    if (!PX_ScriptParseAST_MapTokenToR2(
                            analysis, operand2, out))
                        return PX_FALSE;
                    if (!PX_ScriptParseAST_MapTokenToR1(
                            analysis, operand1, out))
                        return PX_FALSE;
...
static px_bool PX_ScriptParseLastInstr_INC(PX_ScriptInterpreter *analysis,
                                           px_vector *op,
                                           px_vector *tk,
                                           px_string *out) {
...
static px_bool PX_ScriptParseLastInstr_POSITIVE(PX_ScriptInterpreter *analysis,
                                                px_vector *op,
                                                px_vector *tk,
                                                px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1;
...
static px_bool PX_ScriptParseLastInstr_SUB(PX_ScriptInterpreter *analysis,
                                           px_vector *op,
                                           px_vector *tk,
                                           px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_DEC(PX_ScriptInterpreter *analysis,
                                           px_vector *op,
                                           px_vector *tk,
                                           px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1;
...
static px_bool PX_ScriptParseLastInstr_NEGATIVE(PX_ScriptInterpreter *analysis,
                                                px_vector *op,
                                                px_vector *tk,
                                                px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, *pTop;
...
static px_bool PX_ScriptParseLastInstr_MUL(PX_ScriptInterpreter *analysis,
                                           px_vector *op,
                                           px_vector *tk,
                                           px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_DIV(PX_ScriptInterpreter *analysis,
                                           px_vector *op,
                                           px_vector *tk,
                                           px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_AND(PX_ScriptInterpreter *analysis,
                                           px_vector *op,
                                           px_vector *tk,
                                           px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_OR(PX_ScriptInterpreter *analysis,
                                          px_vector *op,
                                          px_vector *tk,
                                          px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_NOT(PX_ScriptInterpreter *analysis,
                                           px_vector *op,
                                           px_vector *tk,
                                           px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, *pTop;
...
static px_bool PX_ScriptParseLastInstr_XOR(PX_ScriptInterpreter *analysis,
                                           px_vector *op,
                                           px_vector *tk,
                                           px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_INV(PX_ScriptInterpreter *analysis,
                                           px_vector *op,
                                           px_vector *tk,
                                           px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, *pTop;
...
static px_bool PX_ScriptParseLastInstr_MOD(PX_ScriptInterpreter *analysis,
                                           px_vector *op,
                                           px_vector *tk,
                                           px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_SHL(PX_ScriptInterpreter *analysis,
                                           px_vector *op,
                                           px_vector *tk,
                                           px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_SHR(PX_ScriptInterpreter *analysis,
                                           px_vector *op,
                                           px_vector *tk,
                                           px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_INT(PX_ScriptInterpreter *analysis,
                                           px_vector *op,
                                           px_vector *tk,
                                           px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, *pTop;
...
static px_bool PX_ScriptParseLastInstr_FLOAT(PX_ScriptInterpreter *analysis,
                                             px_vector *op,
                                             px_vector *tk,
                                             px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, *pTop;
...
static px_bool PX_ScriptParseLastInstr_STRING(PX_ScriptInterpreter *analysis,
                                              px_vector *op,
                                              px_vector *tk,
                                              px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, *pTop;
...
static px_bool PX_ScriptParseLastInstr_MEMORY(PX_ScriptInterpreter *analysis,
                                              px_vector *op,
                                              px_vector *tk,
                                              px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, *pTop;
...
static px_bool PX_ScriptParseLastInstr_STRLEN(PX_ScriptInterpreter *analysis,
                                              px_vector *op,
                                              px_vector *tk,
                                              px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, *pTop;
    ...
    pTop->pStruct = PX_NULL;
    pTop->_int = 0;
    if (!PX_ScriptParseAST_MapTokenToR2(analysis, operand1, out))
        return PX_FALSE;
    pTop->operandType = PX_SCRIPT_AST_OPERAND_TYPE_INT_CONST;
...
static px_bool PX_ScriptParseLastInstr_MEMLEN(PX_ScriptInterpreter *analysis,
                                              px_vector *op,
                                              px_vector *tk,
                                              px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, *pTop;
...
static px_bool PX_ScriptParseLastInstr_LARGE(PX_ScriptInterpreter *analysis,
                                             px_vector *op,
                                             px_vector *tk,
                                             px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_LARGEEQU(PX_ScriptInterpreter *analysis,
                                                px_vector *op,
                                                px_vector *tk,
                                                px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_LESS(PX_ScriptInterpreter *analysis,
                                            px_vector *op,
                                            px_vector *tk,
                                            px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_LAND(PX_ScriptInterpreter *analysis,
                                            px_vector *op,
                                            px_vector *tk,
                                            px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_LOR(PX_ScriptInterpreter *analysis,
                                           px_vector *op,
                                           px_vector *tk,
                                           px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_LESSEQU(PX_ScriptInterpreter *analysis,
                                               px_vector *op,
                                               px_vector *tk,
                                               px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr_LGEQU(PX_ScriptInterpreter *analysis,
                                             px_vector *op,
                                             px_vector *tk,
                                             px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
    ...
    if (!PX_ScriptParseIsOperandNumericType(operand1)
        ...
        switch (operand1.operandType) {
            ...
            case PX_SCRIPT_AST_OPERAND_TYPE_STRUCT:
                PX_ScriptTranslatorError(analysis,
                                         "Struct-type could not be compared.");
                return PX_FALSE;
            default:
                if (operand1.operandType != operand2.operandType) {
                    PX_ScriptTranslatorError(analysis,
...
static px_bool PX_ScriptParseLastInstr_UNEQU(PX_ScriptInterpreter *analysis,
                                             px_vector *op,
                                             px_vector *tk,
                                             px_string *out) {
    PX_SCRIPT_AST_OPERAND operand1, operand2, *pTop;
...
static px_bool PX_ScriptParseLastInstr(PX_ScriptInterpreter *analysis,
                                       px_vector *op,
                                       px_vector *tk,
                                       px_string *out) {

    PX_SCRIPT_AST_OPCODE opcode;
    opcode = *((PX_SCRIPT_AST_OPCODE *) PX_VECTORAT(
        PX_SCRIPT_AST_OPCODE, op, op->size - 1));
...
static px_bool PX_ScriptParseExpressionStream(
    PX_ScriptInterpreter *analysis,
    px_vector *stream,
    px_int *offset,
    PX_SCRIPT_AST_OPERAND *pretOperand,
    PX_SCRIPT_TRANSLATOR_EXPRESSION_TYPE expectedEndl,
    px_string *out) {
    px_vector vOp;
    px_vector vTk;
    px_vector *op = &vOp;
    ...
    px_int i, LBracketBalance, MBracketBalance, paramBeginIndex[16],
        paramcount = 0;
    px_string fmrString;
    ...
    while (*offset < stream->size) {
        ...
        switch (pVec[*offset].type) {
            case PX_SCRIPT_TRANSLATOR_EXPRESSION_TOKEN: {
                if (!PX_ScriptParse_AST_PushToken(analysis,
                                                  op,
                                                  tk,
                                                  pVec[*offset],
                                                  pcurrentSet,
            ...
            case PX_SCRIPT_TRANSLATOR_EXPRESSION_OP_FUNCTION: {
                ...
                for (i = 0; i < analysis->v_functions.size; i++) {
                    pfunc = PX_VECTORAT(
                        PX_SCRIPT_FUNCTION, &analysis->v_functions, i);
                ...
                if (paramcount != pfunc->parametersCount) {
                    PX_ScriptTranslatorError(analysis,
                                             "Function parameter-count error.");
                    goto _ERROR;
                }
                ...
                {
                    if (i == paramcount - 1) {
                        if (!PX_ScriptParseExpressionStream(
                                analysis,
                                stream,
                                &paramBeginIndex[paramcount - 1],
                                &retOperand,
                                PX_SCRIPT_TRANSLATOR_EXPRESSION_OP_LBRACKETEND,
                                out)) {
                            PX_ScriptTranslatorError(
                                analysis, "Function left-bracket-end missing.");
                            goto _ERROR;
                        }
                    } else {
                        if (!PX_ScriptParseExpressionStream(
                                ...
                                out)) {
                            PX_ScriptTranslatorError(analysis,
                                                     "Function comma missing.");
                            goto _ERROR;
                        }
                    }

                    if (retOperand.operandType
                            == PX_SCRIPT_AST_OPERAND_TYPE_STRUCT
                        || retOperand.operandType
                               == PX_SCRIPT_AST_OPERAND_TYPE_STRUCT_PTR
                        || retOperand.operandType
                               == PX_SCRIPT_AST_OPERAND_TYPE_STRUCT_PTR_CONST) {
                    ...
                    switch (retOperand.operandType) {
                        ...
                        case PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_IDX:
                            ...
                            // 					{
                            // 						PX_ScriptTranslatorError(analysis,"Parameter
                            // not matched."); 						goto _ERROR;
                            // 					}
                            // 					break;
                        case PX_SCRIPT_AST_OPERAND_TYPE_FLOAT:
                        case PX_SCRIPT_AST_OPERAND_TYPE_FLOAT_CONST:
                            if (pfunc->parameters[i].type
                                    != PX_SCRIPT_PARSER_VAR_TYPE_FLOAT
                                && pfunc->parameters[i].type
                                       != PX_SCRIPT_PARSER_VAR_TYPE_INT) {
                                PX_ScriptTranslatorError(
                                    analysis, "Parameter not matched.");
                                goto _ERROR;
                            }
                            break;
                        case PX_SCRIPT_AST_OPERAND_TYPE_STRING:
                        ...
                        case PX_SCRIPT_AST_OPERAND_TYPE_STRUCT_PTR_CONST:
                            if (pfunc->parameters[i].type
                                ...
                                || PX_ScriptParseGetStructByIndex(
                                       analysis, pfunc->parameters[i].setIndex)
                                       != retOperand.pStruct) {
                                PX_ScriptTranslatorError(
                                    analysis, "Parameter not matched.");
                                goto _ERROR;
                ...
                switch (pfunc->retType) {
                    ...
                    case PX_SCRIPT_PARSER_VAR_TYPE_MEMORY:
                        retOperand.operandType
                            = PX_SCRIPT_AST_OPERAND_TYPE_MEMORY_CONST;
                        break;
                    case PX_SCRIPT_PARSER_VAR_TYPE_INT_PTR:
                        retOperand.operandType
                            = PX_SCRIPT_AST_OPERAND_TYPE_INT_PTR_CONST;
                    ...
                    case PX_SCRIPT_PARSER_VAR_TYPE_STRUCT_PTR:
                        retOperand.operandType
                            = PX_SCRIPT_AST_OPERAND_TYPE_STRUCT_PTR_CONST;
                        retOperand.pStruct = PX_ScriptParseGetStructByIndex(
                            analysis, pfunc->retSetIndex);
                        break;
            ...
            case PX_SCRIPT_TRANSLATOR_EXPRESSION_OP_MBRACKETBEGIN: {
                ...
                if (level > pVec[*offset].oplevel) {
                    ...
                    if (!PX_ScriptParseAST_PushOpcode(
                            analysis, op, pVec[*offset]))
                        goto _ERROR;
                } else {
                    level = pVec[*offset].oplevel;
                    do {
...
#define PX_SCRIPT_EXPRESSION_ACCEPT_BRACKET_START                              \
    (PX_SCRIPT_EXPRESSION_ACCEPT_LBRACKET_START                                \
     | PX_SCRIPT_EXPRESSION_ACCEPT_MBRACKET_START)
#define PX_SCRIPT_EXPRESSION_ACCEPT_BRACKET_END                                \
    (PX_SCRIPT_EXPRESSION_ACCEPT_LBRACKET_END                                  \
     | PX_SCRIPT_EXPRESSION_ACCEPT_MBRACKET_END)
static px_bool PX_ScriptParseExpression(PX_ScriptInterpreter *analysis,
                                        px_char *expr,
                                        px_string *out,
                                        PX_SCRIPT_AST_OPERAND *retOperand) {
    px_int i;
    ...
    PX_VectorInitialize(
        analysis->mp, &stream, sizeof(PX_SCRIPT_EXPR_STREAM), 32);
    PX_VectorInitialize(
        analysis->mp, &opStack, sizeof(PX_SCRIPT_AST_OPCODE), 32);
    PX_VectorInitialize(
        analysis->mp, &tkStack, sizeof(PX_SCRIPT_AST_OPERAND), 32);

    //
    accept_type = PX_SCRIPT_EXPRESSION_ACCEPT_SINGLE
                  | PX_SCRIPT_EXPRESSION_ACCEPT_TOKEN
        ...
        if (type == PX_LEXER_LEXEME_TYPE_TOKEN) {
            ...
            if (PX_strequ(lexer.CurLexeme.buffer, "STRLEN")) {
                ...
                if (type == PX_LEXER_LEXEME_TYPE_DELIMITER
                    ...
                    Op.oplevel = 0;
                    accept_type = PX_SCRIPT_EXPRESSION_ACCEPT_BINARY
                                  | PX_SCRIPT_EXPRESSION_ACCEPT_MBRACKET_START
                ...
                PX_StringCat(&Op.code, lexer.CurLexeme.buffer);
                PX_VectorPushback(&stream, &Op);
            } else if (lexer.CurLexeme.buffer[0] == '0'
                       && lexer.CurLexeme.buffer[1] == 'X') {
                for (i = 2; lexer.CurLexeme.buffer[i]; i++) {
                    if (lexer.CurLexeme.buffer[i] >= '0'
                        && lexer.CurLexeme.buffer[i] <= 'F') {
                        continue;
                    } else {
                        ret = PX_FALSE;
                        PX_ScriptTranslatorError(analysis,
            ...
            switch (lexer.Symbol) {
                ...
                case '-': {
                    ...
                    if (type == PX_LEXER_LEXEME_TYPE_DELIMITER
                        ...
                        if (accept_type & PX_SCRIPT_EXPRESSION_ACCEPT_BINARY) {
                            ...
                            Op.opclass = PX_SCRIPT_TRANSLATOR_OP_CLASS_BINARY;
                            Op.oplevel
                                = PX_ScriptParseGetOpLevel("->", PX_TRUE);
                            PX_StringInitialize(analysis->mp, &Op.code);
                            PX_StringCat(&Op.code, "->");
                            PX_VectorPushback(&stream, &Op);
                ...
                case ']': {
                    if ((accept_type & PX_SCRIPT_EXPRESSION_ACCEPT_MBRACKET_END)
                        == 0) {
                        ret = PX_FALSE;
                        PX_ScriptTranslatorError(analysis,
                                                 "Unexpected opcode.");
                ...
                case ')': {
                    if ((accept_type & PX_SCRIPT_EXPRESSION_ACCEPT_LBRACKET_END)
                        == 0) {
                        ret = PX_FALSE;
                        PX_ScriptTranslatorError(analysis,
                                                 "Unexpected opcode.");
                ...
                case '.': {
                    ...
                    Op.opclass = PX_SCRIPT_TRANSLATOR_OP_CLASS_SINGLE;
                    Op.oplevel = PX_ScriptParseGetOpLevel(
                        lexer.CurLexeme.buffer, PX_FALSE);
                    PX_StringInitialize(analysis->mp, &Op.code);
                    PX_StringCat(&Op.code, ".");
                    PX_VectorPushback(&stream, &Op);
                    accept_type = PX_SCRIPT_EXPRESSION_ACCEPT_TOKEN;
                ...
                case '!': {
                    ...
                    if (type != PX_LEXER_LEXEME_TYPE_DELIMITER) {
                        ...
                        if ((accept_type & PX_SCRIPT_EXPRESSION_ACCEPT_SINGLE)
                            == 0) {
                            ret = PX_FALSE;
                            PX_ScriptTranslatorError(analysis,
                                                     "Unexpected opcode.");
                ...
                case '%': {
                    ...
                    Op.opclass = PX_SCRIPT_TRANSLATOR_OP_CLASS_SINGLE;
                    Op.oplevel = PX_ScriptParseGetOpLevel(
                        lexer.CurLexeme.buffer, PX_TRUE);
                    PX_StringInitialize(analysis->mp, &Op.code);
                    PX_StringCat(&Op.code, "%");
                    PX_VectorPushback(&stream, &Op);
                    accept_type = PX_SCRIPT_EXPRESSION_ACCEPT_LBRACKET_START
                ...
                case '<': {
                    ...
                    if (PX_LexerGetNextLexeme(&lexer)
                        ...
                        accept_type = PX_SCRIPT_EXPRESSION_ACCEPT_LBRACKET_START
                                      | PX_SCRIPT_EXPRESSION_ACCEPT_TOKEN
                                      | PX_SCRIPT_EXPRESSION_ACCEPT_SINGLE;
                    } else if (lexer.Symbol == '-' || lexer.Symbol == '+'
...
static px_bool PX_ScriptParseVar(PX_ScriptInterpreter *analysis) {
    ...
    if (PX_strequ(analysis->lexer.CurLexeme.buffer,
                  PX_SCRIPT_TRANSLATOR_KEYWORD_VAR_INT)) {
        resType = PX_SCRIPT_PARSER_VAR_TYPE_INT;
    } else if (PX_strequ(analysis->lexer.CurLexeme.buffer,
                         PX_SCRIPT_TRANSLATOR_KEYWORD_VAR_FLOAT)) {
        resType = PX_SCRIPT_PARSER_VAR_TYPE_FLOAT;
    } else if (PX_strequ(analysis->lexer.CurLexeme.buffer,
                         PX_SCRIPT_TRANSLATOR_KEYWORD_VAR_STRING)) {
        resType = PX_SCRIPT_PARSER_VAR_TYPE_STRING;
    } else if (PX_strequ(analysis->lexer.CurLexeme.buffer,
                         PX_SCRIPT_TRANSLATOR_KEYWORD_VAR_MEMORY)) {
    ...
    while (PX_TRUE) {
        ...
        if (analysis->lexer.Symbol == '[') {
            ...
            if (PX_ScriptTranslatorNextToken(&analysis->lexer)
                != PX_LEXER_LEXEME_TYPE_TOKEN) {
                PX_ScriptTranslatorError(analysis, "Invalid var token");
                PX_StringFree(&variable.Mnemonic);
                goto _ERROR;
            ...
            while (PX_TRUE) {
                ...
                PX_StringCat(&variable.GlobalInitializeValue,
                             analysis->lexer.CurLexeme.buffer);
            }
        ...
        if (analysis->functionInside) {
            ...
            for (i = 0; i < analysis->v_variablesStackTable.size; i++) {
                if (PX_strequ(variable.Mnemonic.buffer,
                              PX_VECTORAT(PX_SCRIPT_VARIABLES,
            ...
            pvar = PX_VECTORAT(PX_SCRIPT_VARIABLES,
                               &analysis->v_variablesStackTable,
            ...
            if (variable.bInitialized) {
                ...
                PX_StringFormat2(
                    &code,
                    "%1=%2",
                    PX_STRINGFORMAT_STRING(variable.Mnemonic.buffer),
                    PX_STRINGFORMAT_STRING(
                        variable.GlobalInitializeValue.buffer));
                if (!PX_ScriptParseExpression(
                        analysis, code.buffer, &exprgen, &retOperand)) {
                    PX_StringFree(&code);
                ...
                if (variable.type == PX_SCRIPT_PARSER_VAR_TYPE_STRUCT) {
                    px_string code;
                    PX_SCRIPT_STRUCT *pset = PX_ScriptParseGetStructByIndex(
                        analysis, variable.setIndex);
                    PX_StringInitialize(analysis->mp, &code);
                    if (pset) {
                        px_int j;
                        for (j = 0; j < pset->members.size; j++) {
                            pvar = PX_VECTORAT(
                                PX_SCRIPT_VARIABLES, &pset->members, j);
            ...
            for (i = 0; i < analysis->v_variablesGlobalTable.size; i++) {
                if (PX_strequ(variable.Mnemonic.buffer,
                              PX_VECTORAT(PX_SCRIPT_VARIABLES,
                                          &analysis->v_variablesGlobalTable,
                                          i)
                                  ->Mnemonic.buffer)) {
                    PX_ScriptTranslatorError(analysis, "variable redefined.");
                    PX_StringFree(&variable.Mnemonic);
...
static px_bool PX_ScriptParseStruct(PX_ScriptInterpreter *analysis) {
    ...
    if (!(pset = PX_ScriptParseGetStructInfo(
              analysis, analysis->lexer.CurLexeme.buffer))) {
        PX_ScriptTranslatorError(analysis, "Invalid struct type");
        goto _ERROR;
    }

    variable.setIndex
        = PX_ScriptParseGetSetIndex(analysis, analysis->lexer.CurLexeme.buffer);
    variable.layer = analysis->v_astStructure.size;
    ...
    while (PX_TRUE) {
        ...
        if (analysis->lexer.Symbol == '[') {
            if (variable.type == PX_SCRIPT_PARSER_VAR_TYPE_STRUCT_PTR) {
                PX_ScriptTranslatorError(
                    analysis, "Array of struct pointers is not support.");
                PX_StringFree(&variable.Mnemonic);
                goto _ERROR;
            }
            ...
            if (variable.type == PX_SCRIPT_PARSER_VAR_TYPE_STRUCT_PTR)
                ...
                variable.size
                    = PX_atoi(analysis->lexer.CurLexeme.buffer) * pset->size;
        ...
        if (analysis->functionInside) {
            ...
            pvar = PX_VECTORLAST(PX_SCRIPT_VARIABLES,
                                 &analysis->v_variablesStackTable);
            variable.BeginIndex = pvar->BeginIndex + variable.size;
...
px_bool PX_ScriptParseStructDefine(PX_ScriptInterpreter *analysis) {
    ...
    PX_VectorInitialize(
        analysis->mp, &vSet.members, sizeof(PX_SCRIPT_SETMEMBER), 1);

    if ((PX_ScriptTranslatorNextTokenSN(&analysis->lexer))
        != PX_LEXER_LEXEME_TYPE_DELIMITER) {
        goto _ERROR;
    }
    if (analysis->lexer.Symbol != '{') {
    ...
    while (PX_TRUE) {
        ...
        if ((pSet = PX_ScriptParseGetStructInfo(
                 analysis, analysis->lexer.CurLexeme.buffer))
            != PX_NULL) {
            member.defvar.setIndex = PX_ScriptParseGetSetIndex(
                analysis, analysis->lexer.CurLexeme.buffer);
            ...
            while (PX_TRUE) {
                ...
                PX_StringCat(&member.defvar.Mnemonic,
                             analysis->lexer.CurLexeme.buffer);
                ...
                if (analysis->lexer.Symbol == '[') {
                    if (member.defvar.type
                        == PX_SCRIPT_PARSER_VAR_TYPE_STRUCT_PTR) {
                        PX_ScriptTranslatorError(
                            analysis, "Array of struct is not support.");
                        PX_StringFree(&member.defvar.Mnemonic);
                        goto _ERROR;
                    }
                    ...
                    if (type != PX_LEXER_LEXEME_TYPE_DELIMITER) {
                        PX_ScriptTranslatorError(
                            analysis, "\";\" is expected but not found.");
                        PX_StringFree(&member.defvar.Mnemonic);
                        goto _ERROR;
                    }
            ...
            while (PX_TRUE) {
                ...
                if (analysis->lexer.Symbol == '[') {
                    ...
                    member.offset = vSet.size;
                    member.defvar.size
                        = PX_atoi(analysis->lexer.CurLexeme.buffer);
                    vSet.size += PX_atoi(analysis->lexer.CurLexeme.buffer);
                    ...
                    if (analysis->lexer.Symbol == '=') {
                        PX_ScriptTranslatorError(
                            analysis, "Initialize of array is not support.");
                        PX_StringFree(&member.defvar.Mnemonic);
                        goto _ERROR;
                    }
...
px_bool
PX_ScriptParseFunctionDefined(PX_ScriptInterpreter *analysis,
    ...
    if (PX_strequ(analysis->lexer.CurLexeme.buffer,
        ...
        func.retType = PX_SCRIPT_PARSER_VAR_TYPE_MEMORY;
                         ...
                         PX_SCRIPT_TRANSLATOR_KEYWORD_VAR_VOID)) {
        func.retType = PX_SCRIPT_PARSER_VAR_TYPE_VOID;
    } else if ((func.retSetIndex = PX_ScriptParseGetSetIndex(
                    analysis, analysis->lexer.CurLexeme.buffer))
               != -1) {
    ...
    if (PX_StringLen(&analysis->lexer.CurLexeme)
        > PX_SCRIPT_FUNCTION_NAME_MAX_LEN) {
    ...
    while (PX_TRUE) {
        ...
        } else if (PX_ScriptParseGetStructInfo(
                       analysis, analysis->lexer.CurLexeme.buffer)) {
            fvar.type = PX_SCRIPT_PARSER_VAR_TYPE_STRUCT;

            if ((fvar.setIndex = PX_ScriptParseGetSetIndex(
                     analysis, analysis->lexer.CurLexeme.buffer))
                == -1) {
        ...
        if (fvar.type == PX_SCRIPT_PARSER_VAR_TYPE_STRUCT) {
            PX_ScriptTranslatorError(
                analysis,
                "Could not use a struct for parameter.consider use a struct "
                "pointer.");
            goto _ERROR;
        }
        ...
        if (analysis->v_variablesStackTable.size == 0) {
            ...
            PX_SCRIPT_VARIABLES *plastvar = PX_VECTORLAST(
                PX_SCRIPT_VARIABLES, &analysis->v_variablesStackTable);
        ...
        if (PX_strequ(pfunc->name, func.name)) {
            ...
            for (j = 0; j < func.parametersCount; j++) {
                if (func.parameters[j].type != pfunc->parameters[j].type) {
                    PX_ScriptTranslatorError(
                        analysis, "Function parameters are not compatible");
                    goto _ERROR;
                }
            }
...
px_bool PX_ScriptParseFunctionGuiderCode(PX_ScriptInterpreter *analysis) {
    ...
    if (!PX_StringInitialize(analysis->mp, &guiderCode))
        return PX_FALSE;

    stacksize = analysis->currentAllocStackSize;

    if (analysis->currentFunc.type
    ...
    if (analysis->functionguider != -1) {
        if (!PX_StringInsert(
                &analysis->code, analysis->functionguider, guiderCode.buffer))
            return PX_FALSE;
    } else {
        PX_ASSERT();
...
px_bool PX_ScriptParseLastBlockEnd(PX_ScriptInterpreter *analysis) {
    ...
    astStruct
        = *PX_VECTORLAST(PX_SCRIPT_AST_STRUCTURE, &analysis->v_astStructure);
...
px_bool PX_ScriptParseLastCodeblockEnd(PX_ScriptInterpreter *analysis) {
    ...
    if (!PX_ScriptParsePopAstStructure(analysis))
        return PX_FALSE;
    ...
    switch (astStruct.type) {
        case PX_SCRIPT_AST_STRUCTURE_TYPE_FUNCTION: {
            ...
            // Function Guider
            //////////////////////////////////////////////////////////////////////////
            if (!PX_ScriptParseFunctionGuiderCode(analysis))
                return PX_FALSE;
        ...
        case PX_SCRIPT_AST_STRUCTURE_TYPE_IF: {
            ...
            if (type == PX_LEXER_LEXEME_TYPE_TOKEN
                && PX_strequ(analysis->lexer.CurLexeme.buffer,
                             PX_SCRIPT_TRANSLATOR_KEYWORD_ELSE)) {
                // generate code
                PX_StringInitialize(analysis->mp, &fmrString);
                PX_StringFormat1(&fmrString,
                                 "JMP _ELSE_%1\n",
    ...
    while (analysis->v_variablesStackTable.size) {
        PX_SCRIPT_VARIABLES *pvar = PX_VECTORLAST(
            PX_SCRIPT_VARIABLES, &analysis->v_variablesStackTable);
        if (pvar->layer >= layer) {
            if (pvar->bInitialized)
                PX_StringFree(&pvar->GlobalInitializeValue);
...
px_bool PX_ScriptParseIfLastAST(PX_ScriptInterpreter *analysis) {
    ...
    if (analysis->v_astStructure.size) {
        astStruct = *PX_VECTORLAST(PX_SCRIPT_AST_STRUCTURE,
                                   &analysis->v_astStructure);
        switch (astStruct.type) {
            case PX_SCRIPT_AST_STRUCTURE_TYPE_IF: {
                if (astStruct._if.oneline_expr) {
                    return PX_ScriptParseLastCodeblockEnd(analysis);
            ...
            case PX_SCRIPT_AST_STRUCTURE_TYPE_SWITCH: {
                if (astStruct._compare.oneline_expr) {
                    PX_ScriptTranslatorError(analysis,
                                             "expression in useless compared.");
                    return PX_FALSE;
                }
            } break;
            case PX_SCRIPT_AST_STRUCTURE_TYPE_CASE: {
...
px_bool PX_ScriptParseGetExpression(PX_ScriptInterpreter *analysis,
                                    px_string *expr,
                                    px_char end) {
    ...
    while (PX_TRUE) {
        ...
        if (type == PX_LEXER_LEXEME_TYPE_DELIMITER
            && analysis->lexer.Symbol == end && !mBracket && !lBracket) {
            break;
        }
...
px_bool PX_ScriptCompilerCompile(PX_SCRIPT_LIBRARY *lib,
                                 const px_char *name,
                                 px_string *ASM,
                                 px_int LocalStackSize,
                                 px_char error[],
                                 px_int err_size) {
    PX_SCRIPT_SETMEMBER *psetmem;
    PX_SCRIPT_STRUCT *pset;
    ...
    for (i = 0; i < lib->codeLibraries.size; i++) {
        if (PX_strequ(name,
                      PX_VECTORAT(PX_SCRIPT_CODE, &lib->codeLibraries, i)
                          ->name.buffer)) {
            PX_VECTORAT(PX_SCRIPT_CODE, &lib->codeLibraries, i)->bInclude
                = PX_TRUE;
            PX_StringCopy(
                &codes,
                &PX_VECTORAT(PX_SCRIPT_CODE, &lib->codeLibraries, i)->code);
    ...
    PX_VectorInitialize(lib->mp,
                        &analysis.v_variablesGlobalTable,
                        sizeof(PX_SCRIPT_VARIABLES),
                        1);
    PX_VectorInitialize(lib->mp,
                        &analysis.v_variablesStackTable,
                        sizeof(PX_SCRIPT_VARIABLES),
                        1);
    PX_VectorInitialize(
        lib->mp, &analysis.v_struct, sizeof(PX_SCRIPT_STRUCT), 1);
    PX_VectorInitialize(
        lib->mp, &analysis.v_functions, sizeof(PX_SCRIPT_FUNCTION), 1);
    PX_VectorInitialize(
        lib->mp, &analysis.v_astStructure, sizeof(PX_SCRIPT_AST_STRUCTURE), 1);
    ...
    while (PX_TRUE) {
        state = PX_LexerGetState(&analysis.lexer);
        if ((type = PX_ScriptTranslatorNextToken(&analysis.lexer))
            == PX_LEXER_LEXEME_TYPE_ERR) {
            goto _ERROR;
        }
        if (type == PX_LEXER_LEXEME_TYPE_END) {
            break;
        ...
        //////////////////////////////////////////////////////////////////////////
        if (PX_strequ(analysis.lexer.CurLexeme.buffer,
                      PX_SCRIPT_TRANSLATOR_KEYWORD_RUNTIME)) {
            type = PX_ScriptTranslatorNextToken(&analysis.lexer);
            if (type != PX_LEXER_LEXEME_TYPE_TOKEN) {
                PX_ScriptTranslatorError(&analysis, "unexpected lexeme.");
                goto _ERROR;
            ...
            if (PX_strequ(analysis.lexer.CurLexeme.buffer,
                          PX_SCRIPT_TRANSLATOR_KEYWORD_THREAD)) {
                type = PX_ScriptTranslatorNextToken(&analysis.lexer);
                if (type != PX_LEXER_LEXEME_TYPE_TOKEN) {
                    PX_ScriptTranslatorError(&analysis, "unexpected lexeme.");
                    goto _ERROR;
        ...
        if ((pset = PX_ScriptParseGetStructInfo(
            ...
            if (type == PX_LEXER_LEXEME_TYPE_DELIMITER
                ...
                if (analysis.functionInside) {
                    PX_ScriptTranslatorError(&analysis,
                                             "must be funtion inside");
                    goto _ERROR;
                }

                if (!PX_ScriptParseFunctionDefined(
                        &analysis, PX_SCRIPT_TRANSLATOR_FUNCTION_TYPE_CUSTOM)) {
                    PX_ScriptTranslatorError(&analysis,
                ...
                if (type == PX_LEXER_LEXEME_TYPE_DELIMITER
                    ...
                    PX_strcpy(buildAstStruct._func.name,
                              analysis.currentFunc.name,
                              sizeof(analysis.currentFunc.name));
                    PX_VectorPushback(&analysis.v_astStructure,
                                      &buildAstStruct);
                ...
                if (pset) {
                    ...
                    // 					if (isBeginExpression)
                    // 					{
                    // 						PX_ScriptTranslatorError(&analysis,"Define variable
                    // before expression"); 						goto _ERROR;
                    // 					}
                    if (!PX_ScriptParseVar(&analysis)) {
                        PX_ScriptTranslatorError(&analysis,
                                                 "Invalid variable defined");
                        goto _ERROR;
                    }
                }
                continue;
        ...
        //////////////////////////////////////////////////////////////////////////
        if (PX_strequ(analysis.lexer.CurLexeme.buffer,
                      PX_SCRIPT_TRANSLATOR_KEYWORD_VAR_STRUCT)) {
            if (analysis.functionInside) {
            ...
            if ((type = PX_ScriptTranslatorNextTokenSN(&analysis.lexer))
                    == PX_LEXER_LEXEME_TYPE_DELIMITER
                && analysis.lexer.Symbol == '{') {
                PX_LexerSetState(state);
                {
                    if (!PX_ScriptParseStructDefine(&analysis))
                        goto _ERROR;
                }
            } else {
                PX_ScriptTranslatorError(&analysis, "unexpected lexeme.");
        ...
        if (PX_strequ(analysis.lexer.CurLexeme.buffer,
                      PX_SCRIPT_TRANSLATOR_KEYWORD_HOST)) {
            if (analysis.functionInside) {
                PX_ScriptTranslatorError(&analysis, "must be funtion inside");
                goto _ERROR;
            }
            ...
            if (PX_strequ(analysis.lexer.CurLexeme.buffer,
                ...
                || PX_strequ(analysis.lexer.CurLexeme.buffer,
                             PX_SCRIPT_TRANSLATOR_KEYWORD_VAR_VOID)
                || PX_ScriptParseGetStructInfo(&analysis,
                                               analysis.lexer.CurLexeme.buffer)
                       != PX_NULL) {
                PX_LexerSetState(state);
                if (!PX_ScriptParseFunctionDefined(
                        &analysis, PX_SCRIPT_TRANSLATOR_FUNCTION_TYPE_HOST)) {
                    PX_ScriptTranslatorError(&analysis,
                                             "Invalid function defined");
                    goto _ERROR;
                }
            } else {
                PX_ScriptTranslatorError(&analysis, "unexpected lexeme.");
            ...
            if (type == PX_LEXER_LEXEME_TYPE_DELIMITER
                && analysis.lexer.Symbol == '{') {
                PX_ScriptTranslatorError(&analysis,
                                         "Could not implement host function.");
                goto _ERROR;
            } else if (analysis.lexer.Symbol == ';') {
                continue;
            } else {
        ...
        if (PX_strequ(analysis.lexer.CurLexeme.buffer,
                      PX_SCRIPT_TRANSLATOR_KEYWORD_EXPORT)) {
            state = PX_LexerGetState(&analysis.lexer);
            ...
            if (PX_strequ(analysis.lexer.CurLexeme.buffer,
                ...
                PX_LexerSetState(state);
                if (!PX_ScriptParseFunctionDefined(
                        &analysis, PX_SCRIPT_TRANSLATOR_FUNCTION_TYPE_EXPORT)) {
                    PX_ScriptTranslatorError(&analysis,
                                             "Invalid function defined");
                    goto _ERROR;
                }
        ...
        //////////////////////////////////////////////////////////////////////////
        if (PX_strequ(analysis.lexer.CurLexeme.buffer,
                      PX_SCRIPT_TRANSLATOR_KEYWORD_IF)) {
            if (!analysis.functionInside) {
                PX_ScriptTranslatorError(&analysis, "must be funtion inside");
                goto _ERROR;
            }
            ...
            if (!PX_ScriptParseExpression(
                    &analysis, expression.buffer, &expCode, &retOperand)) {
                PX_StringFree(&expression);
                PX_StringFree(&expCode);
                PX_ScriptTranslatorError(&analysis, "unexpected lexeme.");
                goto _ERROR;
        ...
        if (PX_strequ(analysis.lexer.CurLexeme.buffer,
                      PX_SCRIPT_TRANSLATOR_KEYWORD_WHILE)) {
            if (!analysis.functionInside) {
                PX_ScriptTranslatorError(&analysis, "must be funtion inside");
                goto _ERROR;
            }
        ...
        if (PX_strequ(analysis.lexer.CurLexeme.buffer,
                      PX_SCRIPT_TRANSLATOR_KEYWORD_ASM)) {
            if (!analysis.functionInside) {
                PX_ScriptTranslatorError(&analysis, "must be funtion inside");
                goto _ERROR;
            }
            ...
            while (PX_TRUE) {
                ...
                for (i = 0; i < analysis.v_variablesStackTable.size; i++) {
                    pvar = PX_VECTORAT(PX_SCRIPT_VARIABLES,
                                       &analysis.v_variablesStackTable,
                                       i);
                    if (pvar->type == PX_SCRIPT_PARSER_VAR_TYPE_INT
                        || pvar->type == PX_SCRIPT_PARSER_VAR_TYPE_FLOAT
                        || pvar->type == PX_SCRIPT_PARSER_VAR_TYPE_STRING
                        || pvar->type == PX_SCRIPT_PARSER_VAR_TYPE_MEMORY)
                        if (PX_strequ(pvar->Mnemonic.buffer,
                                      analysis.lexer.CurLexeme.buffer)) {
                            PX_StringFormat1(
                                &fmrString,
                                "LOCAL[%1]",
        ...
        if (PX_strequ(analysis.lexer.CurLexeme.buffer,
                      PX_SCRIPT_TRANSLATOR_KEYWORD_FOR)) {
            if (!analysis.functionInside) {
                PX_ScriptTranslatorError(&analysis, "must be funtion inside");
                goto _ERROR;
            }
        ...
        //////////////////////////////////////////////////////////////////////////
        if (PX_strequ(analysis.lexer.CurLexeme.buffer,
                      PX_SCRIPT_TRANSLATOR_KEYWORD_SWITCH)) {
            if (!analysis.functionInside) {
                PX_ScriptTranslatorError(&analysis, "must be funtion inside");
                goto _ERROR;
            }
        ...
        //////////////////////////////////////////////////////////////////////////
        if (PX_strequ(analysis.lexer.CurLexeme.buffer,
                      PX_SCRIPT_TRANSLATOR_KEYWORD_CASE)) {
            if (!analysis.functionInside) {
                PX_ScriptTranslatorError(&analysis, "must be funtion inside");
                goto _ERROR;
            }
            ...
            for (i = analysis.v_astStructure.size - 1; i >= 0; i--) {
                astStruct = *PX_VECTORAT(
                    PX_SCRIPT_AST_STRUCTURE, &analysis.v_astStructure, i);
            ...
            while (PX_TRUE) {
                ...
                while (PX_TRUE) {
                    ...
                    if (type == PX_LEXER_LEXEME_TYPE_DELIMITER
                        && analysis.lexer.Symbol == ';' && !mBracket
                        && !lBracket) {
                        break;
                    }
                ...
                if (!PX_ScriptParseIsCompareAbleOperand(
                        retOperand, astStruct._compare.OriginOperand)) {
                    PX_StringFree(&expression);
                    PX_StringFree(&expCode);
                    PX_ScriptTranslatorError(&analysis,
        ...
        if (PX_strequ(analysis.lexer.CurLexeme.buffer,
                      PX_SCRIPT_TRANSLATOR_KEYWORD_BREAK)) {
            type = PX_ScriptTranslatorNextToken(&analysis.lexer);
        ...
        _BREAKOUT:
            if (i < 0) {
                PX_ScriptTranslatorError(&analysis,
                                         "Could not matched  break to block.");
                goto _ERROR;
            }
        ...
        if (PX_strequ(analysis.lexer.CurLexeme.buffer,
                      PX_SCRIPT_TRANSLATOR_KEYWORD_CONTINUE)) {
            type = PX_ScriptTranslatorNextToken(&analysis.lexer);
        ...
        //////////////////////////////////////////////////////////////////////////
        if (PX_strequ(analysis.lexer.CurLexeme.buffer,
                      PX_SCRIPT_TRANSLATOR_KEYWORD_RETURN)) {
            if (!analysis.functionInside) {
                PX_ScriptTranslatorError(&analysis, "must be funtion inside");
                goto _ERROR;
            }
            ...
            switch (analysis.currentFunc.retType) {
                ...
                case PX_SCRIPT_PARSER_VAR_TYPE_STRUCT_PTR:
                    ...
                    if (retOperand.pStruct
                        != PX_ScriptParseGetStructByIndex(
                            &analysis, analysis.currentFunc.retSetIndex)) {
                        PX_ScriptTranslatorError(&analysis,
                                                 "Return-type not matched.");
                        goto _ERROR;
                    }
    ...
    globalSize = 0;
    for (i = 0; i < analysis.v_variablesGlobalTable.size; i++) {
        globalSize += PX_VECTORAT(PX_SCRIPT_VARIABLES,
                                  &analysis.v_variablesGlobalTable,
                                  i)
                          ->size;
    }
    ...
    PX_StringInitialize(analysis.mp, &fmrString);
    PX_StringFormat3(&fmrString,
                     ".GLOBAL %1\n.STACK %2\n.THREAD %3\n",
                     PX_STRINGFORMAT_INT(globalSize),
                     PX_STRINGFORMAT_INT(LocalStackSize),
                     PX_STRINGFORMAT_INT(thread));
    ...
    for (i = 0; i < analysis.v_variablesGlobalTable.size; i++) {
        PX_StringFree(&PX_VECTORAT(PX_SCRIPT_VARIABLES,
                                   &analysis.v_variablesGlobalTable,
                                   i)
                           ->Mnemonic);
        if (PX_VECTORAT(
            ...
            PX_StringFree(&PX_VECTORAT(PX_SCRIPT_VARIABLES,
                                       &analysis.v_variablesGlobalTable,
                                       i)
                               ->GlobalInitializeValue);
        }
    }

    for (i = 0; i < analysis.v_variablesStackTable.size; i++) {
        PX_SCRIPT_VARIABLES *pvar = PX_VECTORAT(
            PX_SCRIPT_VARIABLES, &analysis.v_variablesStackTable, i);
        PX_StringFree(&pvar->Mnemonic);
        if (pvar->bInitialized) {
            PX_StringFree(&pvar->GlobalInitializeValue);
...
_ERROR:
    ...
    for (i = 0; i < analysis.v_variablesStackTable.size; i++) {
        PX_StringFree(&PX_VECTORAT(PX_SCRIPT_VARIABLES,
                                   ...
                                   i)
                           ->Mnemonic);
        if (PX_VECTORAT(PX_SCRIPT_VARIABLES, &analysis.v_variablesStackTable, i)
                ->bInitialized) {
            PX_StringFree(&PX_VECTORAT(PX_SCRIPT_VARIABLES,
                                       &analysis.v_variablesStackTable,
                                       i)
...
px_bool PX_ScriptInterpreterExpression(PX_ScriptInterpreter *analysis,
                                       px_char *expr,
                                       px_string *out,
                                       PX_SCRIPT_AST_OPERAND *retOperand) {

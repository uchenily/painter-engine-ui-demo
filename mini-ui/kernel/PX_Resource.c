...
px_bool PX_ResourceLibraryInitialize(px_memorypool *mp,
                                     PX_ResourceLibrary *lib) {
    lib->mp = mp;
...
px_bool PX_ResourceLibraryLoad(PX_ResourceLibrary *lib,
    ...
    switch (type) {
        ...
        case PX_RESOURCE_TYPE_TEXTURE:
            if (!PX_TextureCreateFromMemory(
                    lib->mp, data, datasize, &res.texture))
                return PX_FALSE;
            break;
...
px_bool PX_ResourceLibraryAdd(PX_ResourceLibrary *lib,
                              PX_Resource res,
                              const px_char key[]) {
    if (PX_MapGet(&lib->map, key) == PX_NULL) {
        if (PX_MapPut(
                &lib->map, key, PX_ListPush(&lib->resources, &res, sizeof(res)))
...
PX_Resource *PX_ResourceLibraryGet(PX_ResourceLibrary *lib,
                                   const px_char key[]) {
    return (PX_Resource *) PX_MapGet(&lib->map, key);
}

px_bool PX_ResourceLibraryAddTexture(PX_ResourceLibrary *lib,
                                     const px_char key[],
                                     px_texture *pTexture) {
    PX_Resource res;
    res.Type = PX_RESOURCE_TYPE_TEXTURE;
    if (!PX_TextureCopy(lib->mp, pTexture, &res.texture))
        return PX_FALSE;
    PX_MapPut(&lib->map, key, PX_ListPush(&lib->resources, &res, sizeof(res)));
    return PX_TRUE;
}
...
px_texture *PX_ResourceLibraryGetTexture(PX_ResourceLibrary *lib,
                                         const px_char key[]) {
...
px_string *PX_ResourceLibraryGetString(PX_ResourceLibrary *lib,
                                       const px_char key[]) {
    PX_Resource *pres = PX_ResourceLibraryGet(lib, key);

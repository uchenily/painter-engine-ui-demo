...
static px_void PX_VM_Error(PX_VM *Ins, px_char *log) { PX_LOG(log); }

static px_void PX_VM_LOG(px_char *log) { PX_LOG(log); }

#define PX_SCRIPT_VM_LOG

px_bool PX_VMInitialize(PX_VM *Ins,
                        px_memorypool *mp,
                        const px_byte *code,
                        px_int size) {
    ...
    Ins->VM_memsize
        = header->globalsize + header->stacksize * header->threadcount;
    ...
    if ((Ins->_mem = (PX_VM_VARIABLE *) MP_Malloc(
             mp, sizeof(PX_VM_VARIABLE) * Ins->VM_memsize))
        == PX_NULL) {
        goto _ERROR;
    }

    if ((Ins->pThread = (PX_VM_Thread *) MP_Malloc(
             mp, sizeof(PX_VM_Thread) * Ins->maxThreadCount))
        == PX_NULL) {
        goto _ERROR;
    }
    ...
    if (Ins->funcCount != 0) {
        if ((Ins->_func = (PX_VM_EXPORT_FUNCTION *) MP_Malloc(
                 mp, sizeof(PX_VM_EXPORT_FUNCTION) * Ins->funcCount))
            == PX_NULL) {
            goto _ERROR;
        }
    ...
    if (Ins->hostCount != 0) {
        if ((Ins->_host = (PX_ASM_HOST_NODE *) MP_Malloc(
                 mp, sizeof(PX_ASM_HOST_NODE) * Ins->hostCount))
            == PX_NULL) {
            goto _ERROR;
        }
...
PX_VM_VARIABLE
PX_VM_VaribaleCopy(PX_VM *Ins, PX_VM_VARIABLE var, px_bool *bOutOfMemory) {
    ...
    *bOutOfMemory = PX_FALSE;
    ...
    if (var.type == PX_VM_VARIABLE_TYPE_STRING) {
        ...
        if (!PX_MemoryCat(
                &cpyVar._memory, var._memory.buffer, var._memory.usedsize)) {
            cpyVar.type = PX_VM_VARIABLE_TYPE_INT;
            *bOutOfMemory = PX_TRUE;
        }
    }
...
static PX_VM_VARIABLE PX_VM_GetParamConst(PX_VM *Ins,
                                          px_char optype,
                                          px_int param,
    ...
    switch (optype) {
        ...
        case PX_SCRIPT_ASM_OPTYPE_LOCAL_CONST: {
            if (pT->BP - param >= 0 && pT->BP - param < Ins->VM_memsize)
                return PX_VM_VaribaleCopy(
                    Ins, Ins->_mem[pT->BP - param], bOutofMemory);
            else
                goto _ERROR;
        } break;
        case PX_SCRIPT_ASM_OPTYPE_LOCAL_REGREF: {
            if (param < PX_VM_REG_COUNT) {
                ...
                if (rVar.type == PX_VM_VARIABLE_TYPE_INT) {
                    if (pT->BP - rVar._int >= 0
                        && pT->BP - rVar._int < Ins->VM_memsize) {
                        return PX_VM_VaribaleCopy(
                            Ins, Ins->_mem[rVar._int - pT->BP], bOutofMemory);
                    } else {
                        goto _ERROR;
                    }
                } else {
            ...
            if (param >= 0 && param < Ins->VM_memsize) {
                ...
                if (rVar.type == PX_VM_VARIABLE_TYPE_INT) {
                    if (pT->BP - rVar._int >= 0
                        && pT->BP - rVar._int < Ins->VM_memsize) {
                        return PX_VM_VaribaleCopy(
                            Ins, Ins->_mem[pT->BP - rVar._int], bOutofMemory);
                    } else {
                        goto _ERROR;
                    }
                } else {
        ...
        case PX_SCRIPT_ASM_OPTYPE_GLOBAL_SPREF: {
            if (pT->SP + param >= 0 && pT->SP + param < Ins->VM_memsize)
                return PX_VM_VaribaleCopy(
                    Ins, Ins->_mem[pT->SP + param], bOutofMemory);
            else
                goto _ERROR;
        } break;
            ...
            if (param < PX_VM_REG_COUNT) {
                ...
                if (rVar.type == PX_VM_VARIABLE_TYPE_INT) {
                    if (rVar._int >= 0 && rVar._int < Ins->VM_memsize) {
                        return PX_VM_VaribaleCopy(
                            Ins, Ins->_mem[rVar._int], bOutofMemory);
                    } else {
                        goto _ERROR;
                    }
                } else {
...
PX_VM_VARIABLE *
PX_VMGetVariablePointer(PX_VM *Ins, px_char optype, px_int param) {
    PX_VM_VARIABLE rVar;
    PX_VM_Thread *pT = &Ins->pThread[Ins->T];
    switch (optype) {
        case PX_SCRIPT_ASM_OPTYPE_REG: {
...
PX_VM_RUNRETURN PX_VMRunThread(PX_VM *Ins, px_int tick) {
    ...
    while (PX_TRUE) {
        ...
        if (opCode != PX_SCRIPT_ASM_INSTR_OPCODE_MOV
            && opCode != PX_SCRIPT_ASM_INSTR_OPCODE_ADD
            && opCode != PX_SCRIPT_ASM_INSTR_OPCODE_SUB
            && opCode != PX_SCRIPT_ASM_INSTR_OPCODE_PUSH
            && opCode != PX_SCRIPT_ASM_INSTR_OPCODE_POP) {
            if (opType[0] == PX_SCRIPT_ASM_OPTYPE_IP
                || opType[0] == PX_SCRIPT_ASM_OPTYPE_SP
        ...
        switch (opCode) {
            case PX_SCRIPT_ASM_INSTR_OPCODE_MOV: {
                cVar = PX_VM_GetParamConst(
                    Ins, opType[1], PX_SCRIPT_VM_PARAM(1), &bOutofMemory);
                if (bOutofMemory) {
                ...
                pVar = PX_VMGetVariablePointer(
                    Ins, opType[0], PX_SCRIPT_VM_PARAM(0));
                if (pVar == PX_NULL) {
                ...
                if (cVar.type == PX_VM_VARIABLE_TYPE_STRING) {
                    ...
                    if (!PX_MemoryAlloc(&pVar->_memory,
                                        cVar._memory.usedsize)) {
                        PX_VM_Error(Ins, "MOV crash.");
                        goto _ERROR;
                    }
                    if (!PX_MemoryCat(&pVar->_memory,
                                      cVar._memory.buffer,
                                      cVar._memory.usedsize)) {
                        PX_VM_Error(Ins, "MOV crash.");
                        goto _ERROR;
            ...
            case PX_SCRIPT_ASM_INSTR_OPCODE_ADD: {
                ...
                if (pVar->type == PX_VM_VARIABLE_TYPE_INT
                    && cVar.type == PX_VM_VARIABLE_TYPE_FLOAT) {
            ...
            case PX_SCRIPT_ASM_INSTR_OPCODE_SUB: {
                ...
                if (pVar->type == PX_VM_VARIABLE_TYPE_INT
                    && cVar.type == PX_VM_VARIABLE_TYPE_INT) {
                    pVar->_int -= cVar._int;
                } else if (pVar->type == PX_VM_VARIABLE_TYPE_INT
                           && cVar.type == PX_VM_VARIABLE_TYPE_FLOAT) {
                    pVar->_float = pVar->_int - cVar._float;
                    pVar->type = PX_VM_VARIABLE_TYPE_FLOAT;
            ...
            case PX_SCRIPT_ASM_INSTR_OPCODE_STRCAT: {
                ...
                if (cVar.type == PX_VM_VARIABLE_TYPE_STRING) {
                    ...
                    if (!PX_StringCatChar(&pVar->_string, (px_char) cVar._int))
                        goto _ERROR;
                } else {
                    goto _ERROR;
                }
            ...
            case PX_SCRIPT_ASM_INSTR_OPCODE_STRREP: {
                ...
                if (bOutofMemory)
                    goto _ERROR;
                sVar = PX_VM_GetParamConst(
                    Ins, opType[2], PX_SCRIPT_VM_PARAM(2), &bOutofMemory);
                if (bOutofMemory)
                    goto _ERROR;
                if (pVar->type != PX_VM_VARIABLE_TYPE_STRING) {
                ...
                PX_StringReplace(
                    &pVar->_string, cVar._string.buffer, sVar._string.buffer);
                pT->IP += (4 + 3 * 4);
            } break;
            ...
            case PX_SCRIPT_ASM_INSTR_OPCODE_STRFRI: {
                ...
                if (!PX_StringCat(&pVar->_string, numOut))
                    goto _ERROR;
                pT->IP += (4 + 2 * 4);
            } break;
            ...
            case PX_SCRIPT_ASM_INSTR_OPCODE_STRFRF: {
                ...
                numOut[0] = 0;
                if (cVar.type == PX_VM_VARIABLE_TYPE_INT)
                    PX_ftoa((px_float) cVar._int,
                            numOut,
                            sizeof(numOut),
                            PX_VM_DEFALUT_FLOAT_PRECCISION);
                else
                    PX_ftoa(cVar._float,
                            numOut,
                            sizeof(numOut),
            ...
            case PX_SCRIPT_ASM_INSTR_OPCODE_STRTMEM: {
                ...
                if (pVar->type == PX_VM_VARIABLE_TYPE_STRING) {
                    ...
                    if (!PX_MemoryCat(&cVar._memory,
                                      pVar->_string.buffer,
                                      pVar->_string.bufferlen))
                        goto _ERROR;
                    PX_StringFree(&pVar->_string);
            ...
            case PX_SCRIPT_ASM_INSTR_OPCODE_STRFIND: {
                ...
                PX_VM_VaribaleFree(Ins, pVar);
                if (sVar.type != PX_VM_VARIABLE_TYPE_STRING
                    || tVar.type != PX_VM_VARIABLE_TYPE_STRING) {
            ...
            case PX_SCRIPT_ASM_INSTR_OPCODE_MEMFIND: {
                ...
                PX_VM_VaribaleFree(Ins, pVar);
                if (sVar.type != PX_VM_VARIABLE_TYPE_MEMORY
                    || tVar.type != PX_VM_VARIABLE_TYPE_MEMORY) {
                    PX_VM_Error(Ins, "memfind parameters error.");
                    goto _ERROR;
                }
                pVar->_int = -1;
                for (i = 0;
                     i < sVar._memory.usedsize - tVar._memory.usedsize + 1;
                     i++) {
                    if (PX_memequ(sVar._memory.buffer + i,
                                  tVar._memory.buffer,
                                  tVar._memory.usedsize)) {
                        pVar->_int = i;
                        pVar->type = PX_VM_VARIABLE_TYPE_INT;
                        break;
                    }
            ...
            case PX_SCRIPT_ASM_INSTR_OPCODE_MEMCAT: {
                ...
                if (bOutofMemory)
                    goto _ERROR;
                if (pVar->type != PX_VM_VARIABLE_TYPE_MEMORY
                    || sVar.type != PX_VM_VARIABLE_TYPE_MEMORY) {
                    if (pVar->type != PX_VM_VARIABLE_TYPE_MEMORY
                        || sVar.type != PX_VM_VARIABLE_TYPE_INT) {
                        PX_VM_Error(Ins, "memcat crash.");
                        goto _ERROR;
                    } else {
                        if (!PX_MemoryCat(&pVar->_memory, &sVar._int, 1))
                            goto _ERROR;
                        pT->IP += (4 + 2 * 4);
                    }
                } else {
            ...
            case PX_SCRIPT_ASM_INSTR_OPCODE_MEMTSTR: {
                ...
                if (pVar->type == PX_VM_VARIABLE_TYPE_MEMORY) {
                    if (pVar->_memory.usedsize == 0) {
                        ...
                        lastchar
                            = pVar->_memory.buffer[pVar->_memory.usedsize - 1];

                        pVar->_memory.buffer[pVar->_memory.usedsize - 1] = 0;

                        PX_StringInitialize(Ins->mp, &cVar._string);

                        if (!PX_StringCat(&cVar._string,
                                          (px_char *) pVar->_memory.buffer)) {
                            PX_VM_Error(Ins, "memstr crash.");
                            goto _ERROR;
                        }
                        if (PX_strlen((px_char *) pVar->_memory.buffer)
                            == pVar->_memory.usedsize - 1)
            ...
            case PX_SCRIPT_ASM_INSTR_OPCODE_DATACPY: {
                cVar = PX_VM_GetParamConst(
                    Ins, opType[0], PX_SCRIPT_VM_PARAM(0), &bOutofMemory);
                if (bOutofMemory)
                    goto _ERROR;
                sVar = PX_VM_GetParamConst(
                    Ins, opType[1], PX_SCRIPT_VM_PARAM(1), &bOutofMemory);
                ...
                if (cVar._int + tVar._int > Ins->VM_memsize || cVar._int < 0
                    || sVar._int + tVar._int > Ins->VM_memsize
                    || sVar._int < 0) {
                    PX_VM_Error(Ins, "Out of memory access");
                    goto _ERROR;
                }
                PX_memcpy(&Ins->_mem[cVar._int],
                          &Ins->_mem[sVar._int],
                          sizeof(PX_VM_VARIABLE) * tVar._int);
            ...
            case PX_SCRIPT_ASM_INSTR_OPCODE_JE: {
                ...
                if (cVar.type == sVar.type) {
                    if (cVar.type != PX_VM_VARIABLE_TYPE_STRING
                        && cVar.type != PX_VM_VARIABLE_TYPE_MEMORY) {
                        ...
                        if (cVar.type == PX_VM_VARIABLE_TYPE_STRING)
                            if (PX_strequ(cVar._string.buffer,
                                          sVar._string.buffer)) {
                                if (tVar._int < 0 || tVar._int > Ins->binsize) {
                                    PX_VM_Error(Ins,
                                                "JE jump to invalid address");
                                    goto _ERROR;
                                }
                                pT->IP = tVar._int;
                                break;
                        ...
                        if (cVar.type == PX_VM_VARIABLE_TYPE_MEMORY)
                            if (PX_memequ(cVar._memory.buffer,
                                          sVar._memory.buffer,
                                          cVar._memory.usedsize)) {
                                if (tVar._int < 0 || tVar._int > Ins->binsize) {
                                    PX_VM_Error(Ins,
            ...
            case PX_SCRIPT_ASM_INSTR_OPCODE_JL: {
                ...
                if (cVar.type == PX_VM_VARIABLE_TYPE_STRING
                    || sVar.type == PX_VM_VARIABLE_TYPE_STRING) {
                    PX_VM_Error(Ins, "JL parameters error");
                    goto _ERROR;
                }
                if (cVar.type == PX_VM_VARIABLE_TYPE_MEMORY
            ...
            case PX_SCRIPT_ASM_INSTR_OPCODE_LGE: {
                ...
                if (sVar.type == tVar.type) {
                    if (sVar.type != PX_VM_VARIABLE_TYPE_STRING
                        ...
                        if (sVar.type == PX_VM_VARIABLE_TYPE_STRING)
                            if (PX_strequ(sVar._string.buffer,
                                          tVar._string.buffer)) {
                                pVar->_int = 1;
                                pT->IP += (4 + 3 * 4);
                                break;
                    ...
                    if (sVar.type == PX_VM_VARIABLE_TYPE_INT
                        && tVar.type == PX_VM_VARIABLE_TYPE_FLOAT) {
            ...
            case PX_SCRIPT_ASM_INSTR_OPCODE_CALL: {
                ...
                if (opType[0] == PX_SCRIPT_ASM_OPTYPE_HOST) {
                    if (Ins->_host[cVar._int].map != PX_NULL) {
                        px_int runtick;
                        runtick
                            = ((PX_VM_Function_Modules) (Ins->_host[cVar._int]
                                                             .map))(
                                Ins, Ins->_host[cVar._int].userptr);
                        if (runtick == 0) {
            ...
            case PX_SCRIPT_ASM_INSTR_OPCODE_ADR: {
                ...
                switch (opType[1]) {
                    ...
                    case PX_SCRIPT_ASM_OPTYPE_LOCAL:
                        ...
                        break;
                    case PX_SCRIPT_ASM_OPTYPE_GLOBAL_REGREF:
                        if (pT->R[PX_SCRIPT_VM_PARAM(1)].type
                        ...
                        break;
                    case PX_SCRIPT_ASM_OPTYPE_GLOBAL_GLOBALREF:
                        if (Ins->_mem[PX_SCRIPT_VM_PARAM(1)].type
                            != PX_VM_VARIABLE_TYPE_INT) {
                            PX_VM_Error(Ins, "ADR error");
                            goto _ERROR;
                        }
                        ...
                        break;
                    case PX_SCRIPT_ASM_OPTYPE_GLOBAL_LOCALREF:
                        if (Ins->_mem[pT->BP - PX_SCRIPT_VM_PARAM(1)].type
                            != PX_VM_VARIABLE_TYPE_INT) {
                            PX_VM_Error(Ins, "ADR error");
                            goto _ERROR;
                        }
                        pVar->_int
                            = Ins->_mem[pT->BP - PX_SCRIPT_VM_PARAM(1)]._int;
                        break;
                    case PX_SCRIPT_ASM_OPTYPE_LOCAL_REGREF:
                        if (Ins->_mem[PX_SCRIPT_VM_PARAM(1)].type
                            != PX_VM_VARIABLE_TYPE_INT) {
                    ...
                    case PX_SCRIPT_ASM_OPTYPE_LOCAL_GLOBALREF:
                        ...
                        pVar->_int
                            = pT->BP - Ins->_mem[PX_SCRIPT_VM_PARAM(1)]._int;
                        break;
                    case PX_SCRIPT_ASM_OPTYPE_LOCAL_LOCALREF:
                        if (Ins->_mem[pT->BP - PX_SCRIPT_VM_PARAM(1)].type
                            != PX_VM_VARIABLE_TYPE_INT) {
...
px_bool PX_VMCallFunctionEx(PX_VM *Ins,
                            px_int threadID,
                            const px_char *functionName,
                            PX_VM_VARIABLE args[],
                            px_int paramcount,
                            px_int returnIP) {
    px_int i, j, ip = -1;
    ...
    if (Ins->pThread[threadID].Activated == PX_FALSE) {
        ...
        Ins->pThread[threadID].SP
            = Ins->VM_memsize - Ins->VM_Stacksize * threadID;
        Ins->pThread[threadID].BP = Ins->pThread[threadID].SP;
        Ins->pThread[threadID].IP = ip;
        for (j = 0; j < sizeof(Ins->pThread[threadID].R)
                            / sizeof(Ins->pThread[threadID].R[0]);
             j++) {
...
px_bool PX_VMRunFunction(PX_VM *Ins,
                         px_int threadID,
                         const px_char *functionName,
                         PX_VM_VARIABLE args[],
                         px_int paramcount) {
    return PX_VMCallFunctionEx(
        Ins, threadID, functionName, args, paramcount, -1);
}

px_bool PX_VMCallFunction(PX_VM *Ins,
                          px_int threadID,
                          const px_char *functionName,
                          PX_VM_VARIABLE args[],
                          px_int paramcount) {
    return PX_VMCallFunctionEx(Ins,
                               threadID,
                               functionName,
                               args,
                               paramcount,
...
px_bool PX_VMRunFunctionIndex(PX_VM *Ins,
                              px_int threadID,
                              px_int funcIndex,
                              PX_VM_VARIABLE args[],
                              px_int paramcount) {
...
px_bool PX_VMBeginThreadFunction(PX_VM *Ins,
                                 px_int threadID,
                                 const px_char *func,
                                 PX_VM_VARIABLE args[],
                                 px_int paramcount) {
...
px_bool PX_VMBeginThreadFunctionIndex(PX_VM *Ins,
                                      px_int threadID,
                                      px_int funcIndex,
                                      PX_VM_VARIABLE args[],
                                      px_int paramcount) {
...
px_void PX_VMSuspend(PX_VM *Ins) { Ins->Suspend = PX_TRUE; }

px_void PX_VMContinue(PX_VM *Ins) { Ins->Suspend = PX_FALSE; }
...
px_bool PX_ScriptVM_InstanceInitialize(PX_VM *Ins,
                                       px_memorypool *mp,
                                       px_byte *code,
                                       px_int size) {
...
px_bool PX_VMRegistHostFunction(PX_VM *Ins,
                                const px_char *name,
                                PX_VM_Function_Modules funcModules,
                                px_void *userptr) {
    px_int i;
    px_char uprname[__PX_SCRIPT_ASM_MNEMONIC_NAME_LEN];
...
px_void PX_VM_RET(PX_VM *Ins, PX_VM_VARIABLE cVar) {
    ...
    if (cVar.type == PX_VM_VARIABLE_TYPE_STRING) {
        ...
        PX_MemoryCat(
            &pVar->_memory, cVar._memory.buffer, cVar._memory.usedsize);
        pVar->type = cVar.type;
    } else {
        *pVar = cVar;
    }
...
PX_VM_VARIABLE PX_VM_Variable_const_memory(const px_byte *buffer,
                                           px_int _size) {
    PX_VM_VARIABLE var;
    var.type = PX_VM_VARIABLE_TYPE_MEMORY;
    var._memory.allocsize = 0;
...
px_bool PX_VMDebuggerMapInitialize(px_memorypool *mp,
                                   PX_VM_DebuggerMap *pDebugMap) {
    pDebugMap->mp = mp;
    if (!PX_StringInitialize(mp, &pDebugMap->source))
        return PX_FALSE;
    if (!PX_VectorInitialize(
            mp, &pDebugMap->map, sizeof(PX_SCRIPT_ASM_SOURCE_MAP), 64))
        return PX_FALSE;
    return PX_TRUE;
}
...
px_void PX_VMThreadStop(PX_VM *Ins, px_int ThreadId) {
    ...
    if (ThreadId < Ins->maxThreadCount) {
        if (Ins->pThread[ThreadId].Activated) {
            popn = Ins->VM_memsize - Ins->VM_Stacksize * ThreadId
                   - Ins->pThread[ThreadId].SP;
            ...
            if (Ins->pThread[i].Activated) {
                ...
                if (tick >= 0) {
                    if (tick < PX_VM_ATOM_INSTRUCTMENTS) {
                        ...
                        if (ret == PX_VM_RUNRETURN_WAIT
                            || ret == PX_VM_RUNRETURN_SUSPEND) {
                            continue;
                        } else {
                        ...
                        PX_VM_RUNRETURN ret
                            = PX_VMRunThread(Ins, PX_VM_ATOM_INSTRUCTMENTS);
                        if (ret == PX_VM_RUNRETURN_WAIT
                            || ret == PX_VM_RUNRETURN_SUSPEND) {
                            continue;
                        } else {
...
px_bool PX_VMDebuggerGetVarParamName(
    PX_VM *Ins, px_char optype, px_int param, px_char content[], px_int size) {
    PX_VM_Thread *pT = Ins->pThread + Ins->T;
    switch (optype) {
        case PX_SCRIPT_ASM_OPTYPE_IP: {
            return PX_FALSE;
        ...
        case PX_SCRIPT_ASM_OPTYPE_LOCAL_REGREF: {
            PX_sprintf1(
                content, size, "LOCAL[R%1]", PX_STRINGFORMAT_INT(param));
        } break;
        case PX_SCRIPT_ASM_OPTYPE_LOCAL_GLOBALREF: {
            PX_sprintf1(
                content, size, "GLOBAL[LOCAL[%1]]", PX_STRINGFORMAT_INT(param));
        } break;
        case PX_SCRIPT_ASM_OPTYPE_LOCAL_LOCALREF: {
            PX_sprintf1(
                content, size, "LOCAL[LOCAL[%1]]", PX_STRINGFORMAT_INT(param));
        } break;
        case PX_SCRIPT_ASM_OPTYPE_GLOBAL_CONST: {
            PX_sprintf1(
                content, size, "GLOBAL[%1]", PX_STRINGFORMAT_INT(param));
        } break;
        case PX_SCRIPT_ASM_OPTYPE_GLOBAL_SPREF: {
            PX_sprintf1(content,
                        size,
                        "GLOBAL[%1]",
                        PX_STRINGFORMAT_INT(pT->SP + param));
        } break;
        case PX_SCRIPT_ASM_OPTYPE_GLOBAL_REGREF: {
            PX_sprintf1(
                content, size, "GLOBAL[R%1]", PX_STRINGFORMAT_INT(param));
        } break;
        case PX_SCRIPT_ASM_OPTYPE_GLOBAL_GLOBALREF: {
            PX_sprintf1(content,
                        size,
                        "GLOBAL[GLOBAL[%1]]",
                        PX_STRINGFORMAT_INT(param));
        } break;
        case PX_SCRIPT_ASM_OPTYPE_GLOBAL_LOCALREF: {
            PX_sprintf1(
                content, size, "GLOBAL[LOCAL[%1]]", PX_STRINGFORMAT_INT(param));
...
px_void PX_VMDebuggerGetParamName(
    PX_VM *Ins, px_char optype, px_int param, px_char content[], px_int size) {
    PX_VM_Thread *pT = Ins->pThread + Ins->T;
    switch (optype) {
        case PX_SCRIPT_ASM_OPTYPE_IP: {
        ...
        case PX_SCRIPT_ASM_OPTYPE_FLOAT: {
            ...
            PX_sprintf1(content,
                        size,
                        "%1.6",
                        PX_STRINGFORMAT_FLOAT(*(px_float *) &param));
        } break;
        case PX_SCRIPT_ASM_OPTYPE_STRING: {
            PX_sprintf1(content,
                        size,
                        "%1",
                        PX_STRINGFORMAT_STRING(Ins->_string + param));
        } break;

        case PX_SCRIPT_ASM_OPTYPE_MEMORY: {
            PX_sprintf1(
                content, size, "memory ptr: %1", PX_STRINGFORMAT_INT(param));
        } break;
...
px_int PX_VMDebuggerInstruction(PX_VM *Ins,
                                px_int IP,
                                px_char *opcode,
                                px_char opcode_name[16],
                                px_int *paramCount,
                                px_char opType[3],
                                px_int param[3]) {
    px_char opCode;
...
px_int PX_VMDebuggerMapIpToLine(PX_VM_DebuggerMap *pDebugMap, px_int ip) {
    ...
    for (i = 0; i < pDebugMap->map.size; i++) {
        PX_SCRIPT_ASM_SOURCE_MAP *pmap
            = PX_VECTORAT(PX_SCRIPT_ASM_SOURCE_MAP, &pDebugMap->map, i);

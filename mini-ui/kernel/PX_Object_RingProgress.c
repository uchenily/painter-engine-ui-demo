...
px_void PX_Object_RingProgressRender(px_surface *psurface,
                                     PX_Object *pObject,
                                     px_uint elapsed) {
    PX_Object_RingProgress *pDesc
        = PX_ObjectGetDesc(PX_Object_RingProgress, pObject);
    px_int angle;
    ...
    do {
        ...
        if (pDesc->target_value - (px_int) pDesc->target_value) {
            PX_sprintf1(content,
                        sizeof(content),
                        "%1.2",
                        PX_STRINGFORMAT_FLOAT(pDesc->current_value));
            ...
            PX_itoa(
                (px_int) pDesc->current_value, content, sizeof(content), 10);
            PX_FontModuleDrawText(psurface,
                                  pDesc->numericFM,
                                  (px_int) objx,
                                  (px_int) objy,
...
PX_Object *PX_Object_RingProgressCreate(px_memorypool *mp,
                                        PX_Object *Parent,
                                        px_float x,
                                        px_float y,
                                        px_float radius,
...
px_void PX_Object_RingProgressSetCurrentValue(PX_Object *pObject,
                                              px_float value) {
    PX_Object_RingProgress *pDesc = PX_Object_GetRingProgress(pObject);
    if (pDesc) {

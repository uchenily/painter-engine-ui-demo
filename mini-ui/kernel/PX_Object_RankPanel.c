...
px_void PX_Object_RankPanelRender(px_surface *psurface,
                                  PX_Object *pObject,
                                  px_uint elapsed) {
    ...
    pDesc->a1_angle += (PX_OBJECT_RANKPANEL_RING1_NORMAL_SPEED
                        + (PX_OBJECT_RANKPANEL_RING1_MAX_SPEED
                           - PX_OBJECT_RANKPANEL_RING1_NORMAL_SPEED)
                              * (pDesc->current_value) / pDesc->max)
                       * elapsed / 1000;
    if (pDesc->a1_angle > 360) {
        pDesc->a1_angle -= (px_int) (pDesc->a1_angle / 360) * 360.f;
    }
    pDesc->a2_angle += (PX_OBJECT_RANKPANEL_RING2_NORMAL_SPEED
                        + (PX_OBJECT_RANKPANEL_RING2_MAX_SPEED
                           - PX_OBJECT_RANKPANEL_RING2_NORMAL_SPEED)
                              * (pDesc->current_value) / pDesc->max)
                       * elapsed / 1000;
    if (pDesc->a2_angle > 360) {
        pDesc->a2_angle -= (px_int) (pDesc->a2_angle / 360) * 360.f;
    }
    ...
    PX_GeoDrawSector(psurface,
                     (px_int) objx,
                     (px_int) objy,
                     pObject->diameter / 2 + 15,
                     pObject->diameter / 2 + 5,
                     aColor,
                     (px_int) pDesc->a1_angle,
                     (px_int) pDesc->a1_angle + 90);
    PX_GeoDrawSector(psurface,
                     (px_int) objx,
                     (px_int) objy,
                     pObject->diameter / 2 + 15,
                     pObject->diameter / 2 + 5,
                     aColor,
                     (px_int) pDesc->a1_angle + 180,
                     (px_int) pDesc->a1_angle + 270);

    PX_GeoDrawSector(psurface,
                     (px_int) objx,
                     (px_int) objy,
                     pObject->diameter / 2 + 27,
                     pObject->diameter / 2 + 39,
                     aColor,
                     (px_int) pDesc->a2_angle,
                     (px_int) pDesc->a2_angle + 45);
    PX_GeoDrawSector(psurface,
                     (px_int) objx,
                     (px_int) objy,
                     pObject->diameter / 2 + 27,
                     pObject->diameter / 2 + 39,
                     aColor,
                     (px_int) pDesc->a2_angle + 90,
                     (px_int) pDesc->a2_angle + 135);
    PX_GeoDrawSector(psurface,
                     (px_int) objx,
                     (px_int) objy,
                     pObject->diameter / 2 + 27,
                     pObject->diameter / 2 + 39,
                     aColor,
                     (px_int) pDesc->a2_angle + 180,
                     (px_int) pDesc->a2_angle + 225);
    PX_GeoDrawSector(psurface,
                     (px_int) objx,
                     (px_int) objy,
                     pObject->diameter / 2 + 27,
                     pObject->diameter / 2 + 39,
                     aColor,
                     (px_int) pDesc->a2_angle + 270,
                     (px_int) pDesc->a2_angle + 315);
    ...
    do {
        ...
        PX_FontModuleDrawText(psurface,
                              ...
                              PX_ALIGN_CENTER,
                              content,
                              PX_ColorInverse(pDesc->rankColor[colorIndex]));
    } while (0);
}

PX_Object *PX_Object_RankPanelCreate(px_memorypool *mp,
                                     PX_Object *Parent,
                                     px_float x,
                                     px_float y,
                                     px_float radius,

...
static PX_Object *PX_ObjectGetObjectLink(PX_Object *pObject,
    ...
    while (payload[oft] != '.' && payload[oft] != '\0'
           && oft < PX_OBJECT_ID_MAXLEN - 1) {
        id[oft] = payload[oft];
        oft++;
    }
...
PX_Object_Event PX_OBJECT_BUILD_EVENT_STRING(px_uint Event,
...
PX_Object_Event PX_Object_Event_CursorOffset(PX_Object_Event e,
                                             px_point offset) {
    switch (e.Event) {
        ...
        case PX_OBJECT_EVENT_CURSORDRAG:
            PX_Object_Event_SetCursorX(
                &e, PX_Object_Event_GetCursorX(e) + offset.x);
            PX_Object_Event_SetCursorY(
                &e, PX_Object_Event_GetCursorY(e) + offset.y);
            return e;
        default:
            return e;
    }
...
px_int PX_Object_Event_GetIndex(PX_Object_Event e) { return e.Param_int[0]; }

px_void PX_Object_Event_SetImpactTargetObject(PX_Object_Event *e,
                                              PX_Object *pObject) {
...
px_int PX_Object_Event_GetInt(PX_Object_Event e) { return e.Param_int[0]; }
...
PX_Object *PX_ObjectCreate(px_memorypool *mp,
                           PX_Object *Parent,
                           px_float x,
                           px_float y,
                           px_float z,
...
PX_Object *PX_ObjectCreateEx(px_memorypool *mp,
                             PX_Object *Parent,
                             px_float x,
                             px_float y,
                             px_float z,
                             ...
                             px_int size) {
    PX_Object *pObject
        = PX_ObjectCreate(mp, Parent, x, y, z, Width, Height, Lenght);
    if (pObject) {
...
static px_void
PX_ObjectRenderEx(px_surface *pSurface, PX_Object *pObject, px_uint elapsed) {
    if (pObject == PX_NULL) {
        return;
    }
...
px_void
PX_ObjectRender(px_surface *pSurface, PX_Object *pObject, px_uint elapsed) {
    if (pObject == PX_NULL) {
        return;
    }
...
px_bool PX_ObjectIsPointInRegion(PX_Object *pObject, px_float x, px_float y) {
    ...
    if (r)
        return PX_isPoint2DInCircle(
            PX_POINT2D(x, y), PX_POINT2D(objx, objy), r);
    else
        return PX_isXYInRegion(x, y, objx, objy, objw, objh);
}

px_bool PX_ObjectIsCursorInRegion(PX_Object *Object, PX_Object_Event e) {
    return PX_ObjectIsPointInRegion(
        Object, PX_Object_Event_GetCursorX(e), PX_Object_Event_GetCursorY(e));
}

px_float PX_ObjectGetHeight(PX_Object *Object) { return Object->Height; }

px_float PX_ObjectGetWidth(PX_Object *Object) { return Object->Width; }
...
px_void PX_ObjectInitialize(px_memorypool *mp,
                            PX_Object *pObject,
                            PX_Object *Parent,
                            px_float x,
                            px_float y,
...
px_void PX_ObjectEnable(PX_Object *Object) { Object->Enabled = PX_TRUE; }

px_void PX_ObjectDisable(PX_Object *Object) { Object->Enabled = PX_FALSE; }

px_int PX_ObjectRegisterEvent(PX_Object *Object,
                              px_uint Event,
                              px_void (*ProcessFunc)(PX_Object *,
                                                     PX_Object_Event e,
                                                     px_void *user_ptr),
                              px_void *user) {
    PX_OBJECT_EventAction *pPoint;
    PX_OBJECT_EventAction *pAction = (PX_OBJECT_EventAction *) MP_Malloc(
        Object->mp, sizeof(PX_OBJECT_EventAction));
    if (pAction == PX_NULL) {
        return PX_FALSE;
    }
...
px_void PX_ObjectExecuteEvent(PX_Object *pPost, PX_Object_Event Event) {
    ...
    if (pPost->Visible == PX_FALSE || pPost->Enabled == PX_FALSE
        || pPost->ReceiveEvents == PX_FALSE) {
        return;
    }

    EventAction = pPost->pEventActions;
    while (EventAction) {
        if (EventAction->EventAction == PX_OBJECT_EVENT_ANY
            || EventAction->EventAction == Event.Event) {
...
px_bool PX_ObjectPostEventLink(PX_Object *pPost, PX_Object_Event Event) {
    ...
    if (pPost->Visible == PX_FALSE || pPost->Enabled == PX_FALSE
        || pPost->ReceiveEvents == PX_FALSE) {
        if (PX_ObjectPostEventLink(pPost->pNextBrother, Event) == PX_FALSE)
            return PX_FALSE;
        return PX_TRUE;
    }
...
px_void
PX_ObjectSetPosition(PX_Object *Object, px_float x, px_float y, px_float z) {
    if (Object == PX_NULL) {
        PX_ASSERT();
        return;
    }
...
px_void PX_ObjectSetSize(PX_Object *Object,
                         px_float Width,
                         px_float Height,
                         px_float length) {
    if (Object != PX_NULL) {
...
px_float PX_Designer_GetX(PX_Object *pObject) { return pObject->x; }

px_float PX_Designer_GetY(PX_Object *pObject) { return pObject->y; }

px_float PX_Designer_GetWidth(PX_Object *pObject) { return pObject->Width; }

px_float PX_Designer_GetHeight(PX_Object *pObject) { return pObject->Height; }

px_bool PX_Designer_GetEnable(PX_Object *pObject) { return pObject->Enabled; }

px_void PX_Designer_SetX(PX_Object *pObject, px_float v) { pObject->x = v; }

px_void PX_Designer_SetY(PX_Object *pObject, px_float v) { pObject->y = v; }

...
px_bool PX_CompilerCompile(PX_Compiler *compiler,
                           px_memory *bin,
    ...
    if (PX_ScriptCompilerCompile(&compiler->lib,
                                 entryScript,
                                 &asmcodeString,
                                 PX_COMPILER_DEFAULT_STACK_SIZE,
                                 compiler->error,
                                 sizeof(compiler->error))) {
        PX_ScriptAsmOptimization(&asmcodeString);

        if (!PX_ScriptAsmCompile(
                compiler->mp, asmcodeString.buffer, bin, pdebugmap)) {
            return PX_FALSE;
        }
...
px_char *PX_CompilerError(PX_Compiler *compiler) { return compiler->error; }

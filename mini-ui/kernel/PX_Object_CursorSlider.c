...
px_void PX_Object_CursorSliderUpdate(PX_Object *pObject, px_dword elapsed) {
    PX_Object_CursorSlider *pDesc
        = PX_ObjectGetDesc(PX_Object_CursorSlider, pObject);
    px_point2D v;
    px_float d, vd;

    vd = elapsed / 1000.0f * pDesc->reg_velocity;
    v = PX_Point2DSub(PX_POINT2D(pDesc->reg_x, pDesc->reg_y),
...
px_void PX_Object_CursorSliderRender(px_surface *psurface,
                                     PX_Object *pObject,
                                     px_dword elapsed) {
    px_int i;
    px_float d;
    ...
    while (update_elapsed) {
        ...
        for (i = 1; i < PX_COUNTOF(pDesc->sliderpoints); i++) {
            d = PX_Point2DDistance(pDesc->sliderpoints[i],
                                   pDesc->sliderpoints[i - 1]);
            if (d <= 0.1f) {
                pDesc->sliderpoints[i] = pDesc->sliderpoints[i - 1];
            } else {
                pDesc->sliderpoints[i].x = pDesc->sliderpoints[i].x
                                           + (pDesc->sliderpoints[i - 1].x
                                              - pDesc->sliderpoints[i].x)
                                                 / 2;
                pDesc->sliderpoints[i].y = pDesc->sliderpoints[i].y
                                           + (pDesc->sliderpoints[i - 1].y
                                              - pDesc->sliderpoints[i].y)
                                                 / 2;
            }
        }
    }
    ...
    for (i = PX_COUNTOF(pDesc->sliderpoints) - 2; i >= 0; i--) {
        ...
        target_a = (PX_OBJECT_CURSORSLIDER_COUNT - i) * 1.0f
                   / (PX_OBJECT_CURSORSLIDER_COUNT - 1);
        ...
        for (k = 0; k < (px_int) d; k++) {
            ...
            PX_GeoDrawPenCircle(psurface,
                                current_p.x,
                                current_p.y,
                                pObject->diameter / 2 * a,
                                pDesc->color);
        }
    }
    PX_GeoDrawPenCircle(psurface,
                        pDesc->sliderpoints[0].x,
                        pDesc->sliderpoints[0].y,
                        pObject->diameter / 2,
                        pDesc->color);
}

px_void
PX_Object_CursorSliderSetXY(PX_Object *pObject, px_float x, px_float y) {
...
px_void PX_Object_CursorSliderOnMove(PX_Object *pObject,
                                     PX_Object_Event e,
                                     px_void *ptr) {
    PX_Object_CursorSliderSetXY(
        pObject, PX_Object_Event_GetCursorX(e), PX_Object_Event_GetCursorY(e));
}

PX_Object *PX_Object_CursorSliderCreate(px_memorypool *mp,
                                        PX_Object *Parent,
                                        px_float x,
                                        px_float y,
                                        px_float radius,
    ...
    pObject = PX_ObjectCreateEx(mp,
                                ...
                                PX_Object_CursorSliderUpdate,
                                PX_Object_CursorSliderRender,
                                0,
                                &desc,
                                sizeof(desc));
    ...
    PX_ObjectRegisterEvent(
        pObject, PX_OBJECT_EVENT_CURSORMOVE, PX_Object_CursorSliderOnMove, 0);
    return pObject;
}

...
static px_void PX_Object_WidgetOnButtonClose(PX_Object *pObject,
                                             PX_Object_Event e,
                                             px_void *ptr) {
    PX_Object *pWidgetObject = (PX_Object *) ptr;
    PX_ObjectExecuteEvent(pWidgetObject,
                          PX_OBJECT_BUILD_EVENT(PX_OBJECT_EVENT_CLOSE));
    PX_Object_WidgetHide(pWidgetObject);
}

px_void PX_Object_Widget_EventDispatcher(PX_Object *pObject,
                                         PX_Object_Event e,
                                         px_void *ptr) {
    PX_Object_Event oe;
    PX_Object_Widget *pwidget = PX_Object_GetWidget(pObject);
    ...
    switch (e.Event) {
        case PX_OBJECT_EVENT_CURSORDOWN: {
            ...
            if (PX_ObjectIsCursorInRegion(pObject, e)) {
                ...
                if (!pwidget->bDraging) {
                    if (x > objx && x < objx + pObject->Width && y > objy
                        && y <= objy + PX_OBJECT_WIDGET_BAR_SIZE) {
    ...
    oe = PX_Object_Event_CursorOffset(
        e, PX_POINT(-objx, -objy - PX_OBJECT_WIDGET_BAR_SIZE, 0));
    PX_ObjectPostEvent(pwidget->root, oe);
}

px_void PX_Object_WidgetRender(px_surface *psurface,
                               PX_Object *pObject,
                               px_uint elapsed) {
    PX_Object_Widget *pwidget = PX_Object_GetWidget(pObject);
    px_float objx, objy, objWidth, objHeight;
    ...
    if (pObject->Width != (px_int) (pwidget->renderTarget.width)
        || pObject->Height
               != (px_int) (pwidget->renderTarget.height
                            + PX_OBJECT_WIDGET_BAR_SIZE)) {
        px_texture *old = &pwidget->renderTarget;
        if (!PX_TextureCreate(pObject->mp,
                              &pwidget->renderTarget,
    ...
    do {
        ...
        PX_SurfaceClear(&pwidget->renderTarget,
                        0,
                        0,
                        pwidget->renderTarget.width - 1,
                        pwidget->renderTarget.height - 1,
                        pwidget->backgroundcolor);
        PX_ObjectRender(&pwidget->renderTarget, pwidget->root, elapsed);
        PX_SurfaceRender(psurface,
                         &pwidget->renderTarget,
                         (px_int) objx,
                         (px_int) objy + PX_OBJECT_WIDGET_BAR_SIZE,
                         PX_ALIGN_LEFTTOP,
                         PX_NULL);
        if (pwidget->showShader) {
            ...
            PX_GeoDrawRect(psurface,
                           ...
                           (px_int) objy + 6,
                           (px_int) (objx + objWidth + 6),
                           (px_int) (objy + objHeight + 6),
                           sd);
            PX_GeoDrawRect(psurface,
                           (px_int) (objx + 6),
    ...
    PX_GeoDrawRect(psurface,
                   (px_int) objx,
                   (px_int) objy,
                   (px_int) (objx + pObject->Width - 1),
                   (px_int) objy + PX_OBJECT_WIDGET_BAR_SIZE - 1,
                   pwidget->barColor);
...
PX_Object *PX_Object_WidgetCreate(px_memorypool *mp,
                                  PX_Object *Parent,
                                  px_int x,
                                  px_int y,
                                  px_int width,
    ...
    PX_Object_Widget *pWidget
        = (PX_Object_Widget *) MP_Malloc(mp, sizeof(PX_Object_Widget));

    if (width <= 0)
        width = 1;
    if (height <= PX_OBJECT_WIDGET_BAR_SIZE)
        height = PX_OBJECT_WIDGET_BAR_SIZE + 1;
    ...
    if (!PX_TextureCreate(mp,
                          &pWidget->renderTarget,
                          width,
                          height - PX_OBJECT_WIDGET_BAR_SIZE))
        return PX_FALSE;

    pWidget->root = PX_ObjectCreate(mp, PX_NULL, 0, 0, 0, 0, 0, 0);
    pWidget->btn_close
        = PX_Object_PushButtonCreate(mp,
                                     pObject,
                                     width - PX_OBJECT_WIDGET_BAR_SIZE,
                                     3,
                                     PX_OBJECT_WIDGET_BAR_SIZE - 6,
                                     PX_OBJECT_WIDGET_BAR_SIZE - 6,
                                     "-",
                                     PX_NULL);

    if (!pWidget->btn_close)
        goto _ERROR;
    ...
    PX_ObjectRegisterEvent(pWidget->btn_close,
                           PX_OBJECT_EVENT_EXECUTE,
                           PX_Object_WidgetOnButtonClose,
                           pObject);
    PX_ObjectRegisterEvent(pObject,
                           PX_OBJECT_EVENT_ANY,
                           PX_Object_Widget_EventDispatcher,
                           PX_NULL);

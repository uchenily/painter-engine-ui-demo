...
static px_void PX_MenuSubMenuUpdateEx(PX_Object_Menu *pMenu,
    ...
    for (pNode = PX_ListNodeAt(&pItem->Items, 0); pNode;
         pNode = PX_ListNodeNext(pNode)) {
        px_int cWidth, cHeight;
        PX_Object_Menu_Item *pSubItem
            = PX_LIST_NODETDATA(PX_Object_Menu_Item, pNode);
        PX_FontModuleTextGetRenderWidthHeight(
            pMenu->fontmodule, pSubItem->Text, &cWidth, &cHeight);
        if (cWidth > maxWidth) {
            maxWidth = cWidth;
        }
    ...
    for (pNode = PX_ListNodeAt(&pItem->Items, 0); pNode;
        ...
        PX_Object_Menu_Item *pSubItem
            = PX_LIST_NODETDATA(PX_Object_Menu_Item, pNode);
        PX_MenuSubMenuUpdateEx(
            pMenu, pSubItem, pSubItem->x + pSubItem->width, pSubItem->y);
    }
}
static px_void PX_MenuSubMenuUpdate(px_int x, px_int y, PX_Object_Menu *pMenu) {
    ...
    for (pNode = PX_ListNodeAt(&pMenu->root.Items, 0); pNode;
         pNode = PX_ListNodeNext(pNode)) {
        px_int cWidth, cHeight;
        PX_Object_Menu_Item *pSubItem
            = PX_LIST_NODETDATA(PX_Object_Menu_Item, pNode);
    ...
    for (pNode = PX_ListNodeAt(&pMenu->root.Items, 0); pNode;
        ...
        PX_Object_Menu_Item *pSubItem
            = PX_LIST_NODETDATA(PX_Object_Menu_Item, pNode);
        PX_MenuSubMenuUpdateEx(
            pMenu, pSubItem, pSubItem->x, pSubItem->y + pSubItem->height);
    }
}
static px_bool PX_Object_MenuInitialize(px_memorypool *mp,
...
static px_void PX_MenuClearParent(PX_Object_Menu *pMenu,
                                  PX_Object_Menu_Item *pItem) {
    px_list_node *pNode = PX_NULL;

    for (pNode = PX_ListNodeAt(&pItem->pParent->Items, 0); pNode;
         pNode = PX_ListNodeNext(pNode)) {
        PX_Object_Menu_Item *pSubItem
            = PX_LIST_NODETDATA(PX_Object_Menu_Item, pNode);
...
static px_void PX_MenuClearChild(PX_Object_Menu *pMenu,
                                 PX_Object_Menu_Item *pItem) {
    px_list_node *pNode = PX_NULL;
...
px_bool PX_Menu_CursorAction(PX_Object *pObject,
                             PX_Object_Menu_Item *pItem,
                             px_float x,
                             px_float y,
    ...
    if (!pMenu->activating) {
        for (pNode = PX_ListNodeAt(&pMenu->root.Items, 0); pNode;
            ...
            if (PX_isXYInRegion(x,
                                ...
                                (px_float) pSubItem->x,
                                (px_float) pSubItem->y,
                                (px_float) pSubItem->width,
                                (px_float) pSubItem->height)
                && pSubItem->enable) {
        ...
        if (!PX_Menu_CursorAction(
                pObject, &pMenu->root, x, y, PX_MENU_CURSORACTION_DOWN)) {
            PX_MenuClearState(pObject);
        }
    }
}
px_void PX_MenuPostEvent_OnCursorMove(PX_Object *pObject, PX_Object_Event e) {
    ...
    px_float y = PX_Object_Event_GetCursorY(e);
    PX_Menu_CursorAction(
        pObject, &pMenu->root, x, y, PX_MENU_CURSORACTION_MOVE);
}
...
static px_void PX_MenuRenderItem(px_surface *pSurface,
                                 PX_Object_Menu *pMenu,
                                 PX_Object_Menu_Item *pItem,
                                 px_bool broot,
                                 px_dword elapsed) {
    if (pItem->width && pItem->height) {
        ...
        if (broot)
            ...
            if (pItem->Items.size != 0) {
                PX_GeoDrawTriangle(
                    pSurface,
                    PX_POINT2D((px_float) (pItem->x + pItem->width - 6),
                               (px_float) (pItem->y + pItem->height / 2)),
                    PX_POINT2D((px_float) (pItem->x + pItem->width - 12),
                               (px_float) (pItem->y + pItem->height / 2 - 4)),
                    PX_POINT2D((px_float) (pItem->x + pItem->width - 12),
                               (px_float) (pItem->y + pItem->height / 2 + 4)),
                    pMenu->fontColor);
            }
...
px_void PX_Object_MenuRenderEx(px_surface *pSurface,
                               PX_Object_Menu *pMenu,
                               px_dword elapsed) {
    px_list_node *pNode = PX_NULL;
...
PX_Object_Menu_Item *PX_Object_MenuAddItem(PX_Object *pObject,
                                           PX_Object_Menu_Item *parent,
                                           const px_char Text[],
                                           PX_MenuExecuteFunc _callback,
                                           px_void *ptr) {
    ...
    item.user_ptr = ptr;
    item.enable = PX_TRUE;
    ret = (PX_Object_Menu_Item *) PX_ListPush(
        &parent->Items, &item, sizeof(item));
    if (ret != PX_NULL) {
...
px_void
PX_Object_MenuRender(px_surface *psurface, PX_Object *Obj, px_uint elapsed) {
    PX_Object_MenuRenderEx(psurface, PX_Object_GetMenu(Obj), elapsed);
}

px_void PX_Object_MenuOnCursorEvent(PX_Object *pObject,
                                    PX_Object_Event e,
                                    px_void *ptr) {
    PX_Object_MenuOnCursorEventEx(pObject, e);
}

PX_Object *PX_Object_MenuCreate(px_memorypool *mp,
                                PX_Object *Parent,
                                px_int x,
                                px_int y,
                                px_int width,
    ...
    PX_Object *pObject;
    PX_Object_Menu *pMenu
        = (PX_Object_Menu *) MP_Malloc(mp, sizeof(PX_Object_Menu));
    ...
    pObject
        = PX_ObjectCreate(mp, Parent, (px_float) x, (px_float) y, 0, 0, 0, 0);
    if (pObject == PX_NULL) {

...
px_void PX_CanvasVM_PenDrawSamples(PX_CanvasVM *pCanvas,
                                   px_surface *psurface,
                                   px_point samples[],
                                   px_int detail) {
    px_int i;
    ...
    if (pCanvas->currentPath[0].z <= 1)
        clr._argb.a = (px_byte) (clr._argb.a / (pCanvas->reg_step / 6)
                                 * pCanvas->currentPath[0].z);
    for (i = 0; i < detail; i++) {
...
px_void PX_CanvasVMOn_PenPaint(PX_CanvasVM *pCanvas, px_void *ptr) {
    ...
    switch (pCanvas->reg_filtermode) {
        ...
        case PX_CanvasVM_FilterMode_PainterEngineFilter: {
            ...
            PX_GeoDrawPenSamplesLine(ptarget_surface,
                                     samples,
                                     count,
                                     pCanvas->reg_size,
                                     pCanvas->reg_color,
                                     pCanvas->reg_step,
                                     pCanvas->reg_filterfactor);
        } break;
        case PX_CanvasVM_FilterMode_Bezier: {
            ...
            for (i = 0; i < count; i++) {
                ...
                samples[i].y = pCanvas->currentPath[i].y;
                samples[i].z = pCanvas->currentPath[i].z;
                PX_GeoDrawPenCircle(ptarget_surface,
                                    pCanvas->currentPath[i].x,
                                    pCanvas->currentPath[i].y,
                                    pCanvas->reg_size,
                                    pCanvas->reg_color);
            }
            for (i = 1; i < count; i++) {
            ...
            for (i = 1; i < lenght; i++) {
                ...
                p1 = PX_GeoGetBezierCurvePoint(
                    samples, count, (i - 1) * 1.0f / (lenght - 1));
                for (j = 0; j < count; j++) {
                    samples[j].x = pCanvas->currentPath[j].x;
                    samples[j].y = pCanvas->currentPath[j].y;
                    samples[j].z = pCanvas->currentPath[j].z;
                }
                p2 = PX_GeoGetBezierCurvePoint(
                    samples, count, i * 1.0f / (lenght - 1));
                PX_GeoDrawPenLine(ptarget_surface,
                                  p1.x,
                                  p1.y,
                                  p2.x,
                                  p2.y,
        ...
        case PX_CanvasVM_FilterMode_Breseham: {
            ...
            for (i = 1; i < count; i++) {
                PX_GeoDrawBresenhamLine(ptarget_surface,
                                        (px_int) pCanvas->currentPath[i - 1].x,
                                        (px_int) pCanvas->currentPath[i - 1].y,
                                        (px_int) pCanvas->currentPath[i].x,
                                        (px_int) pCanvas->currentPath[i].y,
                                        pCanvas->reg_color);
            }
        } break;
        default:
            if (pCanvas->currentPathCount > 1) {
                ...
                detail = (detail > PX_CANVASVM_MAX_DETAIL_SIZE
                              ? PX_CANVASVM_MAX_DETAIL_SIZE
                              : detail);

                PX_CanvasVMFilter(
                    pCanvas, PX_CanvasVM_FilterMode_Liner, 1, samples, detail);
                PX_CanvasVM_PenDrawSamples(
                    pCanvas, ptarget_surface, samples, detail);
            }

            for (i = 2; i < pCanvas->currentPathCount - 1; i++) {
                px_float dx, dy;
                dx = (pCanvas->currentPath[i].x
                      - pCanvas->currentPath[i - 1].x);
                dy = (pCanvas->currentPath[i].y
                      - pCanvas->currentPath[i - 1].y);
                detail = (px_int) PX_sqrt(dx * dx + dy * dy);
                detail = (detail == 0 ? 1 : detail);
                detail = (detail > PX_CANVASVM_MAX_DETAIL_SIZE
                              ? PX_CANVASVM_MAX_DETAIL_SIZE
                ...
                PX_CanvasVMFilter(
                    pCanvas, pCanvas->reg_filtermode, i, samples, detail);
                PX_CanvasVM_PenDrawSamples(
                    pCanvas, ptarget_surface, samples, detail);
            }
            ...
            if (pCanvas->currentPathCount > 1) {
                px_float dx, dy;
                dx = (pCanvas->currentPath[pCanvas->currentPathCount - 1].x
                      - pCanvas->currentPath[pCanvas->currentPathCount - 2].x);
                dy = (pCanvas->currentPath[pCanvas->currentPathCount - 1].y
                      - pCanvas->currentPath[pCanvas->currentPathCount - 2].y);
                ...
                PX_CanvasVMFilter(pCanvas,
                                  PX_CanvasVM_FilterMode_Liner,
                                  pCanvas->currentPathCount - 1,
                                  samples,
                                  detail);
                PX_CanvasVM_PenDrawSamples(
                    pCanvas, ptarget_surface, samples, detail);
...
px_bool PX_CanvasVMInitialize(px_memorypool *mp, PX_CanvasVM *pCanvas) {
    ...
    if (!PX_VectorInitialize(
            mp, &pCanvas->payload_offsets, sizeof(px_int), 256)) {
        return PX_FALSE;
    }
...
px_void PX_CanvasVMOnBegin(PX_CanvasVM *pCanvas,
...
px_void PX_CanvasVMFilter(PX_CanvasVM *pCanvas,
                          PX_CanvasVM_FilterMode mode,
                          px_int index,
                          px_point samples[PX_CANVASVM_MAX_DETAIL_SIZE],
                          px_int detail) {
    if (detail <= 0) {
    ...
    if (index < 0 || index >= pCanvas->currentPathCount) {
        ...
        switch (mode) {
            ...
            case PX_CanvasVM_FilterMode_Liner: {
                ...
                step = PX_sqrt((x1 - x0) * (x1 - x0) + (y1 - y0) * (y1 - y0))
                       / detail;
                for (i = 0; i < detail; i++) {
                    samples[i] = PX_PointAdd(PX_POINT(x0, y0, z0),
                                             PX_PointMul(v, step * i));
                }
            } break;
            case PX_CanvasVM_FilterMode_BSpline: {
                if (index == 0) {
                    ...
                    v = PX_PointNormalization(v);
                    step
                        = PX_sqrt((x1 - x0) * (x1 - x0) + (y1 - y0) * (y1 - y0))
                          / detail;
                    for (i = 0; i < detail; i++) {
                        samples[i] = PX_PointAdd(PX_POINT(x0, y0, z0),
                                                 PX_PointMul(v, step * i));
                    ...
                    if (index < pCanvas->currentPathCount - 1) {
                        ...
                        px_float x0, y0, z0, x1, y1, z1, x2, y2, z2, x3, y3, z3,
                            d1, d2, d3;
                        px_int i;
                        ...
                        v = PX_Point2DMul(v,
                                          -0.25f * pCanvas->reg_filterfactor);
...
px_void
PX_CanvasVMOnMove(PX_CanvasVM *pCanvas, px_float x, px_float y, px_float z) {
    if (pCanvas->reg_state != PX_CanvasVM_State_Painting) {
        return;
    }
    ...
    if (pCanvas->reg_current_tool_index >= 0
        && pCanvas->reg_current_tool_index < pCanvas->reg_tools_count) {
        ...
        if (pCanvas->currentPathCount == 0) {
            ...
            if (pCanvas->currentPathCount > 2) {
                ...
                px_point2D v1, v2;
                v1 = PX_POINT2D(
                    pCanvas->currentPath[pCanvas->currentPathCount - 1].x
                        - pCanvas->currentPath[pCanvas->currentPathCount - 2].x,
                    pCanvas->currentPath[pCanvas->currentPathCount - 1].y
                        - pCanvas->currentPath[pCanvas->currentPathCount - 2]
                              .y);
...
px_void
PX_CanvasVMPaint(PX_CanvasVM *pCanvas, px_surface *prender, px_surface *pmask) {
    if (pCanvas->reg_current_tool_index >= 0
        && pCanvas->reg_current_tool_index < pCanvas->reg_tools_count) {
        pCanvas->ptarget_surface = prender;
        pCanvas->pmask_surface = pmask;
        pCanvas->tools[pCanvas->reg_current_tool_index].paint(
            pCanvas, pCanvas->tools[pCanvas->reg_current_tool_index].ptr);
        return;
    }
    PX_ASSERT();
}
...
px_bool PX_CanvasVMSelectToolByName(PX_CanvasVM *pCanvas,
                                    const px_char name[]) {
    px_int i;
    for (i = 0; i < PX_COUNTOF(pCanvas->tools); i++) {
...
px_void PX_CanvasVMStorePayload(PX_CanvasVM *pCanvas) {
    ...
    if (!PX_MemoryCat(&pCanvas->payload, &header, sizeof(header)))
        return;
    if (!PX_MemoryCat(&pCanvas->payload,
                      pCanvas->currentPath,
                      sizeof(PX_CanvasNode) * pCanvas->currentPathCount))
        return;
    pCanvas->currentPathCount = 0;
}

px_void PX_CanvasVMRegisterTool(PX_CanvasVM *pCanvas,

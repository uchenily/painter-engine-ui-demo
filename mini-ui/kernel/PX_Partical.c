...
px_void PX_ParticalLauncherInitializeDefaultInfo(
...
px_bool PX_ParticalLauncherInitialize(PX_ParticalLauncher *launcher,
                                      px_memorypool *mp,
                                      PX_ParticalLauncher_InitializeInfo Info) {
    ...
    launcher->ParticalPool = (PX_Partical_Atom *) MP_Malloc(
        mp, sizeof(PX_Partical_Atom) * launcher->LauncherInfo.maxCount);
    PX_memset(launcher->ParticalPool,
              0,
...
px_void PX_ParticalLauncherSetPosition(PX_ParticalLauncher *launcher,
                                       px_float x,
                                       px_float y,
                                       px_float z) {
...
px_void PX_ParticalAtomUpdate(PX_ParticalLauncher *env,
    ...
    if (pAtom->reg_alive) {
        ...
        while (updateTime) {
            ...
            pAtom->reg_currentrotation
                += pAtom->reg_rotation * atomTime / 1000.f;
...
px_void PX_ParticalLauncherUpdate(PX_ParticalLauncher *launcher,
                                  px_dword elapsed) {
    px_int i, j;
    px_int redTime = 0;
    px_int gencount = 0;
    ...
    if (launcher->LauncherInfo.launchCount != 0) {
        if (launcher->LauncherInfo.generateDuration != 0) {
            if (launcher->elapsed / launcher->LauncherInfo.generateDuration
                < ((launcher->elapsed + elapsed)
                   / launcher->LauncherInfo.generateDuration)) {
                gencount = (launcher->elapsed + elapsed)
                               / launcher->LauncherInfo.generateDuration
                           - launcher->elapsed
                                 / launcher->LauncherInfo.generateDuration;
                redTime = launcher->elapsed + elapsed
                          - (launcher->elapsed
                             + launcher->LauncherInfo.generateDuration)
                                / launcher->LauncherInfo.generateDuration
                                * launcher->LauncherInfo.generateDuration;
            ...
            if (launcher->LauncherInfo.launchCount != 0) {
                for (j = 0; j < (px_int) launcher->LauncherInfo.maxCount; j++) {
                    if (launcher->ParticalPool[j].reg_alive == 0) {
                        ...
                        PX_memset(&launcher->ParticalPool[j],
                                  0,
                                  sizeof(launcher->ParticalPool[j]));
                        launcher->lastgenIndex = j;
                        // position
                        launcher->ParticalPool[j].position
                            = launcher->LauncherInfo.position;
                        launcher->ParticalPool[j].position.x
                            += (px_float) (launcher->LauncherInfo
                                               .deviation_position_distanceRange
                                           * PX_cosd(randAngle));
                        launcher->ParticalPool[j].position.y
                            += (px_float) (launcher->LauncherInfo
                                               .deviation_position_distanceRange
                                           * PX_sind(randAngle));
                        // velocity
                        dirAngle = (px_float) PX_RadianToAngle(
                            PX_atan2(launcher->LauncherInfo.direction.y,
                                     launcher->LauncherInfo.direction.x));
                        dirAngle += (px_float) PX_randRange(
                            -launcher->LauncherInfo.deviation_rangAngle,
                            launcher->LauncherInfo.deviation_rangAngle);
                        var = launcher->LauncherInfo.velocity;
                        var += (px_float) PX_randRange(
                            launcher->LauncherInfo.deviation_velocity_min,
                            launcher->LauncherInfo.deviation_velocity_max);
                        launcher->ParticalPool[j].velocity = PX_POINT(
                            PX_cos_angle(dirAngle), PX_sin_angle(dirAngle), 0);
                        launcher->ParticalPool[j].velocity = PX_PointMul(
                            launcher->ParticalPool[j].velocity, var);
                        // size
                        var = launcher->LauncherInfo.atomsize;
                        var += (px_float) PX_randRange(
                            launcher->LauncherInfo.deviation_atomsize_min,
                        ...
                        // size inc
                        launcher->ParticalPool[j].reg_sizeinc
                            = launcher->LauncherInfo.sizeincrease;
                        // alpha inc
                        launcher->ParticalPool[j].reg_alphainc
                            = launcher->LauncherInfo.alphaincrease;
                        // a
                        launcher->ParticalPool[j].reg_a
                            = launcher->LauncherInfo.a;
                        // ak
                        launcher->ParticalPool[j].reg_ak
                            = launcher->LauncherInfo.ak;
                        // alive
                        launcher->ParticalPool[j].reg_alive
                            = launcher->LauncherInfo.alive;
                        // texture
                        launcher->ParticalPool[j].ptexture
                            = launcher->LauncherInfo.tex;

                        if (launcher->LauncherInfo.Create_func) {
                            launcher->ParticalPool[j]
                                = launcher->LauncherInfo.Create_func(
                                    launcher, launcher->genIndex);
                        }

                        PX_ParticalAtomUpdate(
                            launcher, &launcher->ParticalPool[j], redTime);

                        if (launcher->ParticalPool[j].position.x
                            < launcher->lefttopX) {
                            launcher->lefttopX
                                = launcher->ParticalPool[j].position.x;
                        }
                        if (launcher->ParticalPool[j].position.x
                            > launcher->rightBottomX) {
                            launcher->rightBottomX
                                = launcher->ParticalPool[j].position.x;
                        }
                        if (launcher->ParticalPool[j].position.y
                            < launcher->leftTopY) {
                            launcher->leftTopY
                                = launcher->ParticalPool[j].position.y;
                        }
                        if (launcher->ParticalPool[j].position.y
                            > launcher->rightBottomY) {
                            launcher->rightBottomY
                                = launcher->ParticalPool[j].position.y;
                        }
...
px_bool PX_ParticalIsInSurfaceRegion(px_point atomPoint,
                                     px_int atomWidth,
                                     px_int atomHeight,
                                     px_float scale,
                                     px_int surfaceWidth,
...
px_void PX_ParticalLauncherRender(px_surface *surface,
    ...
    if (!PX_isRectCrossRect(
            PX_RECT(
                0, 0, (px_float) surface->width, (px_float) surface->height),
            PX_RECT(launcher->lefttopX + launcher->LauncherInfo.position.x,
                    launcher->leftTopY + launcher->LauncherInfo.position.y,
                    launcher->rightBottomX - launcher->lefttopX + 1,
                    launcher->rightBottomY - launcher->leftTopY + 1))) {
        return;
    }
    ...
    for (m = 0; m < launcher->LauncherInfo.maxCount; m++, i++) {
        ...
        if (launcher->ParticalPool[i].reg_alive != 0
            && launcher->ParticalPool[i].reg_size != 0) {
            if (launcher->ParticalPool[i].reg_size != 1.0
                || launcher->ParticalPool[i].reg_currentrotation != 0) {
                if (launcher->ParticalPool[i].reg_size < 0) {
                    launcher->ParticalPool[i].reg_size = 0;
                    continue;
                }
                if (launcher->ParticalPool[i].reg_alpha == 1.0
                    && launcher->ParticalPool[i].hdrR == 1.0f
                    && launcher->ParticalPool[i].hdrG == 1.0f
                    && launcher->ParticalPool[i].hdrB == 1.0f)
                    PX_TextureRenderEx(
                        surface,
                        launcher->ParticalPool[i].ptexture,
                        (px_int) (pos.x),
                        (px_int) (pos.y),
                        PX_ALIGN_CENTER,
                        PX_NULL,
                        launcher->ParticalPool[i].reg_size,
...
px_void PX_ParticalLauncherSetDirection(PX_ParticalLauncher *launcher,
                                        px_point direction) {

...
px_bool PX_JsonInterpret_Object(PX_Json *pjson,
                                px_lexer *lexer,
                                PX_Json_Object *json_Object);
px_bool PX_JsonInterpret_Value(PX_Json *pjson,
                               px_lexer *lexer,
...
PX_Json_Value *PX_JsonGetObjectValue(PX_Json_Value *json_value,
                                     const px_char name[]) {
    px_int i;
    ...
    for (i = 0; i < json_value->_object.values.size; i++) {
        PX_Json_Value *pValue
            = PX_LISTAT(PX_Json_Value, &json_value->_object.values, i);
...
PX_Json_Value *PX_JsonGetObjectValueByIndex(PX_Json_Value *json_value,
                                            px_int i) {
...
PX_Json_Value *PX_JsonValueGetValue(PX_Json_Value *it,
                                    const px_char _payload[]) {
    px_int r_offset = 0;
    px_int s_offset = 0;
...
static px_bool PX_JsonInterpret_Value_Array(PX_Json *pjson,
                                            px_lexer *lexer,
                                            PX_Json_Value *_value) {
    PX_LEXER_LEXEME_TYPE type;
    ...
    if (type == PX_LEXER_LEXEME_TYPE_DELIMITER) {
        ...
        if (type == PX_LEXER_LEXEME_TYPE_TOKEN) {
            ...
            if (PX_strlen(lexer->CurLexeme.buffer) >= 3) {
                if (lexer->CurLexeme.buffer[0] == '0'
                    && (lexer->CurLexeme.buffer[1] == 'x'
                        || lexer->CurLexeme.buffer[1] == 'X')) {
                    _value->type = PX_JSON_VALUE_TYPE_NUMBER;
                    _value->_number
                        = (px_dword) PX_htoi(lexer->CurLexeme.buffer + 2);
                    return PX_TRUE;
                }
            }
        } else if (type == PX_LEXER_LEXEME_TYPE_CONATINER
                   && lexer->CurrentContainerType == json_quotes) {
            px_int i;
            _value->type = PX_JSON_VALUE_TYPE_STRING;
            if (!PX_StringInitialize(pjson->mp, &_value->_string))
                goto _ERROR;
            ;
            PX_LexerGetIncludedString(lexer, &lexer->CurLexeme);
        ...
        if (type == PX_LEXER_LEXEME_TYPE_DELIMITER && lexer->Symbol == '{') //{
        {
                ...
                && lexer->Symbol == '}') //{}
            {
                ...
                while (PX_TRUE) {
                    ...
                    if (!PX_JsonInterpret_Value(
                            pjson, lexer, &_value, PX_FALSE)) {
                        goto _ERROR;
                    }

                    if (!PX_ListPush(
                            &json_Object->values, &_value, sizeof(_value)))
                        goto _ERROR;
...
_ERROR:
    ...
    for (i = 0; i < json_Object->values.size; i++) {
        PX_JsonFreeValue(pjson,
                         PX_LISTAT(PX_Json_Value, &json_Object->values, i));
    }
...
px_bool PX_JsonParse(PX_Json *pjson, const px_char *json_content) {
    ...
    if (!PX_LexerLoadSourceFromMemory(&lexer, json_content))
        goto _ERROR;
...
px_bool PX_JsonBuild_Value(PX_Json_Value *pValue,
                           px_string *_out,
                           px_bool bArrayValue) {
    if (!bArrayValue) {
        // name
        if (!PX_StringCatCharEx(_out, '"'))
            return PX_FALSE;
        if (!PX_StringCatEx(_out, pValue->name.buffer))
            return PX_FALSE;
        if (!PX_StringCatCharEx(_out, '"'))
            return PX_FALSE;
        //:
    ...
    switch (pValue->type) {
        case PX_JSON_VALUE_TYPE_ARRAY: {
            ...
            for (i = 0; i < pValue->_array.size; i++) {
                PX_Json_Value *pv
                    = PX_LISTAT(PX_Json_Value, &pValue->_array, i);
                if (!PX_JsonBuild_Value(pv, _out, PX_TRUE))
                    return PX_FALSE;
                if (i != pValue->_array.size - 1)
                    if (!PX_StringCatCharEx(_out, ','))
                        return PX_FALSE;
            }
            if (!PX_StringCatCharEx(_out, ']'))
        ...
        case PX_JSON_VALUE_TYPE_OBJECT: {
            ...
            for (i = 0; i < pValue->_object.values.size; i++) {
                PX_Json_Value *ptmpValue
                    = PX_LISTAT(PX_Json_Value, &pValue->_object.values, i);
                if (!PX_JsonBuild_Value(ptmpValue, _out, PX_FALSE))
                    return PX_FALSE;
                if (i != pValue->_object.values.size - 1)
                    if (!PX_StringCatEx(_out, ","))
                        return PX_FALSE;
                if (!PX_StringCatEx(_out, "\n"))
                    return PX_FALSE;
    ...
    switch (pParentValue->type) {
        ...
        case PX_JSON_VALUE_TYPE_OBJECT: {
            ...
            for (j = 0; j < pParentValue->_object.values.size; j++) {
                PX_Json_Value *pTarget;
                pTarget = PX_LISTAT(
                    PX_Json_Value, &pParentValue->_object.values, j);
                if (pTarget == pValue) {
                    PX_JsonDeleteValue(pjson, pTarget);
    ...
    switch (json_Value->type) {
        case PX_JSON_VALUE_TYPE_ARRAY: {
            ...
            for (i = 0; i < json_Value->_array.size; i++) {
                PX_JsonFreeValue(
                    pjson, PX_LISTAT(PX_Json_Value, &json_Value->_array, i));
            }
...
px_bool PX_JsonCreateObjectValue(px_memorypool *mp,
                                 PX_Json_Value *pValue,
                                 const px_char name[]) {
    ...
    if (!PX_StringSet(&pValue->name, name))
        return PX_FALSE;
    PX_ListInitialize(mp, &pValue->_object.values);
    return PX_TRUE;
}

px_bool PX_JsonCreateStringValue(px_memorypool *mp,
                                 PX_Json_Value *pValue,
                                 const px_char name[],
                                 const px_char text[]) {
    PX_memset(pValue, 0, sizeof(PX_Json_Value));
    ...
    if (!PX_StringSet(&pValue->_string, text))
        return PX_FALSE;
    return PX_TRUE;
}

px_bool PX_JsonCreateNumberValue(px_memorypool *mp,
                                 PX_Json_Value *pValue,
                                 const px_char name[],
                                 px_double value) {
    PX_memset(pValue, 0, sizeof(PX_Json_Value));
...
px_bool PX_JsonCreateBooleanValue(px_memorypool *mp,
                                  PX_Json_Value *pValue,
                                  const px_char name[],
                                  px_bool b) {
    PX_memset(pValue, 0, sizeof(PX_Json_Value));
...
px_bool PX_JsonCreateArrayValue(px_memorypool *mp,
                                PX_Json_Value *pValue,
                                const px_char name[]) {
    PX_memset(pValue, 0, sizeof(PX_Json_Value));
...
px_bool PX_JsonAddString(PX_Json *pjson,
                         const px_char parent_payload[],
                         const px_char name[],
                         const px_char text[]) {
    ...
    switch (pValue->type) {
        ...
        case PX_JSON_VALUE_TYPE_OBJECT: {
            ...
            if (!pExistValue) {
                PX_JsonCreateStringValue(pjson->mp, &newValue, name, text);
                PX_ListPush(
                    &pValue->_object.values, &newValue, sizeof(newValue));
            } else {
                if (pExistValue->type != PX_JSON_VALUE_TYPE_STRING) {
                    return PX_FALSE;
                }
...
px_bool PX_JsonAddNumber(PX_Json *pjson,
                         const px_char parent_payload[],
                         const px_char name[],
...
px_bool PX_JsonAddBoolean(PX_Json *pjson,
                          const px_char parent_payload[],
                          const px_char name[],
                          const px_bool b) {
    PX_Json_Value *pValue = PX_JsonGetValue(pjson, parent_payload);
...
px_bool PX_JsonAddArray(PX_Json *pjson,
                        const px_char parent_payload[],
                        const px_char name[]) {
    PX_Json_Value *pValue = PX_JsonGetValue(pjson, parent_payload);
    PX_Json_Value newValue;
...
px_bool PX_JsonAddObject(PX_Json *pjson,
                         const px_char parent_payload[],
                         const px_char name[]) {
    PX_Json_Value *pValue = PX_JsonGetValue(pjson, parent_payload);
    PX_Json_Value newValue;
...
const px_char *PX_JsonValueGetString(PX_Json_Value *_pValue,
...
px_double PX_JsonValueGetNumber(PX_Json_Value *_pValue,
                                const px_char payload[]) {
    PX_Json_Value *pValue = PX_JsonValueGetValue(_pValue, payload);
    if (pValue && pValue->type == PX_JSON_VALUE_TYPE_NUMBER) {
        return pValue->_number;
...
px_bool PX_JsonValueGetBoolean(PX_Json_Value *_pValue,
                               const px_char payload[]) {
    PX_Json_Value *pValue = PX_JsonValueGetValue(_pValue, payload);
    if (pValue && pValue->type == PX_JSON_VALUE_TYPE_BOOLEAN) {
        return pValue->_boolean;
...
px_bool PX_JsonSetNumber(PX_Json *pjson,
                         const px_char payload[],
                         const px_double number) {
    PX_Json_Value *pValue = PX_JsonGetValue(pjson, payload);
    if (pValue && pValue->type == PX_JSON_VALUE_TYPE_NUMBER) {
...
px_bool
PX_JsonSetBoolean(PX_Json *pjson, const px_char payload[], const px_bool b) {
    PX_Json_Value *pValue = PX_JsonGetValue(pjson, payload);
    if (pValue && pValue->type == PX_JSON_VALUE_TYPE_BOOLEAN) {
        pValue->_boolean = b;
        return PX_TRUE;
...
px_bool PX_JsonObjectAddValue(PX_Json_Value *pObject, PX_Json_Value *value) {
    if (pObject->type == PX_JSON_VALUE_TYPE_OBJECT) {
        return PX_ListPush(
                   &pObject->_object.values, value, sizeof(PX_Json_Value))
               != PX_NULL;
    }
    return PX_FALSE;
}
...
px_bool PX_JsonArrayAddValue(PX_Json_Value *pArray, PX_Json_Value *value) {
    if (pArray->type == PX_JSON_VALUE_TYPE_ARRAY) {
        return PX_ListPush(&pArray->_array, value, sizeof(PX_Json_Value))
               != PX_NULL;
    }
    return PX_FALSE;
}

...
px_void PX_Object_SliderBarOnMouseLButtonDown(PX_Object *pObject,
                                              PX_Object_Event e,
                                              px_void *user_ptr) {
            ...
            if (pSliderBar->Type == PX_OBJECT_SLIDERBAR_TYPE_HORIZONTAL) {
                switch (pSliderBar->style) {
                    case PX_OBJECT_SLIDERBAR_STYLE_LINER: {
                        Sx = objx
                             + (px_int) ((objw - pSliderBar->SliderButtonLength)
                                         * (1.0f) * relValue / Range);
                        ...
                        if (PX_isPointInRect(
                                PX_POINT((px_float) x, (px_float) y, 0),
                                rect)) {
                    ...
                    case PX_OBJECT_SLIDERBAR_STYLE_BOX: {
                        ...
                        if (PX_isPointInRect(
                            ...
                            pSliderBar->Value
                                += Range * pSliderBar->SliderButtonLength
                                   / ((px_int) objw);
            ...
            if (pSliderBar->Type == PX_OBJECT_SLIDERBAR_TYPE_VERTICAL) {
                switch (pSliderBar->style) {
                    case PX_OBJECT_SLIDERBAR_STYLE_LINER: {
                        Sx = (px_int) objx;
                        Sy = (px_int) objy
                             + ((px_int) objh - pSliderBar->SliderButtonLength)
                                   * relValue / Range;
...
px_void PX_Object_SliderBarOnCursorNormal(PX_Object *pObject,
                                          PX_Object_Event e,
                                          px_void *user_ptr) {
    PX_Object_SliderBar *pSliderBar = PX_Object_GetSliderBar(pObject);
...
px_void PX_Object_SliderBarOnCursorDrag(PX_Object *pObject,
                                        PX_Object_Event e,
                                        px_void *user_ptr) {
    PX_Object_SliderBar *pSliderBar = PX_Object_GetSliderBar(pObject);
    px_float x, y;
    ...
    if (pSliderBar) {
        ...
        if (pSliderBar->Type == PX_OBJECT_SLIDERBAR_TYPE_HORIZONTAL) {
            ...
            if (x > objx + objWidth) {
                pSliderBar->DargButtonX
                    = objWidth - pSliderBar->SliderButtonLength;
                return;
            }
...
PX_Object *PX_Object_SliderBarCreate(px_memorypool *mp,
                                     PX_Object *Parent,
                                     px_int x,
                                     px_int y,
                                     px_int Width,
    ...
    PX_ObjectRegisterEvent(pObject,
                           ...
                           PX_Object_SliderBarOnMouseLButtonDown,
                           PX_NULL);
    PX_ObjectRegisterEvent(pObject,
                           PX_OBJECT_EVENT_CURSORMOVE,
                           PX_Object_SliderBarOnCursorNormal,
                           PX_NULL);
    PX_ObjectRegisterEvent(pObject,
    ...
    PX_ObjectRegisterEvent(pObject,
                           ...
                           PX_Object_SliderBarOnCursorDrag,
                           PX_NULL);
    return pObject;
}
...
px_void
PX_Object_SliderBarSetRange(PX_Object *pSliderBar, px_int Min, px_int Max) {
    PX_Object_SliderBar *SliderBar = PX_Object_GetSliderBar(pSliderBar);
    if (SliderBar != PX_NULL) {
        SliderBar->Min = Min;
        SliderBar->Max = Max;
...
px_void PX_Object_SliderBarRender(px_surface *psurface,
                                  PX_Object *pObject,
                                  px_uint elapsed) {
    PX_Object_SliderBar *pSliderBar = PX_Object_GetSliderBar(pObject);
    px_int SliderBtnLen;
    ...
    switch (pSliderBar->status) {
        case PX_OBJECT_SLIDERBAR_STATUS_NORMAL: {
            ...
            if (pSliderBar->Type == PX_OBJECT_SLIDERBAR_TYPE_HORIZONTAL) {
                switch (pSliderBar->style) {
                    case PX_OBJECT_SLIDERBAR_STYLE_LINER: {
                        Sx = (objWidth - pSliderBar->SliderButtonLength)
                             * relValue / Range;
                        ...
                        PX_GeoDrawRect(psurface,
                                       (px_int) Sx,
                                       (px_int) Sy,
                                       (px_int) (Sx + SliderBtnLen - 1),
                                       (px_int) (Sy + objHeight - 1),
                                       pSliderBar->color);
                    } break;
                    case PX_OBJECT_SLIDERBAR_STYLE_BOX: {
                        // draw border
                        ...
                        PX_GeoDrawRect(
                            psurface,
                            (px_int) (objx + Sx - SliderBtnLen / 2 + 2),
                            (px_int) (objy + 2),
                            (px_int) (objx + Sx - SliderBtnLen / 2
                                      + SliderBtnLen - 3),
                            (px_int) (objy + objHeight - 3),
                            pSliderBar->color);
                    } break;
                }
            }
            ...
            if (pSliderBar->Type == PX_OBJECT_SLIDERBAR_TYPE_VERTICAL) {
                switch (pSliderBar->style) {
                    case PX_OBJECT_SLIDERBAR_STYLE_LINER: {
                        Sx = objx;
                        Sy = (objHeight - pSliderBar->SliderButtonLength)
                             * relValue / Range;
                        ...
                        PX_GeoDrawRect(psurface,
                                       (px_int) Sx,
                                       (px_int) Sy,
                                       (px_int) (Sx + objWidth - 1),
                                       (px_int) (Sy + SliderBtnLen - 1),
                                       pSliderBar->color);
                    } break;
                    case PX_OBJECT_SLIDERBAR_STYLE_BOX: {
                        // draw border
                        ...
                        PX_GeoDrawRect(
                            psurface,
                            (px_int) objx + 2,
                            (px_int) (objy + Sy - SliderBtnLen / 2 + 2),
                            (px_int) (objx + objWidth - 1 - 2),
                            (px_int) (objy + Sy + SliderBtnLen / 2 - 3),
                            pSliderBar->color);
                    } break;
                }
            }
                ...
                switch (pSliderBar->style) {
                    ...
                    case PX_OBJECT_SLIDERBAR_STYLE_BOX: {
                        ...
                        PX_GeoDrawRect(
                            ...
                            (px_int) (objx + Sx - SliderBtnLen / 2 + 2),
                            (px_int) objy + 2,
                            (px_int) (objx + Sx + SliderBtnLen / 2 - 3),
                            (px_int) (objy + objHeight - 3),
                            pSliderBar->color);
                    } break;
                }
...
px_void PX_Object_SliderBarSetBackgroundColor(PX_Object *pSliderBar,
                                              px_color color) {
    PX_Object_SliderBar *SliderBar = PX_Object_GetSliderBar(pSliderBar);
    if (SliderBar != PX_NULL) {
...
px_void PX_Object_SliderBarSetSliderButtonLength(PX_Object *pSliderBar,
                                                 px_int length) {
    PX_Object_SliderBar *SliderBar = PX_Object_GetSliderBar(pSliderBar);
    if (SliderBar != PX_NULL) {
...
// SliderBar
//////////////////////////////////////////////////////////////////////////
PX_Object *PX_Designer_SliderBarCreate(px_memorypool *mp,
                                       PX_Object *pparent,
                                       px_float x,
                                       px_float y,
                                       px_float width,
    ...
    return PX_Object_SliderBarCreate(mp,
                                     pparent,
                                     (px_int) x,
                                     (px_int) y,
                                     192,
...
px_void PX_Designer_SliderBarSetMax(PX_Object *pobject, px_int max) {
    PX_Object_SliderBarSetRange(
        pobject, PX_ObjectGetDesc(PX_Object_SliderBar, pobject)->Min, max);
}
...
px_void PX_Designer_SliderBarSetMin(PX_Object *pobject, px_int min) {
    PX_Object_SliderBarSetRange(
        pobject, min, PX_ObjectGetDesc(PX_Object_SliderBar, pobject)->Max);
}
...
px_void PX_Designer_SliderBarSetType(PX_Object *pobject, px_int type) {
    switch (type) {
        ...
        case PX_OBJECT_SLIDERBAR_TYPE_VERTICAL:
            PX_ObjectGetDesc(PX_Object_SliderBar, pobject)->Type
                = (PX_OBJECT_SLIDERBAR_TYPE) type;
        default:
            break;
    }
...
px_void PX_Designer_SliderBarSetStyle(PX_Object *pobject, px_int style) {
    switch (style) {
        case PX_OBJECT_SLIDERBAR_STYLE_BOX:
        case PX_OBJECT_SLIDERBAR_STYLE_LINER:
            PX_ObjectGetDesc(PX_Object_SliderBar, pobject)->style
                = (PX_OBJECT_SLIDERBAR_STYLE) style;
        default:
            break;
    }

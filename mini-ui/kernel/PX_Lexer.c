...
static px_char *PX_LexerIsCommentEnd(px_lexer *lexer,
                                     const px_char startch[],
                                     const px_char ch[]);
static px_char *PX_LexerIsContainerStart(px_lexer *lexer, const px_char ch[]);
static char *PX_LexerIsContainerEnd(px_lexer *lexer,
                                    const px_char startch[],
                                    const px_char ch[]);
static px_bool PX_LexerIsContainerTransfer(px_lexer *lexer,
                                           const px_char startch[],
                                           px_char ch);
...
px_char *PX_LexerIsCommentStart(px_lexer *lexer, const px_char ch[]) {
    ...
    for (i = 0; i < lexer->CommentCount; i++) {
        if (PX_memequ(ch,
                      lexer->Comment[i].CommentBegin,
                      PX_strlen(lexer->Comment[i].CommentBegin)))
            return lexer->Comment[i].CommentBegin;
    }
    return PX_NULL;
}

px_char *PX_LexerIsCommentEnd(px_lexer *lexer,
                              const px_char startch[],
                              const px_char ch[]) {
    px_int i = 0;
    for (i = 0; i < lexer->CommentCount; i++) {
        if (PX_memequ(startch,
                      lexer->Comment[i].CommentBegin,
                      PX_strlen(lexer->Comment[i].CommentBegin))
            && PX_memequ(ch,
                         lexer->Comment[i].CommentEnd,
                         PX_strlen(lexer->Comment[i].CommentEnd))) {
...
px_int PX_LexerRegisterContainer(px_lexer *lexer,
                                 const px_char Begin[],
                                 const px_char End[]) {
    px_int i;
    if (lexer->ContainerCount < PX_LEXER_CA_DELIMITER_MAX_COUNT) {
        for (i = 0; i < lexer->ContainerCount; i++) {
            if (lexer->Container[i].ContainerBegin == Begin
                && lexer->Container[i].ContainerEnd == End) {
                return i;
            }
        }
        PX_strcpy(
            lexer->Container[lexer->ContainerCount].ContainerBegin,
            Begin,
            sizeof(lexer->Container[lexer->ContainerCount].ContainerBegin));
        PX_strcpy(lexer->Container[lexer->ContainerCount].ContainerEnd,
                  End,
...
px_void PX_LexerRegisterContainerTransfer(px_lexer *lexer,
                                          px_uint containerIndex,
                                          px_char transfer) {
...
px_char *PX_LexerIsContainerEnd(px_lexer *lexer,
                                const px_char startch[],
                                const px_char ch[]) {
    px_int i = 0;
    for (i = 0; i < lexer->ContainerCount; i++) {
        if (PX_memequ(
                startch, lexer->Container[i].ContainerBegin, PX_strlen(startch))
            && PX_memequ(ch,
                         lexer->Container[i].ContainerEnd,
                         PX_strlen(lexer->Container[i].ContainerEnd))) {
...
px_bool
PX_LexerSortTextMap(px_lexer *lexer, const px_char *SourceText, px_bool map) {
    px_char *chrst, *chred;
    ...
    if ((lexer->Sources
         = (px_char *) MP_Malloc(lexer->mp, PX_strlen(SourceText) + 1))
        == PX_NULL) {
        return PX_FALSE;
    }
    ...
    while (SourceText[r_Offset]) {
        // Comment trim
        if ((chrst = PX_LexerIsCommentStart(lexer, SourceText + r_Offset))
            != 0) {
            r_Offset += PX_strlen(chrst);
            while (!(chred = PX_LexerIsCommentEnd(
                         lexer, chrst, SourceText + r_Offset))) {
        ...
        // container skip
        if ((chrst = PX_LexerIsContainerStart(lexer, SourceText + r_Offset))
            != 0) {
            ...
            while (!(chred = PX_LexerIsContainerEnd(
                         lexer, chrst, SourceText + r_Offset))) {
                if (SourceText[r_Offset] == '\0') {
                    return PX_FALSE;
                }

                if (PX_LexerIsContainerTransfer(
                        lexer, chrst, SourceText[r_Offset])
                    && PX_memequ(
                        SourceText + r_Offset + 1, chrst, PX_strlen(chrst))) {
...
PX_LEXER_LEXEME_TYPE PX_LexerGetNextLexeme(px_lexer *lexer) {
    ...
    if ((chrst = PX_LexerIsContainerStart(
             lexer, (&lexer->Sources[lexer->SourceOffset])))
        != 0) {
        PX_StringCat(&lexer->CurLexeme, chrst);
        lexer->SourceOffset += PX_strlen(chrst);
        while (!(chred = PX_LexerIsContainerEnd(
                     lexer, chrst, &lexer->Sources[lexer->SourceOffset]))) {
            if (PX_LexerIsSourcsEnd(lexer)) {
                return PX_LEXER_LEXEME_TYPE_ERR;
            }
            if (PX_LexerIsContainerTransfer(
                    lexer, chrst, lexer->Sources[lexer->SourceOffset])
                && PX_memequ(&lexer->Sources[lexer->SourceOffset + 1],
                             chrst,
                             PX_strlen(chrst))) {
                PX_StringCatChar(&lexer->CurLexeme,
    ...
    while (!PX_LexerIsSourcsEnd(lexer)
           ...
           && !PX_LexerIsContainerStart(lexer,
                                        &lexer->Sources[lexer->SourceOffset])) {
        PX_StringCatChar(&lexer->CurLexeme,
                         lexer->Sources[lexer->SourceOffset++]);
        ...
        if (lexer->NumericMath && lexer->Sources[lexer->SourceOffset] == '.'
            ...
            if (PX_StringIsNumeric(&lexer->CurLexeme)
                ...
                lexer->SourceOffset++;
                while (
                    !PX_LexerIsSourcsEnd(lexer)
                    && !PX_LexerIsDelimiter(lexer,
                                            lexer->Sources[lexer->SourceOffset])
                    && !PX_LexerIsSpacer(lexer,
                                         lexer->Sources[lexer->SourceOffset])
                    && !PX_LexerIsNewLine(lexer->Sources[lexer->SourceOffset])
                    && !PX_LexerIsContainerStart(
                        lexer, &lexer->Sources[lexer->SourceOffset])) {
                    if (lexer->Sources[lexer->SourceOffset] >= '0'
                        && lexer->Sources[lexer->SourceOffset] <= '9') {
                        lexer->SourceOffset++;
                        continue;
                    } else {
                        match = PX_FALSE;
...
px_void PX_LexerGetIncludedString(px_lexer *lexer, px_string *str) {
    ...
    if (lexer->CurrentLexemeFlag == PX_LEXER_LEXEME_TYPE_CONATINER) {
        left = PX_strlen(
            lexer->Container[lexer->CurrentContainerType].ContainerBegin);
        right = PX_strlen(
            lexer->Container[lexer->CurrentContainerType].ContainerEnd);
...
px_void PX_LexerRegisterComment(px_lexer *lexer,
                                const px_char Begin[],
                                const px_char End[]) {
    px_int i;
    if (lexer->CommentCount < PX_LEXER_CA_COMMENT_MAX_COUNT) {
        for (i = 0; i < lexer->CommentCount; i++) {
            if (lexer->Comment[i].CommentBegin == Begin
                && lexer->Comment[i].CommentEnd == End) {
                return;
            }
        }
        PX_strcpy(lexer->Comment[lexer->CommentCount].CommentBegin,
                  Begin,
                  sizeof(lexer->Comment[lexer->CommentCount].CommentBegin));
        PX_strcpy(lexer->Comment[lexer->CommentCount].CommentEnd,
                  End,
...
px_char PX_LexerGetSymbol(px_lexer *lexer) { return lexer->Symbol; }
...
px_bool PX_LexerIsContainerTransfer(px_lexer *lexer,
                                    const px_char startch[],
                                    px_char ch) {
    px_int i = 0;
    for (i = 0; i < lexer->ContainerCount; i++) {
        if (PX_memequ(
                startch, lexer->Container[i].ContainerBegin, PX_strlen(startch))
            && lexer->Container[i].transfer
            && lexer->Container[i].transfer == ch) {
            return PX_TRUE;
        }
    }
    return PX_FALSE;

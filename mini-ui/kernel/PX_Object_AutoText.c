...
PX_Object *PX_Object_AutoTextCreate(px_memorypool *mp,
                                    PX_Object *Parent,
                                    px_int x,
                                    px_int y,
                                    px_int width,
    ...
    PX_Object_AutoText *pAt
        = (PX_Object_AutoText *) MP_Malloc(mp, sizeof(PX_Object_AutoText));
    ...
    pObject = PX_ObjectCreate(
        mp, Parent, (px_float) x, (px_float) y, 0, (px_float) width, 0, 0);
...
px_void PX_Object_AutoTextRender(px_surface *psurface,
                                 PX_Object *pObject,
                                 px_uint elapsed) {
    ...
    while (PX_TRUE) {
        ...
        if (pAt->fontModule) {
            ...
            fsize = PX_FontModuleGetCharacterDesc(
                pAt->fontModule, Text + cursor, &code, &width, &height);
            if (!fsize) {
                break;
            }
            if (code == '\r') {
                ...
                if (psurface)
                    PX_FontModuleDrawCharacter(psurface,
                                               pAt->fontModule,
                                               x_draw_oft,
                                               y_draw_oft,
                                               code,
            ...
            if (x_draw_oft
                > objx + objWidth - PX_FontGetAscCharactorWidth() * 2) {
                x_draw_oft = (px_int) objx;
                y_draw_oft += __PX_FONT_HEIGHT + 2;
                h += __PX_FONT_HEIGHT + 2;
            }

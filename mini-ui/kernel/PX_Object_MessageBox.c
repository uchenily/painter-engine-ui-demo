...
static px_void PX_Object_MessageBox_BtnYesClick(PX_Object *pObject,
                                                PX_Object_Event e,
                                                px_void *user) {
    PX_Object_MessageBox *pm = PX_Object_GetMessageBox((PX_Object *) user);
    PX_Object_MessageBoxClose((PX_Object *) user);
    if (pm->function_yes) {
        pm->function_yes((PX_Object *) user,
                         PX_OBJECT_BUILD_EVENT(PX_OBJECT_EVENT_EXECUTE),
                         pm->function_yes_ptr);
    }
}

static px_void PX_Object_MessageBox_BtnNoClick(PX_Object *pObject,
                                               PX_Object_Event e,
                                               px_void *user) {
    PX_Object_MessageBox *pm = PX_Object_GetMessageBox((PX_Object *) user);
    PX_Object_MessageBoxClose((PX_Object *) user);
    if (pm->function_no) {
        pm->function_no((PX_Object *) user,
                        PX_OBJECT_BUILD_EVENT(PX_OBJECT_EVENT_EXECUTE),
                        pm->function_yes_ptr);
    }
}

static px_void PX_Object_MessageBox_EditOnEnter(PX_Object *pObject,
                                                PX_Object_Event e,
                                                px_void *user) {
    if (e.Event == PX_OBJECT_EVENT_KEYDOWN) {
        if (PX_Object_Event_GetKeyDown(e) == '\r') {
            PX_Object_MessageBox *pm
                = PX_Object_GetMessageBox((PX_Object *) user);
            PX_Object_MessageBoxClose((PX_Object *) user);
            if (pm->function_yes) {
                pm->function_yes((PX_Object *) user,
                                 PX_OBJECT_BUILD_EVENT(PX_OBJECT_EVENT_EXECUTE),
...
static px_void PX_Object_MessageBoxRender(px_surface *pSurface,
                                          PX_Object *pObject,
                                          px_dword elapsed) {
    px_float x, y, w, h;
    ...
    if (pm->schedule < PX_OBJECT_MESSAGEBOX_DEFAULT_STAGE_1_TIME) {
        px_int width = pSurface->width * pm->schedule * pm->schedule
                       / (PX_OBJECT_MESSAGEBOX_DEFAULT_STAGE_1_TIME
                          * PX_OBJECT_MESSAGEBOX_DEFAULT_STAGE_1_TIME);
        PX_GeoDrawRect(
            pSurface,
            (pSurface->width - width) / 2,
            (pSurface->height - pm->PX_MESSAGEBOX_STAGE_1_HEIGHT) / 2,
            (pSurface->width + width) / 2,
            (pSurface->height + pm->PX_MESSAGEBOX_STAGE_1_HEIGHT) / 2,
            backGroundColor);
    } else if (pm->schedule < PX_OBJECT_MESSAGEBOX_DEFAULT_STAGE_2_TIME) {
        px_int height = PX_OBJECT_MESSAGEBOX_DEFAULT_STAGE_2_HEIGHT
                        * pm->schedule * pm->schedule
                        / (PX_OBJECT_MESSAGEBOX_DEFAULT_STAGE_2_TIME
                           * PX_OBJECT_MESSAGEBOX_DEFAULT_STAGE_2_TIME);
        PX_GeoDrawRect(pSurface,
                       0,
                       (pSurface->height - height) / 2,
                       pSurface->width - 1,
                       (pSurface->height + height) / 2,
                       backGroundColor);
    } else {
        PX_GeoDrawRect(
            pSurface,
            0,
            (pSurface->height - pm->PX_MESSAGEBOX_STAGE_2_HEIGHT) / 2,
            pSurface->width - 1,
            (pSurface->height + pm->PX_MESSAGEBOX_STAGE_2_HEIGHT) / 2,
            backGroundColor);
        PX_GeoDrawRect(
            pSurface,
            0,
            (pSurface->height - pm->PX_MESSAGEBOX_STAGE_2_HEIGHT) / 2 - 30,
            pSurface->width - 1,
            (pSurface->height - pm->PX_MESSAGEBOX_STAGE_2_HEIGHT) / 2 - 10,
            backGroundColor);
        PX_GeoDrawRect(
            pSurface,
            0,
...
px_void PX_Object_MessageBoxAlertOk(PX_Object *pObject,
                                    const px_char *message,
                                    PX_Object_MessageBoxCallBack func_callback,
                                    px_void *ptr) {
    PX_Object_MessageBox *pm = PX_Object_GetMessageBox(pObject);
...
px_void
PX_Object_MessageBoxAlertYesNo(PX_Object *pObject,
                               const char *Message,
                               PX_Object_MessageBoxCallBack func_yescallback,
                               px_void *yesptr,
                               PX_Object_MessageBoxCallBack func_nocallback,
                               px_void *noptr) {
    PX_Object_MessageBox *pm = PX_Object_GetMessageBox(pObject);
...
px_void
PX_Object_MessageBoxInputBox(PX_Object *pObject,
                             const char *Message,
                             PX_Object_MessageBoxCallBack func_yescallback,
                             px_void *yesptr,
                             PX_Object_MessageBoxCallBack func_cancelcallback,
                             px_void *cancelptr) {
    PX_Object_MessageBox *pm = PX_Object_GetMessageBox(pObject);
    pm->function_yes = func_yescallback;
...
PX_Object *PX_Object_MessageBoxCreate(px_memorypool *mp,
                                      PX_Object *parent,
    ...
    pObject = PX_ObjectCreateEx(mp,
                                ...
                                PX_NULL,
                                PX_Object_MessageBoxRender,
                                PX_Object_MessageBoxFree,
                                pm,
                                sizeof(PX_Object_MessageBox));
    pObject->Visible = PX_FALSE;
    ...
    if (!(pm->btn_Ok = PX_Object_PushButtonCreate(
              mp, pObject, 0, 0, 84, 28, "OK", PX_NULL)))
        return PX_FALSE;

    if (!(pm->btn_Cancel = PX_Object_PushButtonCreate(
              mp, pObject, 0, 0, 84, 28, "Cancel", PX_NULL)))
        return PX_FALSE;

    if (!(pm->edit_inputbox
          = PX_Object_EditCreate(mp, pObject, 0, 0, 256, 32, fontmodule)))
        return PX_FALSE;
    ...
    pm->PX_MESSAGEBOX_STAGE_1_HEIGHT
        = PX_OBJECT_MESSAGEBOX_DEFAULT_STAGE_1_HEIGHT;
    pm->PX_MESSAGEBOX_STAGE_2_HEIGHT
        = PX_OBJECT_MESSAGEBOX_DEFAULT_STAGE_2_HEIGHT;
    pm->fillbackgroundcolor = PX_OBJECT_UI_DEFAULT_BACKGROUNDCOLOR;
    PX_ObjectRegisterEvent(pm->edit_inputbox,
                           PX_OBJECT_EVENT_KEYDOWN,
                           PX_Object_MessageBox_EditOnEnter,
                           pObject);
    PX_ObjectRegisterEvent(pm->btn_Ok,
                           PX_OBJECT_EVENT_EXECUTE,
                           PX_Object_MessageBox_BtnYesClick,
                           pObject);
    PX_ObjectRegisterEvent(pm->btn_Cancel,
                           PX_OBJECT_EVENT_EXECUTE,
                           PX_Object_MessageBox_BtnNoClick,
                           pObject);

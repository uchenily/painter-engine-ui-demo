...
//////////////////////////////////////////////////////////////////////////
//
px_void PX_Object_SelectBarSliderOnChanged(PX_Object *pObject,
                                           PX_Object_Event e,
                                           px_void *ptr) {
    PX_Object_SelectBar *pSelectbar = (PX_Object_SelectBar *) ptr;
    pSelectbar->currentDisplayOffsetIndex
        = PX_Object_SliderBarGetValue(pObject);
}
...
static px_void PX_SelecrbarClearCurrentCursor(PX_Object_SelectBar *pSelectbar) {
    ...
    for (i = 0; i < pSelectbar->Items.size; i++) {
        PX_VECTORAT(PX_Object_SelectBar_Item, &pSelectbar->Items, i)->onCursor
            = PX_FALSE;
    }
}
static px_void
PX_SelectbarOnCursorMove(PX_Object *pObject, px_float x, px_float y) {
    ...
    if (pSelectbar->activating) {
        ...
        if (y < objy + objHeight || index < 0
            || index >= pSelectbar->maxDisplayCount
            || index >= pSelectbar->Items.size) {
            return;
        } else {
            PX_VECTORAT(PX_Object_SelectBar_Item,
                        &pSelectbar->Items,
                        index + pSelectbar->currentDisplayOffsetIndex)
                ->onCursor
                = PX_TRUE;
        }
    } else {
        if (PX_isPointInRect(PX_POINT(x, y, 0),
                             PX_RECT(objx, objy, objWidth, objHeight))) {
...
static px_void
PX_SelectbarOnCursorDown(PX_Object *pObject, px_float x, px_float y) {
    PX_Object_SelectBar *pSelectbar = PX_Object_GetSelectBar(pObject);
    px_float objx, objy, objWidth, objHeight;
    px_float inheritX, inheritY;
...
static px_void PX_SelectbarOnCursorEvent(PX_Object *pSelectBarObject,
                                         PX_Object_Event e,
                                         px_void *ptr) {
    switch (e.Event) {
        case PX_OBJECT_EVENT_CURSORMOVE:
        case PX_OBJECT_EVENT_CURSORDRAG: {
            PX_SelectbarOnCursorMove(pSelectBarObject,
                                     PX_Object_Event_GetCursorX(e),
                                     PX_Object_Event_GetCursorY(e));
        } break;
        case PX_OBJECT_EVENT_CURSORDOWN: {
            PX_SelectbarOnCursorDown(pSelectBarObject,
                                     PX_Object_Event_GetCursorX(e),
                                     PX_Object_Event_GetCursorY(e));
        } break;
        case PX_OBJECT_EVENT_CURSORWHEEL: {
            PX_Object_SelectBar *pDesc
                = PX_ObjectGetDesc(PX_Object_SelectBar, pSelectBarObject);
            PX_SelectbarOnCursorWheel(pSelectBarObject,
                                      PX_Object_Event_GetCursorZ(e));
            PX_Object_SelectBarSliderOnChanged(
                pDesc->sliderBar,
                PX_OBJECT_BUILD_EVENT(PX_OBJECT_EVENT_VALUECHANGED),
                pDesc);
        } break;
        default:
            break;
...
static px_void PX_SelectbarRender(px_surface *pRenderSurface,
                                  PX_Object *pObject,
                                  px_dword elpase) {
    PX_Object_SelectBar *pSelectBar = PX_Object_GetSelectBar(pObject);
    px_float objx, objy, objWidth, objHeight;
    ...
    if (pSelectBar->style == PX_OBJECT_SELECTBAR_STYLE_RECT) {
        ...
        if (pSelectBar->onCursor) {
            PX_GeoDrawSolidRoundRect(pRenderSurface,
                                     (px_int) objx,
                                     (px_int) objy,
                                     (px_int) (objx + objWidth - 1),
                                     (px_int) (objy + objHeight - 1),
        ...
        PX_GeoDrawRoundRect(pRenderSurface,
                            (px_int) objx,
                            (px_int) objy,
                            (px_int) (objx + objWidth - 1),
                            (px_int) (objy + objHeight - 1),
    ...
    // font
    do {
        if (pSelectBar->selectIndex >= 0
            && pSelectBar->selectIndex < pSelectBar->Items.size) {
            PX_Object_SelectBar_Item *pItem
                = PX_VECTORAT(PX_Object_SelectBar_Item,
                              &pSelectBar->Items,
                              pSelectBar->selectIndex);
            PX_FontModuleDrawText(pRenderSurface,
                                  pSelectBar->fontmodule,
                                  (px_int) (objx + objHeight / 2 + 1),
    ...
    if (pSelectBar->activating) {
        px_int i;
        px_int count = pSelectBar->maxDisplayCount > pSelectBar->Items.size
                           ? pSelectBar->Items.size
        ...
        for (i = 0; i < count; i++) {
            ...
            PX_FontModuleDrawText(pRenderSurface,
                                  ...
                                  (px_int) (objx + objHeight / 2 + 1),
                                  (px_int) (objy + objHeight
                                            + (i * pSelectBar->ItemHeight)
                                            + pSelectBar->ItemHeight / 2),
                                  PX_ALIGN_LEFTMID,
                                  pItem->Text,
                                  pSelectBar->fontColor);
        }
        ...
        if (pSelectBar->Items.size > pSelectBar->maxDisplayCount) {
            ...
            pSelectBar->sliderBar->Height
                = count * pSelectBar->ItemHeight * 1.0f;
            PX_Object_SliderBarSetBackgroundColor(pSelectBar->sliderBar,
                                                  pSelectBar->backgroundColor);
            PX_Object_SliderBarSetColor(pSelectBar->sliderBar,
                                        pSelectBar->borderColor);
            PX_Object_SliderBarSetRange(pSelectBar->sliderBar,
                                        0,
                                        pSelectBar->Items.size
                                            - pSelectBar->maxDisplayCount);
            PX_Object_SliderBarSetSliderButtonLength(
                pSelectBar->sliderBar,
                (px_int) (pSelectBar->sliderBar->Height
                          * pSelectBar->maxDisplayCount
                          / pSelectBar->Items.size));
        } else {
...
PX_Object *PX_Object_SelectBarCreate(px_memorypool *mp,
                                     PX_Object *Parent,
                                     px_int x,
                                     px_int y,
                                     px_int width,
    ...
    PX_Object_SelectBar *pSelectbar
        = (PX_Object_SelectBar *) MP_Malloc(mp, sizeof(PX_Object_SelectBar));
    ...
    PX_VectorInitialize(
        mp, &pSelectbar->Items, sizeof(PX_Object_SelectBar_Item), 16);

    pObject = PX_ObjectCreate(mp,
                              Parent,
                              (px_float) x,
                              (px_float) y,
                              0,
    ...
    PX_ObjectRegisterEvent(pSelectbar->sliderBar,
...
px_int PX_Object_SelectBarAddItem(PX_Object *pSelectBarObject,
                                  const px_char Text[]) {
...
px_int PX_Object_SelectBarGetItemIndexByText(PX_Object *pObject,
                                             const px_char Text[]) {
    PX_Object_SelectBar *pSelectBar = PX_Object_GetSelectBar(pObject);
    px_int i;
    for (i = 0; i < pSelectBar->Items.size; i++) {
        PX_Object_SelectBar_Item *pItem
            = PX_VECTORAT(PX_Object_SelectBar_Item, &pSelectBar->Items, i);
...
const px_char *PX_Object_SelectBarGetCurrentText(PX_Object *pObject) {
    PX_Object_SelectBar *pSelectBar = PX_Object_GetSelectBar(pObject);
    if (pSelectBar) {
        return PX_VECTORAT(PX_Object_SelectBar_Item,
                           &pSelectBar->Items,
                           pSelectBar->selectIndex)
            ->Text;
    }
...
const px_char *PX_Object_SelectBarGetItemText(PX_Object *pObject,
                                              px_int index) {
    PX_Object_SelectBar *pSelectBar = PX_Object_GetSelectBar(pObject);
    ...
    if (pSelectBar) {
        return PX_VECTORAT(PX_Object_SelectBar_Item, &pSelectBar->Items, index)
            ->Text;
    }
    return "";
}
...
px_void PX_Object_SelectBarSetStyle(PX_Object *pObject,
...
px_void PX_Object_SelectBarSetBackgroundColor(PX_Object *pObject,
                                              px_color color) {
    PX_Object_SelectBar *pSelectBar = PX_Object_GetSelectBar(pObject);
    if (pSelectBar) {
...
PX_Object *PX_Designer_SelectBarCreate(px_memorypool *mp,
                                       PX_Object *pparent,
                                       px_float x,
                                       px_float y,
                                       px_float width,
    ...
    return PX_Object_SelectBarCreate(
        mp, pparent, (px_int) x, (px_int) y, 96, 24, fm);
}

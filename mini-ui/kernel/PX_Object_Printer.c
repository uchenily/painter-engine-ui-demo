...
PX_Object *PX_Object_PrinterPrintText(PX_Object *pObject, const px_char *text) {
    if (pObject->Type == PX_OBJECT_TYPE_PRINTER) {
        ...
        PX_Object_Printer *pDesc = PX_ObjectGetDesc(PX_Object_Printer, pObject);
        PX_Object *pAutoObject
            = PX_Object_AutoTextCreate(pObject->mp,
                                       pDesc->Area,
                                       0,
                                       0,
                                       (px_int) pDesc->Area->Width,
                                       pDesc->fm);

        if (pAutoObject) {
            obj.Object = pAutoObject;
            obj.id = pDesc->id++;
            PX_Object_AutoTextSetTextColor(pAutoObject,
...
PX_Object *PX_Object_PrinterLastPrintText(PX_Object *pObject,
                                          const px_char *text) {
    if (pObject->Type == PX_OBJECT_TYPE_PRINTER) {
...
PX_Object *PX_Object_PrinterPrintImage(PX_Object *pObject,
                                       px_texture *pTexture) {
    if (pObject->Type == PX_OBJECT_TYPE_PRINTER) {
        PX_Object_PrinterLine obj;
        PX_Object_Printer *pDesc = PX_ObjectGetDesc(PX_Object_Printer, pObject);
        PX_Object *pImageObject;
        pImageObject = PX_Object_ImageCreate(pObject->mp,
                                             pDesc->Area,
                                             0,
                                             0,
                                             pTexture->width,
        ...
        PX_ObjectSetSize(pImageObject,
                         (px_float) pTexture->width,
                         (px_float) pTexture->height,
                         0);
...
px_int PX_Object_PrinterGetLastCreateId(PX_Object *pObject) {
    if (pObject->Type != PX_OBJECT_TYPE_PRINTER) {
        ...
        if (pDesc->pObjects.size) {
            return PX_VECTORAT(PX_Object_PrinterLine,
                               &pDesc->pObjects,
                               pDesc->pObjects.size - 1)
                ->id;
        }
        return -1;
    }
...
px_void PX_Object_PrinterOnEnter(PX_Object *pObject,
                                 PX_Object_Event e,
                                 px_void *user_ptr) {
    PX_Object_Printer *pDesc
        = PX_ObjectGetDesc(PX_Object_Printer, (PX_Object *) user_ptr);
    ...
    if (e.Event == PX_OBJECT_EVENT_KEYDOWN) {
        if (PX_Object_Event_GetKeyDown(e) == '\r') {
            PX_ObjectExecuteEvent((PX_Object *) user_ptr,
...
px_void PX_Object_PrinterOnMouseDown(PX_Object *pObject,
                                     PX_Object_Event e,
                                     px_void *user_ptr) {
    PX_Object_Printer *pDesc = PX_ObjectGetDesc(PX_Object_Printer, pObject);
...
px_void PX_Object_PrinterRender(px_surface *psurface,
                                PX_Object *pObject,
                                px_dword elapsed) {
    PX_Object_Printer *pDesc = PX_ObjectGetDesc(PX_Object_Printer, pObject);
    if (pDesc->show) {
...
px_void PX_Object_PrinterSetBackgroundColor(PX_Object *pObject,
                                            px_color color) {
    PX_Object_Printer *pDesc = PX_ObjectGetDesc(PX_Object_Printer, pObject);
    PX_Object_ScrollAreaSetBackgroundColor(pDesc->Area, color);
}

PX_Object *PX_Object_PrinterCreate(px_memorypool *mp,
                                   PX_Object *Parent,
                                   px_int x,
                                   px_int y,
                                   px_int width,
    ...
    pObject = PX_ObjectCreateEx(mp,
                                ...
                                0,
                                PX_Object_PrinterRender,
                                PX_Object_PrinterFree,
                                &Desc,
                                sizeof(Desc));
    ...
    pDesc->column = 0;
    pDesc->fm = fm;
    if (!(pDesc->Area = PX_Object_ScrollAreaCreate(
              mp, pObject, 0, 0, (px_int) width, (px_int) height)))
        return PX_NULL;

    PX_ObjectRegisterEvent(pDesc->Area,
                           PX_OBJECT_EVENT_KEYDOWN,
                           PX_Object_PrinterOnEnter,
                           pObject);
    PX_ObjectRegisterEvent(pDesc->Area,
                           PX_OBJECT_EVENT_CURSORDOWN,
                           PX_Object_PrinterOnMouseDown,
                           pObject);
    PX_Object_ScrollAreaSetBorder(pDesc->Area, PX_NULL);

    if (!(pDesc->Input = PX_Object_EditCreate(
              mp, (pDesc->Area), 0, 0, (px_int) width, fmheight, fm)))
        return PX_NULL;
    ...
    // PX_Object_EditSetLimit(pDesc->Input,
    // "zxcvbnm,./asdfghjkl;'qwertyyuiop[]\\`1234567890-=ZXCVBNM<>?ASDFGHJKL:\"QWERTYUIOP{}|~!@#$%^&*()_+");
    PX_Object_ScrollAreaSetBkColor(pDesc->Area, PX_COLOR(255, 0, 0, 0));
    pDesc->id = 1;

    PX_VectorInitialize(mp,
                        &pDesc->pObjects,
                        sizeof(PX_Object_PrinterLine),
...
// Printer
/////////////////////////////////////////////////////////
PX_Object *PX_Designer_PrinterCreate(px_memorypool *mp,
                                     PX_Object *pparent,
                                     px_float x,
                                     px_float y,
                                     px_float width,
    ...
    return PX_Object_PrinterCreate(mp,
                                   pparent,
                                   (px_int) x,
                                   (px_int) y,
                                   (px_int) width,

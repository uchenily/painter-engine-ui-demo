...
px_void PX_Object_ProtractorRender(px_surface *rendersurface,
                                   PX_Object *pObject,
                                   px_dword elapsed) {
    ...
    PX_GeoDrawCircle(rendersurface,
                     (px_int) objx,
                     (px_int) objy,
                     (px_int) pProtractor->radius,
                     1,
                     pProtractor->color);
    PX_GeoDrawSolidCircle(
        rendersurface, (px_int) objx, (px_int) objy, 3, pProtractor->color);
    ...
    switch (pProtractor->state) {
        case PX_OBJECT_PROTRACTOR_STATE_STANDBY: {
            PX_GeoDrawLine(rendersurface,
                           ...
                           (px_int) objy,
                           (px_int) (objx
                                     + PX_cos_angle(pProtractor->startAngle)
                                           * pProtractor->radius),
                           (px_int) (objy
                                     + PX_sin_angle(pProtractor->startAngle)
                                           * pProtractor->radius),
                           1,
                           pProtractor->color);
        ...
        case PX_OBJECT_PROTRACTOR_STATE_DONE: {
            ...
            PX_GeoDrawSector(rendersurface,
                             ...
                             1,
                             acolor,
                             (px_int) pProtractor->startAngle,
                             (px_int) pProtractor->endAngle);
            ...
            PX_GeoDrawLine(rendersurface,
                           ...
                           (px_int) objy,
                           (px_int) (objx
                                     + PX_cos_angle(pProtractor->endAngle)
                                           * pProtractor->radius),
                           (px_int) (objy
                                     + PX_sin_angle(pProtractor->endAngle)
                                           * pProtractor->radius),
                           1,
                           pProtractor->color);
            ...
            PX_itoa((px_int) PX_Object_ProtractorGetAngle(pObject),
                    degree,
                    sizeof(degree),
                    10);
            PX_FontModuleDrawText(rendersurface,
                                  ...
                                  PX_ALIGN_MIDBOTTOM,
                                  degree,
                                  pProtractor->fontColor);
        } break;
    }
}

px_void PX_Object_ProtractorOnCursorDown(PX_Object *pObject,
                                         PX_Object_Event e,
                                         px_void *ptr) {
    px_float objx, objy, objWidth, objHeight;
...
px_void PX_Object_ProtractorOnCursorMove(PX_Object *pObject,
                                         PX_Object_Event e,
                                         px_void *ptr) {
    px_float objx, objy, objWidth, objHeight;
    px_float inheritX, inheritY, x, y;
    ...
    switch (pProtractor->state) {
        case PX_OBJECT_PROTRACTOR_STATE_STANDBY: {
            pProtractor->startAngle = (px_float) PX_RadianToAngle(
...
px_void PX_Object_ProtractorOnCursorRDown(PX_Object *pObject,
                                          PX_Object_Event e,
                                          px_void *ptr) {
    PX_Object_Protractor *pProtractor = PX_Object_GetProtractor(pObject);
...
PX_Object *PX_Object_ProtractorCreate(
    px_memorypool *mp, PX_Object *Parent, px_int x, px_int y, px_int radius) {
    PX_Object *pObject;
    ...
    pObject = PX_ObjectCreateEx(mp,
                                ...
                                0,
                                PX_Object_ProtractorRender,
                                0,
                                &protractor,
                                sizeof(protractor));
    PX_ObjectRegisterEvent(pObject,
                           PX_OBJECT_EVENT_CURSORDOWN,
                           PX_Object_ProtractorOnCursorDown,
                           PX_NULL);
    PX_ObjectRegisterEvent(pObject,
                           PX_OBJECT_EVENT_CURSORMOVE,
                           PX_Object_ProtractorOnCursorMove,
                           PX_NULL);
    PX_ObjectRegisterEvent(pObject,
                           PX_OBJECT_EVENT_CURSORRDOWN,
                           PX_Object_ProtractorOnCursorRDown,
                           PX_NULL);
    return pObject;
}

px_float PX_Object_ProtractorGetAngle(PX_Object *Object) {
    return PX_Object_GetProtractor(Object)->endAngle
           - PX_Object_GetProtractor(Object)->startAngle;
}

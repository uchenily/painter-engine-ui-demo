...
px_void PX_Object_CanvasOnRender(px_surface *psurface,
                                 PX_Object *pObject,
                                 px_dword elapsed) {
    px_float size;
    px_float x, y, w, h;
    ...
    PX_GeoDrawBorder(psurface,
                     ...
                     PX_COLOR_BLACK);
    PX_SurfaceSetLimit(
        psurface, (px_int) x, (px_int) y, (px_int) (x + w), (px_int) (h + y));
    PX_GeoDrawCircle(psurface,
                     (px_int) (x + pdesc->reg_x),
                     (px_int) (y + pdesc->reg_y),
                     (px_int) (size * pdesc->scale),
                     1,
                     PX_COLOR(255, 0, 0, 0));
...
px_surface *PX_Object_CanvasGetCurrentEditingSurface(PX_Object *pObject) {
    ...
    PX_Object_LayerBox *pLayerBox
        = PX_ObjectGetDesc(PX_Object_LayerBox, pdesc->pLayerBoxObject);
    return PX_Object_LayerBoxGetEditingSurface(pdesc->pLayerBoxObject);
}

px_void PX_Object_CanvasRenderLayerToSurface(PX_Object_LayerBox *pLayerBox,
                                             px_int index,
    ...
    if (pLayerBox->layers[index].attribute == PX_OBJECT_LAYER_ATTRIBUTE_CLIP) {
        ...
        for (j = index + 1; j <= PX_COUNTOF(pLayerBox->layers); j++) {
            // find mask
            if (pLayerBox->layers[j].attribute
                == PX_OBJECT_LAYER_ATTRIBUTE_NORMAL) {
                PX_TextureRenderMask(prendertarget,
                                     &pLayerBox->layers[j].surface_layer,
                                     &pLayerBox->layers[index].surface_layer,
                                     0,
                                     0,
                                     PX_ALIGN_LEFTTOP,
                                     0);
        ...
        PX_TextureRender(prendertarget,
                         &pLayerBox->layers[index].surface_layer,
                         0,
                         0,
                         PX_ALIGN_LEFTTOP,
...
px_void PX_Object_CanvasRenderBackFrontLayersToCache(PX_Object *pObject) {
    ...
    PX_SurfaceClearAll(&pdesc->cache_backward_surface,
...
px_void PX_Object_CanvasRenderEditingLayersToCache(PX_Object *pObject) {
    ...
    for (activatingIndex = 0; activatingIndex < PX_COUNTOF(pLayerBox->layers);
         activatingIndex++) {
        if (pLayerBox->layers[activatingIndex].activating == PX_FALSE) {
            break;
        }
        if (pLayerBox->layers[activatingIndex].editing == PX_TRUE) {
            PX_Object_CanvasRenderLayerToSurface(
                pLayerBox, activatingIndex, &pdesc->cache_editing_surface);
            break;
        }
    }
}
px_void PX_Object_CanvasRenderCombineCache(PX_Object *pObject) {
    PX_Object_Canvas *pdesc = PX_ObjectGetDesc(PX_Object_Canvas, pObject);
    PX_TextureCover(&pdesc->cache_surface,
                    &pdesc->cache_backward_surface,
                    0,
                    0,
                    PX_ALIGN_LEFTTOP);
    PX_TextureRender(&pdesc->cache_surface,
                     &pdesc->cache_editing_surface,
                     0,
                     0,
                     PX_ALIGN_LEFTTOP,
                     0);
    PX_TextureRender(&pdesc->cache_surface,
                     &pdesc->cache_frontward_surface,
                     0,
                     0,
                     PX_ALIGN_LEFTTOP,
                     0);
...
px_point2D PX_Object_CanvasCanvasPostionToViewPosition(PX_Object *pObject,
    ...
    p.x = (p.x / sw * 2) * pdesc->view_surface.width / 2
          + pdesc->view_surface.width / 2;
    p.y = (p.y / sh * 2) * pdesc->view_surface.height / 2
          + pdesc->view_surface.height / 2;
    return p;
}

px_point2D PX_Object_CanvasViewPositionToCanvasPostion(PX_Object *pObject,
                                                       px_point2D ViewPos) {
    PX_Object_Canvas *pdesc = PX_ObjectGetDesc(PX_Object_Canvas, pObject);
    px_float sw, sh;
    px_point2D p;
...
px_void PX_Object_CanvasUpdateSliderBar(PX_Object *pObject) {
    ...
    PX_Object_SliderBarSetRange(
        pdesc->pHSliderBar, (px_int) range_min_x, (px_int) range_max_x);
    PX_Object_SliderBarSetSliderButtonLength(pdesc->pHSliderBar,
                                             (px_int) len_x);
    PX_Object_SliderBarSetValue(pdesc->pHSliderBar, (px_int) valuex);

    PX_Object_SliderBarSetRange(
        pdesc->pVSliderBar, (px_int) range_min_y, (px_int) range_max_y);
    PX_Object_SliderBarSetSliderButtonLength(pdesc->pVSliderBar,
                                             (px_int) len_y);
...
px_void PX_Object_CanvasRenderToLayerPreview(PX_Object *pObject) {
    ...
    PX_Object_Canvas *pdesc = PX_ObjectGetDesc(PX_Object_Canvas, pObject);
    px_texture *ptexture
        = PX_Object_LayerBoxGetPreviewSurface(pdesc->pLayerBoxObject);

    if (pdesc->cache_editing_surface.width
        > pdesc->cache_editing_surface.height) {
    ...
    if (!PX_TextureCreateScale(
            pdesc->mp, &pdesc->cache_surface, neww, newh, &mintex)) {
        return;
    }
    PX_SurfaceClearAll(ptexture, PX_COLOR(0, 0, 0, 0));
    PX_TextureRender(ptexture,
                     &mintex,
                     ptexture->width / 2,
                     ptexture->height / 2,
                     PX_ALIGN_CENTER,
    ...
    vcw = pdesc->view_surface.width / pdesc->scale
          / pdesc->cache_editing_surface.width * neww;
    vch = pdesc->view_surface.height * 1.0f
          / pdesc->cache_editing_surface.height / pdesc->scale * newh;

    PX_GeoDrawBorder(ptexture,
                     (px_int) (ltoftx + vcx - vcw / 2),
                     (px_int) (ltofty + vcy - vch / 2),
                     (px_int) (ltoftx + vcx + vcw / 2),
                     (px_int) (ltofty + vcy + vch / 2),
                     1,
                     PX_COLOR_BLACK);
...
px_void PX_Object_CanvasOnCursorWheeling(PX_Object *pObject,
                                         PX_Object_Event e,
                                         px_void *ptr) {
    PX_Object_Canvas *pdesc = PX_ObjectGetDesc(PX_Object_Canvas, pObject);
    px_float cx, cy, cz;
    ...
    if (PX_ObjectIsCursorInRegion(pObject, e)) {
        ...
        if (cz > 0) {
            ...
            p2d = PX_Object_CanvasViewPositionToCanvasPostion(
                pObject, PX_POINT2D(new_v_x, new_v_y));
...
px_void PX_Object_CanvasOnHSliderBarChanged(PX_Object *pObject,
                                            PX_Object_Event e,
                                            px_void *ptr) {
    PX_Object_Canvas *pdesc
        = PX_ObjectGetDesc(PX_Object_Canvas, (PX_Object *) ptr);
...
px_void PX_Object_CanvasOnVSliderBarChanged(PX_Object *pObject,
                                            PX_Object_Event e,
                                            px_void *ptr) {
    PX_Object_Canvas *pdesc
        = PX_ObjectGetDesc(PX_Object_Canvas, (PX_Object *) ptr);
...
px_void PX_Object_CanvasEnterEdit(PX_Object *pObject) {
    ...
    px_color clr;
    PX_Object_Canvas *pdesc = PX_ObjectGetDesc(PX_Object_Canvas, pObject);
    PX_Object_PainterBox *pPainterBox
        = PX_ObjectGetDesc(PX_Object_PainterBox, pdesc->pPainterBoxObject);
    ...
    PX_CanvasVMOnBegin(
        &pdesc->CanvasVM, opcode, size * pdesc->scale, clr, smooth, 0.35f);
}
...
px_void PX_Object_CanvasExitEdit(PX_Object *pObject) {
    ...
    switch (
        pdesc->CanvasVM.tools[pdesc->CanvasVM.reg_current_tool_index].opcode) {
...
px_void PX_Object_CanvasOnCursorDown(PX_Object *pObject,
                                     PX_Object_Event e,
                                     px_void *ptr) {
    px_float x, y, w, h;
    PX_Object_Canvas *pdesc = PX_ObjectGetDesc(PX_Object_Canvas, pObject);
...
px_void PX_Object_CanvasOnCursorMove(PX_Object *pObject,
                                     PX_Object_Event e,
                                     px_void *ptr) {
    px_float x, y, w, h;
    PX_Object_Canvas *pdesc = PX_ObjectGetDesc(PX_Object_Canvas, pObject);
...
px_void PX_Object_CanvasOnCursorDrag(PX_Object *pObject,
                                     PX_Object_Event e,
                                     px_void *ptr) {
    px_float x, y, w, h;
    PX_Object_Canvas *pdesc = PX_ObjectGetDesc(PX_Object_Canvas, pObject);
    ...
    if (pdesc->mode == PX_OBJECT_CANVAS_MODE_EDIT) {
        // editing
        px_point2D canvaspos;
        canvaspos = PX_Object_CanvasViewPositionToCanvasPostion(
            pObject, PX_POINT2D(pdesc->reg_x, pdesc->reg_y));
        PX_CanvasVMOnMove(
            &pdesc->CanvasVM, pdesc->reg_x, pdesc->reg_y, pdesc->reg_z);
...
px_void PX_Object_CanvasOnCursorUp(PX_Object *pObject,
                                   PX_Object_Event e,
                                   px_void *ptr) {
    px_float x, y, w, h;
    PX_Object_Canvas *pdesc = PX_ObjectGetDesc(PX_Object_Canvas, pObject);
...
PX_Object *PX_Object_CanvasCreate(px_memorypool *mp,
                                  PX_Object *Parent,
                                  px_int x,
                                  px_int y,
                                  px_int width,
                                  px_int height,
                                  PX_Object *pLayerBoxObject,
                                  PX_Object *pPainterBoxObject) {
    ...
    pObject = PX_ObjectCreateEx(mp,
                                ...
                                0,
                                PX_Object_CanvasOnRender,
                                PX_Object_CanvasOnFree,
                                &desc,
                                sizeof(desc));
    ...
    if (!PX_TextureCreate(
            mp, &pdesc->cache_surface, pLayerBox->width, pLayerBox->height)) {
        PX_ASSERT();
        return PX_NULL;
    }
    ...
    if (pdesc->cache_editing_surface.width * 1.25f
        > pdesc->view_surface.width) {
        pdesc->min_scale = pdesc->view_surface.width * 1.0f
                           / (pdesc->cache_editing_surface.width * 1.25f);

        if (pdesc->min_scale
            > pdesc->view_surface.height * 1.0f
                  / (pdesc->cache_editing_surface.height * 1.25f)) {
            pdesc->min_scale = pdesc->view_surface.height * 1.0f
                               / (pdesc->cache_editing_surface.height * 1.25f);
        }
    }
    pdesc->scale = pdesc->min_scale;
    ...
    PX_ObjectRegisterEvent(pObject,
                           ...
                           PX_Object_CanvasOnCursorWheeling,
                           0);

    PX_ObjectRegisterEvent(
        pObject, PX_OBJECT_EVENT_CURSORDOWN, PX_Object_CanvasOnCursorDown, 0);
    PX_ObjectRegisterEvent(
        pObject, PX_OBJECT_EVENT_CURSORMOVE, PX_Object_CanvasOnCursorMove, 0);
    PX_ObjectRegisterEvent(
        pObject, PX_OBJECT_EVENT_CURSORDRAG, PX_Object_CanvasOnCursorDrag, 0);
    PX_ObjectRegisterEvent(
        pObject, PX_OBJECT_EVENT_CURSORMUP, PX_Object_CanvasOnCursorUp, 0);

    PX_ObjectRegisterEvent(pdesc->pHSliderBar,
                           PX_OBJECT_EVENT_VALUECHANGED,
                           PX_Object_CanvasOnHSliderBarChanged,
                           pObject);
    PX_ObjectRegisterEvent(pdesc->pVSliderBar,
                           PX_OBJECT_EVENT_VALUECHANGED,
                           PX_Object_CanvasOnVSliderBarChanged,
                           pObject);
    return pObject;
}

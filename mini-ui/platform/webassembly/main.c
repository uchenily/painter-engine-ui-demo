...
EMSCRIPTEN_KEEPALIVE int load_file(uint8_t *buffer, size_t size) {
    if (load_file_info) {
        if (size < load_file_info->size) {
            ...
            load_file_info->func_callback(
                load_file_info->buffer, size, load_file_info->ptr);
        } else {
            load_file_info->func_callback(
                load_file_info->buffer, 0, load_file_info->ptr);
        }
        free(load_file_info);
...
void PX_RequestDownloadFile(const char *filename,
                            const void *data,
                            size_t size) {
    EM_ASM(
        {
...
void PX_RequestData(const char url[],
                    void *buffer,
                    int size,
                    void *ptr,
                    void (*func_callback)(void *buffer, int size, void *ptr)) {
    PX_RequestData_Info *info
        = (PX_RequestData_Info *) malloc(sizeof(PX_RequestData_Info));
    if (!info) {
    ...
    if (PX_strequ2(url, "open")) {
        EM_ASM(var file_selector = document.createElement('input');
        ...
        emscripten_async_wget_data(
            url, info, px_em_async_wget_onload_func, px_em_arg_callback_func);
    }
}
...
void mainloop(void *ptr) {
    ...
    PX_memcpy(screen->pixels,
              App.runtime.RenderSurface.surfaceBuffer,
              App.runtime.surface_width * App.runtime.surface_height * 4);
    if (SDL_MUSTLOCK(screen))
    ...
    while (SDL_PollEvent(&event)) {
        if (SDL_MOUSEBUTTONDOWN == event.type) {
            ...
            if ((str[0] >= 0x20 && str[0] <= 0x7E) || str[0] == 0x08
                || str[0] == 0x0d || str[0] == 0x0a) {
...
extern "C"
    ...
    printf("SDL Create surface %d %d\n",
           App.runtime.surface_width,
           App.runtime.surface_height);
    screen = SDL_SetVideoMode(App.runtime.surface_width,
                              App.runtime.surface_height,
                              32,
                              SDL_SWSURFACE);

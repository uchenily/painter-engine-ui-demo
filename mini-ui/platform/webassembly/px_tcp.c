...
int PX_TCPSend(PX_TCP *tcp, void *buffer, int size) {
    ...
    switch (tcp->type) {
        case PX_TCP_IP_TYPE_IPV4: {
            while (size > 0) {
                if ((length = send(tcp->socket, (const char *) buffer, size, 0))
                    == -1) {
                    return 0;
                }
                size -= length;
...
int PX_TCPSocketReceived(unsigned int socket,
                         void *buffer,
                         int buffersize,
                         int timeout) {
    int SockAddrSize = sizeof(struct sockaddr_in);
...
int PX_TCPAccept(PX_TCP *tcp, unsigned int *socket, PX_TCP_ADDR *fromAddr) {
    ...
    int len;
    *socket = accept(
        tcp->socket, (struct sockaddr *) &sockaddr, (socklen_t *) &len);
    return *socket != -1;
}
...
int PX_TCPListen(PX_TCP *tcp, unsigned short listen_Port) {
    ...
    if (bind(tcp->socket, (struct sockaddr *) &addrin, sizeof(struct sockaddr))
        == -1) {
        close(tcp->socket);
        return 0;
    }

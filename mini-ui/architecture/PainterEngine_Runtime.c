...
px_bool PX_RuntimeInitialize(PX_Runtime *pe,
                             px_int surface_width,
                             px_int surface_height,
                             px_int window_width,
                             px_int window_height,
                             px_void *runtime_memoryPtr,
                             px_uint size,
                             px_uint ui_size,
                             px_uint resource_size,
                             px_uint game_size) {
    ...
    pe->mp_resources
        = MP_Create(MP_Malloc(&pe->mp, resource_size), resource_size);
    ...
    // resources
    if (!PX_ResourceLibraryInitialize(&pe->mp_resources, &pe->ResourceLibrary))
        return PX_FALSE;
    // surface
    if (pe->surface_width == 0 || pe->surface_height == 0) {
        ...
        if (!PX_SurfaceCreate(
                &pe->mp, surface_width, surface_height, &pe->RenderSurface)) {
            return PX_FALSE;
        }
        PX_SurfaceClear(&pe->RenderSurface,
                        0,
                        0,
                        pe->RenderSurface.width - 1,
                        pe->RenderSurface.height - 1,
...
px_bool PX_RuntimeResize(PX_Runtime *pe,
                         px_int surface_width,
                         px_int surface_height,
                         px_int window_width,
                         px_int window_height) {
    if (PX_SurfaceMemorySize(surface_width, surface_height) + 64
        > (px_int) pe->mp.FreeSize) {
...
px_memorypool PX_RuntimeCreateCalcMemoryPool(PX_Runtime *pe) {
    return MP_Create(MP_Malloc(&pe->mp, PE_MEMORY_CALC_SIZE),
                     PE_MEMORY_CALC_SIZE);
}
...
px_void PX_RuntimeRenderClear(PX_Runtime *runtime, px_color color) {
    PX_SurfaceClear(&runtime->RenderSurface,
                    0,
                    0,
                    runtime->RenderSurface.width - 1,
                    runtime->RenderSurface.height - 1,
                    color);
}

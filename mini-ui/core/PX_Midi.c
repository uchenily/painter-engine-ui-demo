#include "PX_Midi.h"
px_bool PX_MidiInitialize(PX_Midi *midi,
                          px_memorypool *mp,
...
px_bool PX_MidiLoad(PX_Midi *pmidi, const px_byte data[], px_int datasize) {
    ...
    if (!PX_VectorInitialize(pmidi->mp,
                             &pmidi->track,
                             sizeof(PX_Midi_Track),
                             pMThdheader->nnnn >> 8)) {
        return PX_FALSE;
    }
    ...
    for (i = 0; i < pmidi->track.size; i++) {
        ...
        while (PX_TRUE) {
            ...
            } else if (PX_MidiGetNextType(pmidi, i)
...
px_void PX_MidiPlay(PX_Midi *pmidi) { pmidi->state = PX_Midi_State_Play; }

px_void PX_MidiPause(PX_Midi *pmidi) { pmidi->state = PX_Midi_State_Stop; }
...
px_void PX_MidiNoteToString(px_byte note, px_char out[8]) {
    const px_char *note_content[]
        = {"C", "#C", "D", "#D", "E", "F", "#F", "G", "#G", "A", "#A", "B"};
    PX_sprintf2(out,
                8,
                "%1%2",
                PX_STRINGFORMAT_STRING(note_content[note % 12]),
...
px_int PX_MidiGetTrackCount(PX_Midi *pmidi) { return pmidi->track.size; }

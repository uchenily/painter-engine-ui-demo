...
px_bool PX_TRawVerify(px_void *data, px_int size) {
    ...
    if (pHeader->Magic[0] != 'T' || pHeader->Magic[1] != 'R'
        || pHeader->Magic[2] != 'A' || pHeader->Magic[3] != 'W') {
        return PX_FALSE;
    }

    if (size - sizeof(PX_TRaw_Header)
        < pHeader->Height * pHeader->Width * sizeof(px_color)) {
        return PX_FALSE;
    }
...
px_void PX_TRawRender(px_surface *psurface, px_void *data, px_int x, px_int y) {
    ...
    for (i = 0; i < Height; i++) {
        for (j = 0; j < Width; j++) {
            PX_SurfaceDrawPixel(
                psurface, j + x, i + y, pdataColor[Width * i + j]);
        }
    }
...
px_bool PX_TRawBuild(px_surface *psurface, px_byte *TRawBuffer, px_int *size) {
    ...
    _outsize = psurface->height * psurface->width * sizeof(px_color)
               + sizeof(header);
    *size = _outsize;
    header.Magic[0] = 'T';
    header.Magic[1] = 'R';
...
px_uint PX_TRawGetSize(PX_TRaw_Header *pHeader) {
    ...
    return pHeader->Height * pHeader->Width * sizeof(px_color)
           + sizeof(PX_TRaw_Header);
}

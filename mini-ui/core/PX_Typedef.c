...
px_double64 PX_copysign(px_double64 x, px_double64 y) {
    *(1 + (px_int32 *) &x) = (*(1 + (px_int32 *) &x) & 0x7fffffff)
                             | (*(1 + (px_int32 *) &y) & 0x80000000);
    return x;
}

px_double64 PX_scalbn(px_double64 x, px_double64 n) {
    px_int32 k, hx, lx;
    static px_double64 two54
...
px_double64 __px_pow_dd(px_double64 x, px_double64 y) {
    ...
    *(px_int32 *) &t = 0;
    ...
    j += (n << 20);
    if ((j >> 20) <= 0)
        z = PX_scalbn(z, n); /* subnormal output */
    else
        *(1 + (px_int32 *) &z) += (n << 20);
    return s * z;
}

px_double PX_exp(px_double x) { return __px_pow_dd(PX_e, x); }
...
px_double PX_ReLU(px_double x) { return x <= 0 ? 0 : x; }
...
px_double PX_cosd(px_double radian) { return PX_sind((PX_PI / 2 - radian)); }

px_float PX_sin_radian(px_float radian) { return (px_float) PX_sind(radian); }
...
px_double PX_asin(px_double x) { return PX_atan2(x, PX_sqrtd(1.0 - x * x)); }

px_double PX_acos(px_double x) { return PX_atan2(PX_sqrtd(1.0 - x * x), x); }
...
px_int PX_sprintf8(px_char *_out_str,
                   px_int str_size,
                   const px_char fmt[],
                   px_stringformat _1,
                   px_stringformat _2,
...
px_int PX_sprintf7(px_char *str,
                   px_int str_size,
                   const px_char fmt[],
                   px_stringformat _1,
                   px_stringformat _2,
                   ...
                   px_stringformat _7) {
    return PX_sprintf8(
        str, str_size, fmt, _1, _2, _3, _4, _5, _6, _7, PX_STRINGFORMAT_INT(0));
}

px_int PX_sprintf6(px_char *str,
                   px_int str_size,
                   const px_char fmt[],
                   px_stringformat _1,
                   px_stringformat _2,
...
px_int PX_sprintf5(px_char *str,
                   px_int str_size,
                   const px_char fmt[],
                   px_stringformat _1,
                   px_stringformat _2,
...
px_int PX_sprintf4(px_char *str,
                   px_int str_size,
                   const px_char fmt[],
                   px_stringformat _1,
                   px_stringformat _2,
...
px_int PX_sprintf3(px_char *str,
                   px_int str_size,
                   const px_char fmt[],
                   px_stringformat _1,
                   px_stringformat _2,
...
px_int PX_sprintf2(px_char *str,
                   px_int str_size,
                   const px_char fmt[],
                   px_stringformat _1,
                   px_stringformat _2) {
...
px_int PX_sprintf1(px_char *str,
                   px_int str_size,
                   const px_char fmt[],
                   px_stringformat _1) {
    return PX_sprintf8(str,
...
px_matrix PX_MatrixMultiply(px_matrix Mat1, px_matrix Mat2) {
    px_matrix ptmat;
    ptmat.m[0][0]
        = Mat1.m[0][0] * Mat2.m[0][0] /**/ + Mat1.m[0][1] * Mat2.m[1][0] /**/
          + Mat1.m[0][2] * Mat2.m[2][0] /**/ + Mat1.m[0][3] * Mat2.m[3][0];
    ptmat.m[0][1]
        = Mat1.m[0][0] * Mat2.m[0][1] /**/ + Mat1.m[0][1] * Mat2.m[1][1] /**/
          + Mat1.m[0][2] * Mat2.m[2][1] /**/ + Mat1.m[0][3] * Mat2.m[3][1];
    ptmat.m[0][2]
        = Mat1.m[0][0] * Mat2.m[0][2] /**/ + Mat1.m[0][1] * Mat2.m[1][2] /**/
          + Mat1.m[0][2] * Mat2.m[2][2] /**/ + Mat1.m[0][3] * Mat2.m[3][2];
    ptmat.m[0][3]
        = Mat1.m[0][0] * Mat2.m[0][3] /**/ + Mat1.m[0][1] * Mat2.m[1][3] /**/
          + Mat1.m[0][2] * Mat2.m[2][3] /**/ + Mat1.m[0][3] * Mat2.m[3][3];

    ptmat.m[1][0]
        = Mat1.m[1][0] * Mat2.m[0][0] /**/ + Mat1.m[1][1] * Mat2.m[1][0] /**/
          + Mat1.m[1][2] * Mat2.m[2][0] /**/ + Mat1.m[1][3] * Mat2.m[3][0];
    ptmat.m[1][1]
        = Mat1.m[1][0] * Mat2.m[0][1] /**/ + Mat1.m[1][1] * Mat2.m[1][1] /**/
          + Mat1.m[1][2] * Mat2.m[2][1] /**/ + Mat1.m[1][3] * Mat2.m[3][1];
    ptmat.m[1][2]
        = Mat1.m[1][0] * Mat2.m[0][2] /**/ + Mat1.m[1][1] * Mat2.m[1][2] /**/
          + Mat1.m[1][2] * Mat2.m[2][2] /**/ + Mat1.m[1][3] * Mat2.m[3][2];
    ptmat.m[1][3]
        = Mat1.m[1][0] * Mat2.m[0][3] /**/ + Mat1.m[1][1] * Mat2.m[1][3] /**/
          + Mat1.m[1][2] * Mat2.m[2][3] /**/ + Mat1.m[1][3] * Mat2.m[3][3];

    ptmat.m[2][0]
        = Mat1.m[2][0] * Mat2.m[0][0] /**/ + Mat1.m[2][1] * Mat2.m[1][0] /**/
          + Mat1.m[2][2] * Mat2.m[2][0] /**/ + Mat1.m[2][3] * Mat2.m[3][0];
    ptmat.m[2][1]
        = Mat1.m[2][0] * Mat2.m[0][1] /**/ + Mat1.m[2][1] * Mat2.m[1][1] /**/
          + Mat1.m[2][2] * Mat2.m[2][1] /**/ + Mat1.m[2][3] * Mat2.m[3][1];
    ptmat.m[2][2]
        = Mat1.m[2][0] * Mat2.m[0][2] /**/ + Mat1.m[2][1] * Mat2.m[1][2] /**/
          + Mat1.m[2][2] * Mat2.m[2][2] /**/ + Mat1.m[2][3] * Mat2.m[3][2];
    ptmat.m[2][3]
        = Mat1.m[2][0] * Mat2.m[0][3] /**/ + Mat1.m[2][1] * Mat2.m[1][3] /**/
          + Mat1.m[2][2] * Mat2.m[2][3] /**/ + Mat1.m[2][3] * Mat2.m[3][3];

    ptmat.m[3][0]
        = Mat1.m[3][0] * Mat2.m[0][0] /**/ + Mat1.m[3][1] * Mat2.m[1][0] /**/
          + Mat1.m[3][2] * Mat2.m[2][0] /**/ + Mat1.m[3][3] * Mat2.m[3][0];
    ptmat.m[3][1]
        = Mat1.m[3][0] * Mat2.m[0][1] /**/ + Mat1.m[3][1] * Mat2.m[1][1] /**/
          + Mat1.m[3][2] * Mat2.m[2][1] /**/ + Mat1.m[3][3] * Mat2.m[3][1];
    ptmat.m[3][2]
        = Mat1.m[3][0] * Mat2.m[0][2] /**/ + Mat1.m[3][1] * Mat2.m[1][2] /**/
          + Mat1.m[3][2] * Mat2.m[2][2] /**/ + Mat1.m[3][3] * Mat2.m[3][2];
    ptmat.m[3][3]
        = Mat1.m[3][0] * Mat2.m[0][3] /**/ + Mat1.m[3][1] * Mat2.m[1][3] /**/
          + Mat1.m[3][2] * Mat2.m[2][3] /**/ + Mat1.m[3][3] * Mat2.m[3][3];
    return ptmat;
}

px_matrix PX_MatrixAdd(px_matrix Mat1, px_matrix Mat2) {
    px_matrix ptmat;
    ptmat._11 = Mat1._11 + Mat2._11;
    ptmat._12 = Mat1._12 + Mat2._12;
    ptmat._13 = Mat1._13 + Mat2._13;
    ptmat._14 = Mat1._14 + Mat2._14;
    ptmat._21 = Mat1._21 + Mat2._21;
    ptmat._22 = Mat1._22 + Mat2._22;
    ptmat._23 = Mat1._23 + Mat2._23;
    ptmat._24 = Mat1._24 + Mat2._24;
    ptmat._31 = Mat1._31 + Mat2._31;
    ptmat._32 = Mat1._32 + Mat2._32;
    ptmat._33 = Mat1._33 + Mat2._33;
    ptmat._34 = Mat1._34 + Mat2._34;
    ptmat._41 = Mat1._41 + Mat2._41;
    ptmat._42 = Mat1._42 + Mat2._42;
    ptmat._43 = Mat1._43 + Mat2._43;
...
px_void PX_MatrixRotateVector(px_matrix *mat, px_point v_base, px_point v) {
    ...
    px_float cosv
        = PX_PointDot(v_base, v) / PX_PointMod(v_base) / PX_PointMod(v);
...
px_void
PX_MatrixTranslation(px_matrix *mat, px_float x, px_float y, px_float z) {
    mat->_11 = 1.0f;
    mat->_12 = 0.0f;
    mat->_13 = 0.0f;
    mat->_14 = 0.0f;
...
px_void PX_MatrixRotateX(px_matrix *mat, px_float Angle) {
    ...
    mat->_14 = 0.0f;
    mat->_21 = 0.0f;
    mat->_22 = PX_cos_angle(Angle);
    mat->_23 = PX_sin_angle(Angle);
    mat->_24 = 0.0f;
    mat->_31 = 0.0f;
    mat->_32 = -PX_sin_angle(Angle);
    mat->_33 = PX_cos_angle(Angle);
    mat->_34 = 0.0f;
    mat->_41 = 0.0f;
    mat->_42 = 0.0f;
    mat->_43 = 0.0f;
...
px_void PX_MatrixRotateY(px_matrix *mat, px_float Angle) {
    mat->_11 = PX_cos_angle(Angle);
    mat->_12 = 0.0f;
    mat->_13 = PX_sin_angle(Angle);
    mat->_14 = 0.0f;
    mat->_21 = 0.0f;
    mat->_22 = 1.0f;
    mat->_23 = 0.0f;
    mat->_24 = 0.0f;
    mat->_31 = -PX_sin_angle(Angle);
    mat->_32 = 0.0f;
    mat->_33 = PX_cos_angle(Angle);
    mat->_34 = 0.0f;
    mat->_41 = 0.0f;
...
px_void PX_MatrixRotateZ(px_matrix *mat, px_float Angle) {
    ...
    mat->_11 = PX_cos_angle(Angle);
    mat->_12 = PX_sin_angle(Angle);
    mat->_13 = 0.0f;
    mat->_14 = 0.0f;
    mat->_21 = -PX_sin_angle(Angle);
    mat->_22 = PX_cos_angle(Angle);
    mat->_23 = 0.0f;
    mat->_24 = 0.0f;
    mat->_31 = 0.0f;
...
px_void PX_MatrixRotateXRadian(px_matrix *mat, px_float rad) {
    ...
    mat->_14 = 0.0f;
    mat->_21 = 0.0f;
    mat->_22 = PX_cos_radian(rad);
    mat->_23 = PX_sin_radian(rad);
    mat->_24 = 0.0f;
    mat->_31 = 0.0f;
    mat->_32 = -PX_sin_radian(rad);
    mat->_33 = PX_cos_radian(rad);
    mat->_34 = 0.0f;
    mat->_41 = 0.0f;
    mat->_42 = 0.0f;
    mat->_43 = 0.0f;
...
px_void PX_MatrixRotateYRadian(px_matrix *mat, px_float rad) {
    mat->_11 = PX_cos_radian(rad);
    mat->_12 = 0.0f;
    mat->_13 = PX_sin_radian(rad);
    mat->_14 = 0.0f;
    mat->_21 = 0.0f;
    mat->_22 = 1.0f;
    mat->_23 = 0.0f;
    mat->_24 = 0.0f;
    mat->_31 = -PX_sin_radian(rad);
    mat->_32 = 0.0f;
    mat->_33 = PX_cos_radian(rad);
    mat->_34 = 0.0f;
    mat->_41 = 0.0f;
...
px_void PX_MatrixRotateZRadian(px_matrix *mat, px_float rad) {
    ...
    mat->_11 = PX_cos_radian(rad);
    mat->_12 = PX_sin_radian(rad);
    mat->_13 = 0.0f;
    mat->_14 = 0.0f;
    mat->_21 = -PX_sin_radian(rad);
    mat->_22 = PX_cos_radian(rad);
    mat->_23 = 0.0f;
    mat->_24 = 0.0f;
    mat->_31 = 0.0f;
...
px_bool PX_MatrixInverse(px_matrix *mat) { return Gauss(mat->m, mat->m); }
...
px_color_hsl PX_ColorRGBToHSL(px_color color_rgb) {
    ...
    if (max == min) {
        ...
        hsl.S = (hsl.L < 0.5f ? (max - min) / (max + min)
                              : (max - min) / (2.0f - max - min));
    }
    if (r == max)
        hsl.H = (g - b) * 1.0f / (max - min);
    if (g == max)
        hsl.H = 2.0f + (b - r) * 1.0f / (max - min);
    if (b == max)
        hsl.H = 4.0f + (r - g) * 1.0f / (max - min);
    hsl.H = hsl.H * 60;
    if (hsl.H < 0) {
        hsl.H += 360;
    }
...
px_color PX_ColorHSLToRGB(px_color_hsl color_hsl) {
    ...
    if (color_hsl.S == 0) {
        ...
        if (6.0f * temp3 < 1)
            r = temp1 + (temp2 - temp1) * 6.0f * temp3;
        else if (2.0f * temp3 < 1)
            r = temp2;
        else if (3.0f * temp3 < 2)
            r = temp1 + (temp2 - temp1) * ((2.0f / 3.0f) - temp3) * 6.0f;
        else
            r = temp1;
        ...
        if (temp3 > 1)
            temp3 = temp3 - 1.0f;
        if (6.0f * temp3 < 1)
            g = temp1 + (temp2 - temp1) * 6.0f * temp3;
        else if (2.0f * temp3 < 1)
            g = temp2;
        else if (3.0f * temp3 < 2)
            g = temp1 + (temp2 - temp1) * ((2.0f / 3.0f) - temp3) * 6.0f;
        else
            g = temp1;
        ...
        if (temp3 > 1)
            temp3 = temp3 - 1.0f;
        if (6.0f * temp3 < 1)
            b = temp1 + (temp2 - temp1) * 6.0f * temp3;
        else if (2.0f * temp3 < 1)
            b = temp2;
        else if (3.0f * temp3 < 2)
            b = temp1 + (temp2 - temp1) * ((2.0f / 3.0f) - temp3) * 6.0f;
        else
            b = temp1;
    }

    return PX_COLOR((px_uchar) (color_hsl.a * 255),
                    (px_uchar) (r * 255),
                    (px_uchar) (g * 255),
                    (px_uchar) (b * 255));
}
...
px_point2D
PX_Point2DBase(px_point2D base1, px_point2D base2, px_point2D target) {
    base1 = PX_Point2DNormalization(base1);
    base2 = PX_Point2DNormalization(base2);

    return PX_POINT2D((target.x * base2.y - base2.x * target.y)
                          / (base1.x * base2.y - base2.x * base1.y),
                      (target.x * base1.y - base1.x * target.y)
                          / (base2.x * base1.y - base1.x * base2.y));
}
...
px_point PX_PointInverse(px_point p1) { return PX_POINT(-p1.x, -p1.y, -p1.z); }

px_float PX_PointMod(px_point p) {
    return PX_sqrt(p.x * p.x + p.y * p.y + p.z * p.z);
}

px_float PX_Point2DMod(px_point2D p) { return PX_sqrt(p.x * p.x + p.y * p.y); }
...
px_bool PX_isRectCrossRect(px_rect rect1, px_rect rect2) {
    ...
    if (disx * 2 < rect1.width + rect2.width
        && disy * 2 < rect1.height + rect2.height) {
        return PX_TRUE;
    }
    return PX_FALSE;
}
...
px_bool PX_isCircleCrossCircle(px_point center1,
                               px_float radius1,
                               px_point center2,
                               px_float radius2) {
...
px_bool PX_isXYInRegion(px_float x,
                        px_float y,
                        px_float rectx,
                        px_float recty,
                        px_float width,
...
px_bool PX_isLineCrossRect(
    px_point p1, px_point p2, px_rect rect, px_point *cp1, px_point *cp2) {
    px_float k, b;
    px_float calx, caly;
...
px_void PX_FDCT(_IN _OUT px_complex x[], _OUT px_complex X[], px_int N) {
    ...
    for (i = 0; i < N; i++) {
        X[i].re = X[i].re * PX_cosd(i * PX_PI / (N * 2))
                  + X[i].im * PX_sind(i * PX_PI / (N * 2));
        X[i].im = 0;
    }
...
px_void PX_Cepstrum(_IN px_complex x[],
                    _OUT px_complex X[],
                    px_int N,
                    PX_CEPSTRUM_TYPE type) {
    px_int i;
    PX_FFT(x, X, N);
...
px_float PX_PitchEstimation(_IN px_complex x[],
                            px_int N,
                            px_int sampleRate,
                            px_int low_Hz,
                            px_int high_Hz) {
...
px_void PX_PreEmphasise(const px_double *data,
                        px_int len,
                        px_double *out,
...
px_void PX_LinearInterpolationResample(_IN px_double x[],
                                       _OUT px_double X[],
                                       px_int N,
                                       px_int M) {
    px_int k, m = 0;
...
px_void PX_SincInterpolationResample(_IN px_double x[],
                                     _OUT px_double X[],
                                     px_int N,
                                     px_int M) {
...
px_void
PX_UpSampled(_IN px_complex x[], _OUT px_complex X[], px_int N, px_int L) {
    px_int i, j;
    for (i = 0; i < N; i++) {
        X[i * L] = x[i];
        for (j = 1; j < L; j++) {
...
px_void
PX_DownSampled(_IN px_complex x[], _OUT px_complex X[], px_int N, px_int M) {
    px_int i;
    if (((N) & (N - 1)) == 0) {
        PX_FFT(x, X, N);
    } else {
...
px_bool PX_strIsNumeric(const px_char *str) {
    ...
    for (CurrentCharIndex = 1; CurrentCharIndex < PX_strlen(str);
         CurrentCharIndex++) {
        if (!PX_charIsNumeric(str[CurrentCharIndex])) {
            if (str[CurrentCharIndex] == 'e'
...
static px_double __px_pow_i(px_double num, px_int n) {
    ...
    for (i = 1; i <= n; i++) {
        ...
        if (powint > 0) {
            if (powint >= PX_DBL_POSITIVE_MAX
                || powint <= PX_DBL_POSITIVE_MIN) {
                PX_ASSERT();
                break;
            }
        } else {
            if (powint <= PX_DBL_NEGATIVE_MIN
                || powint >= PX_DBL_NEGATIVE_MAX) {
                PX_ASSERT();
                break;
            }
        }
...
px_double PX_log(px_double __x) { return PX_ln(__x); }

px_double PX_lg(px_double __x) { return PX_log10(__x); }

px_double PX_log10(px_double __x) {
    return PX_ln(__x) / 2.30258509299404568401799145468;
}

px_double PX_tand(px_double radian) { return PX_tan_radian((px_float) radian); }
...
px_void
PX_FileGetName(const px_char filefullName[], px_char _out[], px_int outSize) {
    px_int s;
    if (outSize == 0) {
        return;
    }
...
px_void PX_FileGetFullName(const px_char filefullName[],
                           px_char _out[],
                           px_int outSize) {
    px_int s;
    if (outSize == 0) {
...
px_void
PX_FileGetPath(const px_char filefullName[], px_char _out[], px_int outSize) {
    px_int s, i;
    if (outSize == 0) {
        return;
    }
...
px_void
PX_FileGetExt(const px_char filefullName[], px_char _out[], px_int outSize) {
    px_int s;
...
px_bool PX_IsValidIPAddress(const px_char *ip_addr) {
    ...
    px_int bitsNumber
        = 0; // how many bits in ip_addr,The valid ip address should be 4 bits
    ...
    if (len < 7 || len > 15)
        return PX_FALSE; // The valid IP address length should be 7 to 15

    for (i = 0; i < len; i++) {
        if (ip_addr[i] == '.' || i == len - 1) {
            bitsLen = i - position;
            if (i == len - 1)
                bitsLen++; // at the end, Need to add an extra length 1
            if (bitsLen > 3 || bitsLen == 0)
                return PX_FALSE;
            if (bitsLen != 1 && ip_addr[position] == '0')
                return PX_FALSE;

            for (n = 0, bits = 0; n < bitsLen; n++, position++)
                bits = 10 * bits + (ip_addr[position] - '0');
            if (bits > 255)
                return PX_FALSE;
            position++;
            bitsNumber++;
...
px_float PX_PointDistance(px_point p1, px_point p2) {
    return PX_sqrt((p1.x - p2.x) * (p1.x - p2.x)
                   + (p1.y - p2.y) * (p1.y - p2.y));
}
...
px_void
PX_WindowFunction_Apply(px_double data[], px_double window[], px_int N) {
    px_int i;
    for (i = 0; i < N; i++) {
        data[i] *= window[i];
    }
...
px_void PX_WindowFunction_blackMan(px_double data[], px_int N) {
    ...
    for (n = 0; n < N; n++) {
        data[n]
            = 0.42
              - 0.5
                    * PX_cosd((px_float) (2 * PX_PI * (px_double) n / (N - 1))
                              + 0.08f
                                    * PX_cos_radian(
                                        (px_float) (4 * PX_PI * (px_double) n
                                                    / (N - 1))));
    }
}
...
px_void PX_WindowFunction_sinc(px_double data[], px_int N) {
    px_int n;
    for (n = 0; n < N; n++) {
        data[N - n - 1] = (data[n] = PX_sind(PX_PI * (((px_double) n) + 0.5)
                                             / ((px_double) N)));
    }
}
...
px_double PX_Bessel(px_int n, px_double x) {
    ...
    static px_double b[7] = {0.5,
                             ...
                             0.00301532,
                             0.00032411};
    static px_double c[9] = {0.39894228,
                             0.01328592,
                             0.00225319,
                             -0.00157565,
                             0.00916281,
                             ...
                             -0.01647633,
                             0.00392377};
    static px_double d[9] = {0.39894228,
                             -0.03988024,
                             -0.00362018,
                             0.00163801,
                             -0.01031555,
...
px_void PX_WindowFunction_tukey(px_double data[], px_int N) {
    ...
    for (n = 9 * (N - 2) / 10; n <= N - 1; n++) {
        data[n]
            = 0.5
              * (1 - PX_cosd((px_float) (10 * PX_PI * (N - n - 1) / (N + 8))));
        ;
    }
}
...
px_void PX_WindowFunction_hanning(px_double data[], px_int N) {
    ...
    for (n = 0; n < N; n++) {
        data[n]
            = 0.5
              - 0.5 * PX_cosd((px_float) (2 * PX_PI * (px_double) n / (N - 1)));
    }
}
...
static px_double PX_FIRWindow(PX_FIRFILTER_WINDOW_TYPE type,
                              px_int n,
                              px_int i,
                              px_double beta) {
    px_int k;
    ...
    switch (type) {
        ...
        case PX_FIRFILTER_WINDOW_TYPE_TUKEY: {
            ...
            if (i <= k) {
                w = 0.5
                    * (1.0 - PX_cos_radian((px_float) (i * pi / (k + 1.0f))));
            }
            if (i > n - k - 2) {
                w = 0.5
                    * ((px_float) (1.0
                                   - PX_cos_radian(
                                       (px_float) ((n - i - 1.0f) * pi
                                                   / (k + 1.0f)))));
            }
            break;
        }
        ...
        case PX_FIRFILTER_WINDOW_TYPE_HANNING: {
            w = 0.5
                * (1.0
                   - PX_cos_radian((px_float) (2.0f * i * pi / (n - 1.0f))));
            break;
        }
        case PX_FIRFILTER_WINDOW_TYPE_HAMMING: {
            w = 0.54
                - 0.46 * PX_cos_radian((px_float) (2.0f * i * pi / (n - 1.0)));
            break;
        }
        case PX_FIRFILTER_WINDOW_TYPE_BLACKMAN: {
            w = 0.42
                - 0.5 * PX_cos_radian((px_float) (2.0f * i * pi / (n - 1.0f)))
                + 0.08 * PX_cos_radian((px_float) (4.0f * i * pi / (n - 1.0f)));
            break;
        }
...
px_sine
PX_InstantaneousFrequency(px_sine src, px_double p2, px_double delta_t) {
...
px_void PX_FIRFilterBuild(PX_FIRFILTER_TYPE bandtype,
                          px_double fln,
                          px_double fhn,
                          PX_FIRFILTER_WINDOW_TYPE wn,
                          px_double h[],
                          px_int n,
                          px_double beta) {
    px_int i, n2, mid;
    ...
    switch (bandtype) {
        case PX_FIRFILTER_TYPE_LOWPASS: {
            for (i = 0; i <= n2; i++) {
                s = i - delay;
                h[i] = (PX_sind((wc1 * s)) / (pi * s))
                       * PX_FIRWindow(wn, n + 1, i, beta);
                h[n - i] = h[i];
            }
            if (mid == 1) {
                h[n / 2] = wc1 / pi;
...
px_double PX_GroupDelay(px_double f,
                        px_double *B,
                        px_int sizeB,
                        px_double *A,
                        px_int sizeA,
...
static px_double PX_PhaseIIR(
    px_double omega, px_double *B, px_int sizeB, px_double *A, px_int sizeA) {
    px_double C[2];
    px_double HB[2] = {0};
    px_double HA[2] = {0};
    px_double magd2;
...
px_double PX_PhaseDelayDerive(px_double omega,
                              px_double *B,
                              px_int sizeB,
                              px_double *A,
                              px_int sizeA,
...
px_double PX_PhaseDelay(px_double f,
                        px_double *B,
                        px_int sizeB,
                        px_double *A,
                        px_int sizeA,
    ...
    return grpdel
           - omega * PX_PhaseDelayDerive(omega, B, sizeB, A, sizeA, 0.0005f);
}

px_byte PX_ReadBitLE(px_uint32 *bitpointer, const px_byte *bitstream) {
    px_byte result
        = (px_byte) ((bitstream[(*bitpointer) >> 3] >> ((*bitpointer) & 0x7))
                     & 1);
    (*bitpointer)++;
    return result;
}
...
px_uint32
PX_ReadBitsLE(px_uint32 *bitpointer, const px_byte *bitstream, px_int nbits) {
    px_uint32 result = 0;
    px_int i;
    for (i = 0; i < nbits; i++)
...
px_byte PX_ReadBitBE(px_uint32 *bitpointer, const px_byte *bitstream) {
    px_byte result = (px_byte) ((bitstream[(*bitpointer) >> 3]
                                 >> (7 - ((*bitpointer) & 0x7)))
                                & 1);
    (*bitpointer)++;
    return result;
...
px_uint32
PX_ReadBitsBE(px_uint32 *bitpointer, const px_byte *bitstream, px_int nbits) {
    px_uint32 result = 0;
    px_int i;
    for (i = 0; i < nbits; i++)
...
px_void PX_MemoryStreamInitialize(PX_MemoryStream *pStream,
                                  px_byte *bitstream,
                                  px_int size) {
...
px_byte PX_MemoryStreamReadBitLE(PX_MemoryStream *pStream) {
    if (pStream->bitpointer / 8 <= pStream->size * 8) {
        return PX_ReadBitLE((px_uint32 *) &pStream->bitpointer,
                            pStream->bitstream);
    } else {
        return 0;
    }
...
px_byte PX_MemoryStreamReadBitBE(PX_MemoryStream *pStream) {
    if (pStream->bitpointer / 8 <= pStream->size * 8) {
        return PX_ReadBitBE((px_uint32 *) &pStream->bitpointer,
                            pStream->bitstream);
    } else {
        return 0;
    }
...
px_byte PX_MemoryStreamReadByte(PX_MemoryStream *pStream) {
    ...
    if (pStream->bitpointer / 8 + 8 <= pStream->size * 8) {
        px_byte result = (px_byte) PX_ReadBitsLE(
            (px_uint32 *) &pStream->bitpointer, pStream->bitstream, 8);
        return result;
    } else {
        return 0;
    }
...
px_uint32 PX_MemoryStreamReadBitsLE(PX_MemoryStream *pStream, px_int nbits) {
    if (pStream->bitpointer / 8 + nbits <= pStream->size * 8) {
        px_uint32 result = PX_ReadBitsLE(
            (px_uint32 *) &pStream->bitpointer, pStream->bitstream, nbits);
        return result;
    } else {
        return 0;
    }
...
px_uint32 PX_MemoryStreamReadBitsBE(PX_MemoryStream *pStream, px_int nbits) {
    if (pStream->bitpointer / 8 + nbits <= pStream->size * 8) {
        px_uint32 result = PX_ReadBitsBE(
            (px_uint32 *) &pStream->bitpointer, pStream->bitstream, nbits);
        return result;
    } else {
        return 0;
...
px_double PX_HzToMel(px_double hz) { return 2595 * PX_log10(1 + hz / 700); }
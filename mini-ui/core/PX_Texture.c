...
px_bool PX_TextureCreate(px_memorypool *mp,
                         px_texture *tex,
                         px_int width,
                         px_int height) {
...
px_bool PX_TextureCreateFromMemory(px_memorypool *mp,
                                   px_void *data,
                                   px_int size,
                                   px_texture *tex) {
    px_int width;
...
static px_int PX_TextureRenderParallel(px_void *parallel_data) {
    PX_TEXTURERENDER_PARALLEL_DATA *param_data
        = (PX_TEXTURERENDER_PARALLEL_DATA *) parallel_data;
    px_int i, j;
    px_int bR, bG, bB, bA;
    px_color clr;
    ...
    if (param_data->blend) {
        for (j = param_data->top; j <= param_data->bottom; j++) {
            for (i = param_data->left; i <= param_data->right; i++) {
                ...
                PX_SurfaceDrawPixel(param_data->psurface,
                                    param_data->x + i,
                                    param_data->y + j,
                                    clr);
            }
...
px_void PX_TextureRenderClipMirror(px_surface *psurface,
                                   px_texture *tex,
                                   px_int x,
                                   px_int y,
                                   px_int clipx,
                                   ...
                                   PX_ALIGN refPoint,
                                   PX_TEXTURERENDER_BLEND *blend,
                                   PX_TEXTURERENDER_MIRRROR_MODE mirrorMode) {
    px_int i, j;
    px_int bR, bG, bB, bA;
    ...
    if (blend) {
        ...
        for (j = 0; j < cliph; j++) {
            for (i = 0; i < clipw; i++) {
                ...
                if (x + i < psurface->limit_left
                    || x + i > psurface->limit_right
                    || y + j < psurface->limit_top
...
px_void PX_TextureRenderClip(px_surface *psurface,
                             px_texture *tex,
                             px_int x,
                             px_int y,
                             px_int clipx,
                             ...
                             PX_ALIGN refPoint,
                             PX_TEXTURERENDER_BLEND *blend) {
    PX_TextureRenderClipMirror(psurface,
                               tex,
                               x,
                               y,
                               clipx,
...
px_void PX_TextureRenderMirror(px_surface *psurface,
                               px_texture *tex,
                               px_int x,
                               px_int y,
                               PX_ALIGN refPoint,
...
px_void PX_TextureRender(px_surface *psurface,
                         px_texture *tex,
                         px_int x,
                         px_int y,
                         PX_ALIGN refPoint,
                         PX_TEXTURERENDER_BLEND *blend) {
    PX_TextureRenderMirror(psurface,
                           tex,
                           x,
                           y,
                           refPoint,
...
px_void PX_TextureCover(px_surface *psurface,
                        px_texture *tex,
                        px_int x,
                        px_int y,
                        PX_ALIGN refPoint) {
    ...
    for (j = top; j <= bottom; j++) {
        PX_memcpy(
            &psurface->surfaceBuffer[x + left + psurface->width * (y + j)],
            &tex->surfaceBuffer[tex->width * (j) + left],
            sizeof(px_color) * (right - left + 1));
    }
}

px_void PX_TextureGetVisibleRange(px_texture *ptexture,
                                  px_int *pLeft,
                                  px_int *pRight,
                                  px_int *pTop,
                                  px_int *pBottom) {
    px_int x = 0, y = 0;
    px_int left = ptexture->width - 1, right = 0, top = ptexture->height - 1,
           bottom = 0;
...
px_void PX_TextureRenderPixelShader(px_surface *psurface,
                                    px_texture *tex,
                                    px_int x,
                                    px_int y,
                                    PX_ALIGN refPoint,
                                    PX_TexturePixelShader shader,
                                    px_void *ptr) {
...
px_void PX_TextureRenderRotation(px_surface *psurface,
                                 px_texture *tex,
                                 px_int x,
                                 px_int y,
                                 PX_ALIGN refPoint,
                                 PX_TEXTURERENDER_BLEND *blend,
                                 px_int Angle) {
    PX_TextureRenderRotation_sincos(psurface,
                                    tex,
                                    x,
                                    y,
                                    refPoint,
                                    blend,
                                    PX_sin_angle((px_float) Angle),
                                    PX_cos_angle((px_float) Angle));
}

px_void PX_TextureRenderRotation_vector(px_surface *psurface,
                                        px_texture *tex,
                                        px_int x,
                                        px_int y,
                                        PX_ALIGN refPoint,
    ...
    if (p_vector.x || p_vector.y) {
        PX_TextureRenderRotation_sincos(psurface,
                                        ...
                                        refPoint,
                                        blend,
                                        PX_Point2D_sin(p_vector),
                                        PX_Point2D_cos(p_vector));
    }
}

px_void PX_TextureRenderRotation_sincos(px_surface *psurface,
                                        px_texture *tex,
                                        px_int x,
                                        px_int y,
                                        PX_ALIGN refPoint,
    ...
    for (j = top; j < bottom; j++) {
        for (i = left; i < right; i++) {
            ...
            if (SampleX > 0 && (SampleX) < resWidth && SampleY > 0
                && (SampleY) < resHeight) {
                // Sample color from resTexture
                sampleColor = PX_SURFACECOLOR(
                    tex, PX_TRUNC(SampleX), PX_TRUNC(SampleY));
            ...
            if (blend) {
                px_int bA, bR, bG, bB;
                px_color clr = PX_COLOR((px_uchar) mixa,
                                        (px_uchar) mixr,
                                        (px_uchar) mixg,
                                        (px_uchar) mixb);
                px_int Ab = (px_int) (blend->alpha * 1000);
                ...
                PX_SurfaceDrawPixel(psurface,
                                    i + x,
                                    j + y,
                                    PX_COLOR((px_uchar) mixa,
                                             (px_uchar) mixr,
                                             (px_uchar) mixg,
                                             (px_uchar) mixb));
            }
...
px_void PX_TextureRenderMask(px_surface *psurface,
                             px_texture *mask_tex,
                             px_texture *map_tex,
                             px_int x,
                             px_int y,
    ...
    if (blend) {
        ...
        for (j = top; j <= bottom; j++) {
            for (i = left; i <= right; i++) {
                ...
                clr._argb.a = 1 * clr._argb.a
                              * pmaskdata[j * mask_tex->width + i]._argb.a
                              / 255;
                bA = (px_int) (clr._argb.a * Ab / 1000);
                bR = (px_int) (clr._argb.r * Rb / 1000);
                bG = (px_int) (clr._argb.g * Gb / 1000);
...
px_bool PX_TextureCreateScale(px_memorypool *mp,
                              px_texture *resTexture,
                              px_int newWidth,
                              px_int newHeight,
                              px_texture *out) {
...
px_bool PX_TextureCreateRotationRadian(px_memorypool *mp,
                                       px_texture *resTexture,
                                       px_float radian,
                                       px_texture *out) {
    return PX_TextureCreateRotationAngle(
        mp, resTexture, (px_float) (radian * 180 / PX_PI), out);
}

px_bool PX_TextureCreateRotationAngle(px_memorypool *mp,
                                      px_texture *resTexture,
                                      px_float Angle,
                                      px_texture *out) {
    ...
    for (j = 0; j < height; j++) {
        for (i = 0; i < width; i++) {
            ...
            if (SampleX > 0 && (SampleX) < resWidth && SampleY > 0
                ...
                // Sample color from resTexture
                sampleColor = PX_SURFACECOLOR(
                    resTexture, PX_TRUNC(SampleX), PX_TRUNC(SampleY));
                Weight = (1 - PX_FRAC(SampleX)) * (1 - PX_FRAC(SampleY));
                mixa += sampleColor._argb.a * Weight;
                mixr += sampleColor._argb.r * Weight;
                mixg += sampleColor._argb.g * Weight;
...
px_bool PX_TextureRotationAngleToTexture(px_texture *resTexture,
                                         px_float Angle,
                                         px_texture *out) {
    px_int height, width, i, j, resHeight, resWidth;
    px_double SampleX, SampleY, mapX, mapY;
...
px_void PX_TextureFree(px_texture *tex) { PX_SurfaceFree(tex); }

px_int PX_TextureGetRenderRange(px_texture *tex,
                                px_int x,
                                px_int y,
                                px_int clipw,
                                px_int cliph,
...
px_void PX_SurfaceRender(px_surface *psurface,
                         px_surface *surface,
                         px_int x,
                         px_int y,
                         PX_ALIGN refPoint,
...
px_void PX_SurfaceCover(px_surface *pdestSurface,
                        px_surface *pResSurface,
                        px_int x,
                        px_int y,
                        PX_ALIGN refPoint) {
...
px_void PX_SurfaceSetRect(px_surface *psurface,
                          px_int left,
                          px_int top,
                          px_int right,
                          px_int bottom,
    ...
    for (i = top; i <= bottom; i++) {
        PX_memdwordset(psurface->surfaceBuffer + i * psurface->width + left,
                       color._argb.ucolor,
                       right - left + 1);
    }
}

px_void PX_TextureRenderEx(px_surface *psurface,
                           px_texture *resTexture,
                           px_int x,
                           px_int y,
                           PX_ALIGN refPoint,
    ...
    if (blend) {
        ...
        for (j = top; j <= bottom; j++) {
            for (i = left; i <= right; i++) {
                ...
                mapX = (px_int) ((ref_x * invCosAgl - ref_y * invSinAgl) / scale
                                 + resWidth / 2);
                mapY = (px_int) ((ref_x * invSinAgl + ref_y * invCosAgl) / scale
                                 + resHeight / 2);
        ...
        for (j = top; j <= bottom; j++) {
            for (i = left; i <= right; i++) {
                ...
                PX_SurfaceDrawPixel(psurface,
                                    x + i,
                                    y + j,
                                    PX_SURFACECOLOR(resTexture, mapX, mapY));
            }
        }
    }
}

px_void PX_TextureRenderMaskEx(px_surface *psurface,
                               px_texture *mask_tex,
                               px_texture *map_tex,
                               px_int x,
                               px_int y,
...
px_void PX_TextureFill(px_memorypool *mp,
                       px_texture *ptexture,
                       px_int x,
                       px_int y,
                       px_color test_color,
    ...
    while (pstack.size) {
        ...
        // xo mark
        while (x > 0
               && (PX_SurfaceGetPixel(ptexture, x - 1, y)._argb.ucolor
                   == test_color._argb.ucolor)) {
            x--;
        }

        // up
        if (y > 0) {
            if (PX_SurfaceGetPixel(ptexture, x, y - 1)._argb.ucolor
                == test_color._argb.ucolor) {
                __PX_POINT pt;
                pt.x = x;
                pt.y = y - 1;
        ...
        while (x < ptexture->width) {
            if (PX_SurfaceGetPixel(ptexture, x, y)._argb.ucolor
                != test_color._argb.ucolor) {
                break;
            }
            ...
            if (x > 0) {
                // up
                if (y > 0) {
                    if (PX_SurfaceGetPixel(ptexture, x - 1, y - 1)._argb.ucolor
                            != test_color._argb.ucolor
                        && PX_SurfaceGetPixel(ptexture, x, y - 1)._argb.ucolor
                               == test_color._argb.ucolor) {
                        __PX_POINT pt;
                        pt.x = x;
                        pt.y = y - 1;
...
px_void PX_TextureRegionRender(px_surface *psurface,
                               px_texture *resTexture,
                               px_int x,
                               px_int y,
                               px_int oft_left,
...
px_void PX_TextureRegionCopy(px_surface *psurface,
                             px_texture *resTexture,
                             px_int x,
                             px_int y,
                             px_int oft_left,
...
px_bool PX_ShapeCreate(px_memorypool *mp,
                       px_shape *shape,
                       px_int width,
                       px_int height) {
...
px_bool PX_ShapeCreateFromTexture(px_memorypool *mp,
                                  px_shape *shape,
                                  px_texture *texture) {
    px_int x, y;
    px_void *p
        = MP_Malloc(mp, texture->height * texture->width * sizeof(px_uchar));
    if (p != PX_NULL) {
        ...
        for (y = 0; y < texture->height; y++) {
            for (x = 0; x < texture->width; x++) {
                shape->alpha[y * shape->width + x]
                    = ((px_color *)
                           texture->surfaceBuffer)[y * texture->width + x]
                          ._argb.a;
            }
        }
        return PX_TRUE;
...
px_void PX_ShapeFree(px_shape *shape) { MP_Free(shape->MP, shape->alpha); }
...
px_void PX_ShapeRender(px_surface *psurface,
                       px_shape *shape,
                       px_int x,
                       px_int y,
                       PX_ALIGN refPoint,
    ...
    for (j = top; j <= bottom; j++) {
        for (i = left; i <= right; i++) {
            if (pdata[j * shape->width + i] != 0) {
                clr = blendColor;
                clr._argb.a
                    = 1 * clr._argb.a * pdata[j * shape->width + i] / 255;
                PX_SurfaceDrawPixel(psurface, x + i, y + j, clr);
            }
        }
    }
...
px_void PX_ShapeRenderEx(px_surface *psurface,
                         px_shape *shape,
                         px_int x,
                         px_int y,
                         PX_ALIGN refPoint,
    ...
    for (j = top; j <= bottom; j++) {
        for (i = left; i <= right; i++) {
            ...
            clr = blendColor;
            clr._argb.a = 1 * clr._argb.a
                          * shape->alpha[mapY * shape->width + mapX] / 255;
            PX_SurfaceDrawPixel(psurface, x + i, y + j, clr);
        }
    }
}

px_bool PX_ShapeCreateFromMemory(px_memorypool *mp,
                                 px_void *data,
                                 px_int size,
                                 px_shape *shape) {
    px_int width;
    ...
    if (PX_TRawVerify(data, size)) {
        ...
        if (PX_ShapeCreate(mp, shape, width, height)) {
            px_int i, j;
            px_color *pdataColor
                = (px_color *) ((px_char *) data + sizeof(PX_TRaw_Header));

            for (i = 0; i < height; i++) {
                for (j = 0; j < width; j++) {
                    shape->alpha[width * i + j]
                        = pdataColor[width * i + j]._argb.a;
                }
            }
        } else {
            return PX_FALSE;
...
px_void PX_ShapeRenderEx_sincos(px_surface *psurface,
                                px_shape *shape,
                                px_int x,
                                px_int y,
                                PX_ALIGN refPoint,
    ...
    for (j = top; j <= bottom; j++) {
        for (i = left; i <= right; i++) {
            ...
            clr = blendColor;
            clr._argb.a = 1 * clr._argb.a
                          * shape->alpha[shape->width * mapY + mapX] / 255;
            PX_SurfaceDrawPixel(psurface, x + i, y + j, clr);
        }
    }
}

px_void PX_ShapeRenderEx_vector(px_surface *psurface,
                                px_shape *shape,
                                px_int x,
                                px_int y,
                                PX_ALIGN refPoint,
                                ...
                                px_float scale,
                                px_point2D p_vector) {
    PX_ShapeRenderEx_sincos(psurface,
                            shape,
                            x,
                            y,
                            refPoint,
...
px_void PX_TextureRenderEx_sincos(px_surface *psurface,
                                  px_texture *resTexture,
                                  px_int x,
                                  px_int y,
                                  PX_ALIGN refPoint,
...
px_void PX_TextureRenderEx_vector(px_surface *psurface,
                                  px_texture *resTexture,
                                  px_int x,
                                  px_int y,
                                  PX_ALIGN refPoint,
                                  ...
                                  px_float scale,
                                  px_point2D p_vector) {
    PX_TextureRenderEx_sincos(psurface,
                              resTexture,
                              x,
                              y,
                              refPoint,
...
px_bool
PX_TextureCopy(px_memorypool *mp, px_texture *restexture, px_texture *dest) {
    if (!PX_TextureCreate(mp, dest, restexture->width, restexture->height))
        return PX_FALSE;
    PX_memcpy(dest->surfaceBuffer,
              restexture->surfaceBuffer,
...
px_bool PX_TextureScaleToTexture(px_texture *resTexture, px_texture *out) {
    ...
    px_float SampleX, SampleY, SampleArea, u, v, cellw, cellh, mixa, mixr, mixg,
        mixb;
    ...
    for (yoft = 0; yoft < newHeight; yoft++) {
        for (xoft = 0; xoft < newWidth; xoft++) {
            ...
            u = SampleX;
            for (horz = PX_TRUNC(SampleX);
                 horz <= PX_TRUNC(SampleX + SampleWidth);) {
                v = SampleY;
                ...
                for (vcl = PX_TRUNC(SampleY);
                     vcl <= PX_TRUNC(SampleY + SampleHeight);) {
            ...
            mixg > 255 ? mixg = 255 : 0;
            mixb > 255 ? mixb = 255 : 0;
            Dst[xoft + yoft * newWidth] = PX_COLOR((px_uchar) PX_APO(mixa),
                                                   (px_uchar) mixr,
                                                   (px_uchar) mixg,
                                                   (px_uchar) mixb);
            ;

...
px_void MP_UnreleaseInfo(px_memorypool *mp) {
    px_int i;
    for (i = 0;
         i < sizeof(mp->DEBUG_allocdata) / sizeof(mp->DEBUG_allocdata[0]);
         i++) {
...
MemoryNode *PX_MemoryPool_GetFreeTableAddr(px_memorypool *MP) {
    return (MemoryNode *) ((px_uchar *) MP->EndAddr
                           - (sizeof(MemoryNode) * MP->FreeTableCount) + 1);
}

MemoryNode *PX_MemoryPool_GetFreeTable(px_memorypool *MP, px_uint Index) {
    Index++;
    return (MemoryNode *) ((px_uchar *) MP->EndAddr
                           - (sizeof(MemoryNode) * Index) + 1);
}
...
px_void PX_UpdateMaxFreqSize(px_memorypool *MP) {
    ...
    for (i = 0; i < MP->FreeTableCount; i++) {
        ...
        if ((Size = (px_uint) (((px_char *) itNode->EndAddr
                                - (px_char *) itNode->StartAddr)
                               + 1))
...
MemoryNode *PX_AllocFromFreq(px_memorypool *MP, px_uint Size) {
    ...
    if (MP->MaxMemoryfragSize >= Size) {
        ...
        for (i = 0; i < MP->FreeTableCount; i++) {
            itNode = PX_MemoryPool_GetFreeTable(MP, i);
            fSize = (px_uint) ((px_char *) itNode->EndAddr
                               - (px_char *) itNode->StartAddr + 1);

            if (Size <= fSize && (Size + sizeof(MemoryNode) >= fSize)) {
                allocNode = (MemoryNode *) itNode->StartAddr;

                allocNode->StartAddr
                    = (px_char *) itNode->StartAddr + sizeof(MemoryNode);
                ...
                if (Size < fSize) {
                    ...
                    allocNode->StartAddr
                        = (px_char *) itNode->StartAddr + sizeof(MemoryNode);
                    allocNode->EndAddr
                        = (px_char *) itNode->StartAddr + Size - 1;
...
px_memorypool MP_Create(px_void *MemoryAddr, px_uint MemorySize) {
    ...
    for (DEBUG_i = 0;
         DEBUG_i < sizeof(MP.DEBUG_allocdata) / sizeof(MP.DEBUG_allocdata[0]);
         DEBUG_i++) {
...
px_void *MP_Malloc(px_memorypool *MP, px_uint Size) {
    ...
    for (DEBUG_i = 0;
        ...
        if (MP->DEBUG_allocdata[DEBUG_i].startAddr != PX_NULL) {
            MP_Append_data *pAppend;
            pAppend
                = (MP_Append_data *) ((px_uchar *) MP->DEBUG_allocdata[DEBUG_i]
                                          .endAddr
                                      - sizeof(MP_Append_data) + 1);
            itNode = (MemoryNode *) ((px_uchar *) MP->DEBUG_allocdata[DEBUG_i]
                                         .startAddr
                                     - sizeof(MemoryNode));
    ...
    if (Size % __PX_MEMORYPOOL_ALIGN_BYTES) {
        Size = (Size / __PX_MEMORYPOOL_ALIGN_BYTES + 1)
               * __PX_MEMORYPOOL_ALIGN_BYTES;
    }
    // Allocate from freq
    ...
    if (MemNode != 0) {
#if defined(PX_DEBUG_MODE) && defined(PX_MEMORYPOOL_DEBUG_CHECK)
        for (DEBUG_i = 0; DEBUG_i < sizeof(MP->DEBUG_allocdata)
                                        / sizeof(MP->DEBUG_allocdata[0]);
             DEBUG_i++) {
            if (MP->DEBUG_allocdata[DEBUG_i].startAddr == 0) {
                MP->DEBUG_allocdata[DEBUG_i].offset
                    = (px_byte *) MemNode->StartAddr
        ...
        pAppend = (MP_Append_data *) ((px_uchar *) MemNode->EndAddr
                                      - sizeof(MP_Append_data) + 1);
...
px_void MP_Free(px_memorypool *MP, px_void *pAddress) {
...
#if defined(PX_DEBUG_MODE) && defined(PX_MEMORYPOOL_DEBUG_CHECK)
    if (FreeNode.StartAddr != pAddress)
        PX_ASSERT();
        ...
        if (MP->DEBUG_allocdata[DEBUG_i].startAddr == pAddress) {
            ...
            pAppend
                = (MP_Append_data *) ((px_uchar *) MP->DEBUG_allocdata[DEBUG_i]
                                          .endAddr
                                      - sizeof(MP_Append_data) + 1);
            if (MP->DEBUG_allocdata[DEBUG_i].startAddr != FreeNode.StartAddr)
                PX_ASSERT();
            if (MP->DEBUG_allocdata[DEBUG_i].endAddr != FreeNode.EndAddr)
                PX_ASSERT();
            if (pAppend->append != MP_APPENDDATA_MAGIC)
                PX_ASSERT();
            MP->DEBUG_allocdata[DEBUG_i].offset = 0;
    ...
    if (MP->enable_allocdata_tracert) {
        if (DEBUG_i
            == sizeof(MP->DEBUG_allocdata) / sizeof(MP->DEBUG_allocdata[0])) {
            if (MP->ErrorCall_Ptr == PX_NULL) {
                PX_LOG("Invalid address free");
                PX_ASSERT();
            } else
                MP->ErrorCall_Ptr(MP->userptr,
    ...
    if ((px_char *) FreeNode.EndAddr + 1 == (px_char *) MP->AllocAddr) {
        for (i = 0; i < MP->FreeTableCount; i++) {
            itNode = PX_MemoryPool_GetFreeTable(MP, i);
            if ((px_char *) itNode->EndAddr + 1
                == (px_char *) FreeNode.StartAddr) {
                MP->AllocAddr = itNode->StartAddr;
                MP->FreeSize += (px_uint32) ((px_char *) FreeNode.EndAddr
                                             - (px_char *) FreeNode.StartAddr
                                             + sizeof(MemoryNode) + 1);
                MP->FreeSize += (px_uint32) ((px_char *) itNode->EndAddr
                                             - (px_char *) itNode->StartAddr + 1
                                             + sizeof(MemoryNode));
                PX_MemoryPoolRemoveFreeNode(MP, i);
                goto _END;
    ...
    if (bExist == 0) {
        ...
        for (i = 0; i < MP->FreeTableCount; i++) {
            itNode = PX_MemoryPool_GetFreeTable(MP, i);
            if (FreeNode.StartAddr >= itNode->StartAddr
                && FreeNode.StartAddr <= itNode->EndAddr) {
                // alloc error
                PX_ASSERT();
                goto _END;
            }
            if (FreeNode.EndAddr >= itNode->StartAddr
                && FreeNode.EndAddr <= itNode->EndAddr) {
                PX_ASSERT();
                goto _END;
            }
        }
    ...
    for (DEBUG_i = 0;
        ...
        if (MP->DEBUG_allocdata[DEBUG_i].startAddr != PX_NULL) {
            for (i = 0; i < MP->FreeTableCount; i++) {
                itNode = PX_MemoryPool_GetFreeTable(MP, i);
                if (MP->DEBUG_allocdata[DEBUG_i].startAddr >= itNode->StartAddr
                    && MP->DEBUG_allocdata[DEBUG_i].startAddr
                           <= itNode->EndAddr) {
                    // error free
                    PX_ASSERT();
                }
                if (MP->DEBUG_allocdata[DEBUG_i].endAddr >= itNode->StartAddr
                    && MP->DEBUG_allocdata[DEBUG_i].endAddr
                           <= itNode->EndAddr) {
                    PX_ASSERT();
                }
            }
...
px_void
MP_ErrorCatch(px_memorypool *Pool, PX_MP_ErrorCall ErrorCall, px_void *ptr) {
...
px_uint MP_Size(px_memorypool *Pool, px_void *pAddress) {
    ...
    TempNode = (MemoryNode *) TempPointer;
#if defined(PX_DEBUG_MODE) && defined(PX_MEMORYPOOL_DEBUG_CHECK)
    return (px_uint) (((px_char *) (TempNode->EndAddr)
                       - (px_char *) (TempNode->StartAddr))
                      + 1 - sizeof(MP_Append_data));
#else
    return (px_uint32) ((px_char *) (TempNode->EndAddr)
                        - (px_char *) (TempNode->StartAddr))
           + 1;
#endif
}

...
px_bool PX_VectorInitialize(px_memorypool *mp,
                            px_vector *vec,
                            px_int nodeSize,
                            px_int init_size) {
...
px_bool PX_VectorSet(px_vector *vec, px_uint index, px_void *data) {
    ...
    PX_memcpy(
        (px_byte *) vec->data + index * vec->nodesize, data, vec->nodesize);
    return PX_TRUE;
}
...
px_bool PX_VectorPushback(px_vector *vec, px_void *data) {
    ...
    if (vec->size < vec->allocsize) {
        if (data) {
            PX_memcpy((px_byte *) vec->data + vec->size * vec->nodesize,
                      data,
                      vec->nodesize);
        } else {
            PX_memset((px_byte *) vec->data + vec->size * vec->nodesize,
                      0,
                      vec->nodesize);
        }
        vec->size++;
...
px_bool PX_VectorPushTo(px_vector *vec, px_void *data, px_int index) {
    ...
    PX_memcpy((px_byte *) vec->data + (index + 1) * vec->nodesize,
              (px_byte *) vec->data + (index) *vec->nodesize,
              (vec->size - index - 1) * vec->nodesize);
    PX_memcpy(
        (px_byte *) vec->data + (index) *vec->nodesize, data, vec->nodesize);
    return PX_TRUE;
}
...
px_bool PX_VectorErase(px_vector *vec, px_int index) {
    ...
    for (i = index; i < vec->size - 1; i++) {
        PX_memcpy((px_byte *) vec->data + i * vec->nodesize,
                  (px_byte *) vec->data + (i + 1) * vec->nodesize,
                  vec->nodesize);
    }
    vec->size--;
    return PX_TRUE;
...
px_bool PX_VectorResize(px_vector *vec, px_int size) {
    if (size != vec->size) {
        PX_VectorFree(vec);
        if (!PX_VectorInitialize(vec->mp, vec, vec->nodesize, size))
            return PX_FALSE;

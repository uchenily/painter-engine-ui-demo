...
px_bool PX_SurfaceCreate(px_memorypool *mp,
                         px_int width,
                         px_int height,
                         px_surface *surface) {
    if (width > 0 && height > 0) {
...
px_void PX_SurfaceSetLimit(px_surface *ps,
                           px_int limit_left,
                           px_int limit_top,
                           px_int limit_right,
                           px_int limit_bottom) {
...
px_void
PX_SurfaceSetPixel(px_surface *psurface, px_int X, px_int Y, px_color color) {
    if (X <= psurface->limit_right && X >= psurface->limit_left
        && Y <= psurface->limit_bottom && Y >= psurface->limit_top) {
        psurface->surfaceBuffer[X + psurface->width * Y] = color;
    }
}

px_void PX_SurfaceDrawPixelWithoutLimit(px_surface *psurface,
                                        px_int X,
                                        px_int Y,
                                        px_color COLOR) {
    px_color c;
    ...
    if (COLOR._argb.a == 0xff) {
        ...
        c._argb.r = (px_uchar) (((255 - COLOR._argb.a) * c._argb.r
                                 + COLOR._argb.r * COLOR._argb.a)
                                / 255);
        c._argb.g = (px_uchar) (((255 - COLOR._argb.a) * c._argb.g
                                 + COLOR._argb.g * COLOR._argb.a)
                                / 255);
        c._argb.b = (px_uchar) (((255 - COLOR._argb.a) * c._argb.b
                                 + COLOR._argb.b * COLOR._argb.a)
                                / 255);
...
px_void
PX_SurfaceDrawPixel(px_surface *psurface, px_int X, px_int Y, px_color COLOR) {
    if (X > psurface->limit_right || X < psurface->limit_left
        || Y > psurface->limit_bottom || Y < psurface->limit_top) {
        return;
    }
...
px_void PX_SurfaceClear(px_surface *psurface,
                        px_int left,
                        px_int top,
                        px_int right,
                        px_int bottom,
...
px_void PX_SurfaceClearAll(px_surface *psurface, px_color color) {
    PX_SurfaceClear(
        psurface, 0, 0, psurface->width - 1, psurface->height - 1, color);
}
...
PX_HASHMAP_RETURN
PX_MapPut(px_map *m, const px_char stringkey[], px_void *value) {
    px_map_element *newnode
        = (px_map_element *) MP_Malloc(m->mp, sizeof(px_map_element));
    newnode->Ptr = value;
...
PX_HASHMAP_RETURN
PX_MapPutInt(px_map *m, const px_char stringkey[], px_int value) {
    px_map_element *newnode
        = (px_map_element *) MP_Malloc(m->mp, sizeof(px_map_element));
    newnode->_int = value;
    PX_strcpy(newnode->string, stringkey, sizeof(newnode->string));
...
PX_HASHMAP_RETURN
PX_MapPutFloat(px_map *m, const px_char stringkey[], px_float value) {
    px_map_element *newnode
        = (px_map_element *) MP_Malloc(m->mp, sizeof(px_map_element));
...
px_int PX_MapGetSize(px_map *m) { return m->size; }

...
static px_void PX_DrawLinePixel_RecK(px_surface *psurface,
                                     px_int x,
                                     px_int y,
                                     px_color color) {
...
static px_void PX_DrawLinePixel_InvK(px_surface *psurface,
                                     px_int x,
                                     px_int y,
                                     px_color color) {
...
static px_void PX_DrawLinePixel_InvRecK(px_surface *psurface,
                                        px_int x,
                                        px_int y,
                                        px_color color) {
    PX_SurfaceDrawPixel(psurface, 2 * st_symmetry_axis - y, x, color);
}
px_void PX_GeoDrawLine(px_surface *psurface,
                       px_int x0,
                       px_int y0,
                       px_int x1,
                       px_int y1,
    ...
    px_float p0x, p0y, p1x, p1y, p2x, p2y, p3x, p3y, vx, vy, rx, ry, xlen, ylen,
        S, ftemp, ryL, ryR, rxL, rxR;
    px_float k, recK;
    px_color clr;
    px_void (*func_DrawPixel)(
        px_surface * psurface, px_int x, px_int y, px_color color);
    ...
    if (x0 == x1) {
        ...
        if (lineWidth & 1) {
            ...
            for (i = 0; i < len; i++) {
                ...
                PX_SurfaceDrawPixel(
                    psurface, x0 - lineWidth / 2, lm + i, color);
                PX_SurfaceDrawPixel(
                    psurface, x0 + lineWidth / 2, lm + i, color);
            }
        }
    ...
    if (y0 == y1) {
        ...
        if (lineWidth & 1) {
            ...
            for (i = 0; i < len; i++) {
                PX_SurfaceDrawPixel(
                    psurface, lm + i, y0 - lineWidth / 2, color);
                PX_SurfaceDrawPixel(
                    psurface, lm + i, y0 + lineWidth / 2, color);
            }
        }
        ...
        if (PX_TRUNC(p1y) - PX_TRUNC(p2y) != 0) {
            while (y <= PX_TRUNC(p1y)) {
                ...
                if (PX_TRUNC(rxL - xlen) < PX_TRUNC(rxL)) {
                    ...
                    clr = color;
                    clr._argb.a = clr._argb.a
                        = (px_uchar) (clr._argb.a * ftemp);
                    ;
...
px_void PX_GeoDrawBorder(px_surface *psurface,
                         px_int left,
                         px_int top,
                         px_int right,
                         px_int bottom,
...
px_void PX_GeoDrawRect(px_surface *psurface,
                       px_int left,
                       px_int top,
                       px_int right,
                       px_int bottom,
...
static px_void PX_GeoDrawSolidCircle_Ex1(
    px_surface *psurface, px_int x, px_int y, px_int Radius, px_color color) {
...
static px_void PX_GeoDrawSolidCircle_Ex2(px_surface *psurface,
                                         px_float x,
                                         px_float y,
                                         px_float Radius,
                                         px_color color) {
        ...
        for (j = left; j <= right; j++) {
            ...
            if (d < Radius) {
                if ((px_float) Radius - d < 1.414f) {
                    clr = color;
                    clr._argb.a
                        = (px_uchar) (clr._argb.a * ((px_float) Radius - d)
                                      / 1.414f);
...
px_void PX_GeoDrawSolidCircle(
    ...
    if (Sr < Sc) {
        PX_GeoDrawSolidCircle_Ex2(
            psurface, (px_float) x, (px_float) y, (px_float) Radius, color);
    } else {
        PX_GeoDrawSolidCircle_Ex1(psurface, x, y, Radius, color);
    }
}

px_void PX_GeoDrawSolidCircleFast(
    px_surface *psurface, px_int x, px_int y, px_int Radius, px_color color) {
    px_int _x, _y;
    px_int left, top, right, bottom;
    if (color._argb.a == 0 || Radius <= 0) {
...
px_void PX_GeoDrawPenCircle(px_surface *psurface,
                            px_float x,
                            px_float y,
                            px_float Radius,
                            px_color color) {
...
px_void PX_GeoDrawSpray(px_surface *psurface,
                        px_float x,
                        px_float y,
                        px_float Radius,
                        px_color color) {
...
px_void PX_GeoDrawBall(px_surface *psurface,
                       px_float x,
                       px_float y,
                       px_float Radius,
                       px_color color) {
...
px_void PX_GeoDrawPath(px_surface *psurface,
                       px_point path[],
                       px_int pathCount,
                       px_float linewidth,
                       px_color color) {
    ...
    if (pathCount == 1) {
        PX_GeoDrawSolidCircle_Ex2(
            psurface, path[0].x, path[0].y, linewidth / 2, color);
    } else {
        for (i = 0; i < pathCount - 1; i++) {
            px_point path_unit_vector
                = PX_PointNormalization(PX_PointSub(path[i + 1], path[i]));
            px_int length
                = (px_int) PX_PointMod(PX_PointSub(path[i + 1], path[i]));
            px_float step = 0;
            do {
                px_point drawPoint
                    = PX_PointAdd(path[i], PX_PointMul(path_unit_vector, step));
                PX_GeoDrawSolidCircle_Ex2(
                    psurface, drawPoint.x, drawPoint.y, linewidth / 2, color);
...
px_void PX_GeoDrawPath2D(px_surface *psurface,
                         px_point2D path[],
                         px_int pathCount,
                         px_float linewidth,
                         px_color color) {
    ...
    if (pathCount == 1) {
        ...
        for (i = 0; i < pathCount - 1; i++) {
            px_point2D path_unit_vector
                = PX_Point2DNormalization(PX_Point2DSub(path[i + 1], path[i]));
            px_int length
                = (px_int) PX_Point2DMod(PX_Point2DSub(path[i + 1], path[i]));
            px_float step = 0;
            do {
                px_point2D drawPoint = PX_Point2DAdd(
                    path[i], PX_Point2DMul(path_unit_vector, step));
                PX_GeoDrawSolidCircle_Ex2(
                    psurface, drawPoint.x, drawPoint.y, linewidth / 2, color);
                step += (linewidth / 4) < 0.5f ? 0.5f : (linewidth / 4);
            } while (step < length);
...
px_void PX_GeoDrawPenLine(px_surface *psurface,
                          px_float x0,
                          px_float y0,
                          px_float x1,
                          px_float y1,
...
px_void PX_GeoDrawDashLine(px_surface *psurface,
                           px_int x0,
                           px_int y0,
                           px_int x1,
                           px_int y1,
                           ...
                           px_float dash_width,
                           px_color color) {
    px_point2D v = PX_Point2DSub(PX_POINT2D((px_float) x1, (px_float) y1),
                                 PX_POINT2D((px_float) x0, (px_float) y0));
...
px_void PX_GeoDrawPenDots(px_surface *psurface,
                          px_point _samples[],
                          px_int samplescount,
                          px_float radius,
                          px_color color) {
    ...
    for (i = 0; i < samplescount; i++) {
        PX_GeoDrawPenCircle(
            psurface, _samples[i].x, _samples[i].y, radius, color);
    }
}

px_void PX_GeoDrawCircle(px_surface *psurface,
                         px_int x,
                         px_int y,
                         px_int Radius,
                         px_int lineWidth,
    ...
    if (x >= 0 - Radius && y >= 0 - Radius && x < psurface->width + Radius
        ...
        if (i < (x - psurface->width) * (x - psurface->width)
                    + (y - psurface->height) * (y - psurface->height)) {
            i = (x - psurface->width) * (x - psurface->width)
                + (y - psurface->height) * (y - psurface->height);
        }
        ...
        if (i < (Radius - lineWidth / 2) * (Radius - lineWidth / 2)
            && !PX_isPointInRect(PX_POINT((px_float) x, (px_float) y, 0),
                                 PX_RECT(0,
                                         0,
                                         (px_float) psurface->width,
...
static px_void PX_GeoDrawRingPoint(px_surface *psurface,
                                   px_int x,
                                   px_int y,
                                   px_color color,
                                   px_int cx,
...
px_void PX_GeoDrawRing(px_surface *psurface,
                       px_int x,
                       px_int y,
                       px_float Radius,
                       px_float lineWidth,
...
px_void PX_GeoDrawSector(px_surface *psurface,
                         px_int x,
                         px_int y,
                         px_float Radius_outside,
                         px_float Radius_inside,
                         px_color color,
                         px_int start_angle,
                         px_int end_angle) {
    ...
    if (x >= 0 - Radius_outside && y >= 0 - Radius_outside
        && x < psurface->width + Radius_outside
    ...
    do {
        ...
        PX_GeoDrawRingPoint(
            psurface, x + rx, y, clr, x, y, start_mathRegion, end_mathRegion);
        PX_GeoDrawRingPoint(
            psurface, x - rx, y, clr, x, y, start_mathRegion, end_mathRegion);
        PX_GeoDrawRingPoint(
            psurface, x, rx + y, clr, x, y, start_mathRegion, end_mathRegion);
    ...
    for (rx = (px_int) Radius_inside + 1; rx <= ry - 1; rx++) {
        PX_GeoDrawRingPoint(
            psurface, x + rx, y, color, x, y, start_mathRegion, end_mathRegion);
        PX_GeoDrawRingPoint(
            psurface, x - rx, y, color, x, y, start_mathRegion, end_mathRegion);
        PX_GeoDrawRingPoint(
            psurface, x, rx + y, color, x, y, start_mathRegion, end_mathRegion);
        PX_GeoDrawRingPoint(psurface,
                            x,
                            -rx + y,
                            color,
                            x,
...
px_void PX_GeoDrawSolidRoundRect(px_surface *psurface,
                                 px_int left,
                                 px_int top,
                                 px_int right,
                                 px_int bottom,
                                 px_float roundRaduis,
                                 px_color color) {
    ...
    for (y = 0; y < (px_int) roundRaduis; y++) {
        ...
        x = (px_int) roundRaduis
            - (px_int) (PX_sqrt(roundRaduis * roundRaduis
                                - (roundRaduis - y - 1)
                                      * (roundRaduis - y - 1)))
            - 1;
        if (x < 0)
        ...
        for (; x < (px_int) roundRaduis; x++) {
            ...
            if (dis < roundRaduis + 0.5f) {
                ...
                if (dis > roundRaduis)
                    drawColor._argb.a
                        = (px_uchar) (color._argb.a
                                      * (dis - roundRaduis + 0.5f));

                PX_SurfaceDrawPixel(psurface, left + x, top + y, drawColor);

                if (2 * x < Width - 1) {
                    PX_SurfaceDrawPixel(
                        psurface, left + Width - x - 1, top + y, drawColor);
                    if (2 * y < Height - 1) {
                    ...
                    PX_SurfaceDrawPixel(
                        psurface, left + x, top + Height - 1 - y, drawColor);
                }
            }
        }
    }
    ...
    if (Width > 2 * (px_int) roundRaduis) {
        ...
        if (drawWidth > 0 && drawHeight > 0) {
            PX_GeoDrawRect(psurface,
                           left + (px_int) roundRaduis,
                           top,
                           left + (px_int) roundRaduis + drawWidth - 1,
                           top + drawHeight - 1,
                           color);
            PX_GeoDrawRect(psurface,
                           left + (px_int) roundRaduis,
    ...
    if (Height > 2 * (px_int) roundRaduis)
        PX_GeoDrawRect(psurface,
                       left,
                       top + (px_int) roundRaduis,
                       right,
                       bottom - (px_int) roundRaduis,
                       color);
}

px_void PX_GeoDrawRoundRect(px_surface *psurface,
                            px_int left,
                            px_int top,
                            px_int right,
                            px_int bottom,
    ...
    for (y = 0; y < (px_int) roundRaduis; y++) {
        ...
        for (; x < (px_int) roundRaduis; x++) {
            ...
            dis = PX_sqrt((x - r_x) * (x - r_x) + (y - r_y) * (y - r_y));
            if (dis < roundRaduis + 0.5f
                && dis > roundRaduis - linewidth - 0.5f) {
                drawColor = color;
    ...
    if (Height > 2 * (px_int) roundRaduis) {
        PX_GeoDrawRect(psurface,
                       left,
                       top + (px_int) roundRaduis,
                       left + drawlinewidth_int - 1,
                       bottom - (px_int) roundRaduis,
                       color);
        PX_GeoDrawRect(psurface,
                       right - drawlinewidth_int + 1,
                       top + (px_int) roundRaduis,
                       right,
                       bottom - (px_int) roundRaduis,
                       color);
...
px_void PX_GeoDrawBezierCurvePoint(px_surface *rendersurface,
                                   px_point pt[],
                                   px_int pt_count,
                                   px_float t,
                                   px_float radius,
    ...
    for (i = 0; i < pt_count - 1; i++) {
        px_point vector_unit
            = PX_PointNormalization(PX_PointSub(pt[i + 1], pt[i]));
...
px_void PX_GeoDrawBresenhamLine(px_surface *psurface,
                                px_int x0,
                                px_int y0,
                                px_int x1,
                                px_int y1,
...
px_void PX_GeoDrawTriangle(px_surface *psurface,
                           px_point2D p0,
                           px_point2D p1,
                           px_point2D p2,
                           px_color color) {
...
px_void PX_GeoDrawArrow(px_surface *psurface,
                        px_point2D p0,
                        px_point2D p1,
                        px_float size,
                        px_color color) {
    ...
    PX_GeoDrawTriangle(psurface,
                       p0_5,
                       p1,
                       PX_Point2DAdd(p0_5, PX_Point2DMul(vn, arrowsize)),
                       color);
    PX_GeoDrawTriangle(psurface,
                       p0_5,
                       p1,
...
static px_point2D PX_GeoDeBoorCoxRecursion(px_int j,
                                           px_int i,
                                           px_int k,
                                           px_float t,
                                           px_point2D controlPoints[],
                                           px_float knots[]) {
    px_point2D p1, p2;
...
px_void PX_GeoBSpline3(px_point2D controlPoints[4],
                       px_point2D _samples[],
                       px_int samplescount) {
    ...
    for (i = 0; i < samplescount; i++) {
        t = i * step;
        point = PX_GeoDeBoorCoxRecursion(
            2, 2 + (px_int) t, 3, t, controlPoints, knots);
        _samples[i] = (point);
    }
}

px_void PX_GeoDrawPenSamplesLine(px_surface *psurface,
                                 px_point _samples[],
                                 px_int samplescount,
                                 px_float radius,
                                 px_color color,
                                 px_float v,
                                 px_float filter_factor) {
    px_int i, dindex = 1;
    ...
    if (samplescount == 1) {
        ...
        PX_GeoDrawPenCircle(
            psurface, _samples[0].x, _samples[0].y, radius, clr);
        return;
    }
    ...
    while (PX_TRUE) {
        ...
        while (step_distance > 0) {
            ...
            dir = PX_POINT(_samples[dindex].x - guider.x,
                           _samples[dindex].y - guider.y,
        ...
        if (d >= 1) {
            ...
            if (dindex >= samplescount - 1) {
                ...
                // finish the last
                dir = PX_POINT(
                    guider.x - pen.x, guider.y - pen.y, guider.z - pen.z);
                d = (PX_sqrt(dir.x * dir.x + dir.y * dir.y));
                dir = PX_PointNormalization(dir);

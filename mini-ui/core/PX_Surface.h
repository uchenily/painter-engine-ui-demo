...
#define PX_SURFACECOLOR(main_pSurface, X, Y)                                   \
    ((main_pSurface)->surfaceBuffer[(X) + (main_pSurface)->width * (Y)])
#define PX_SurfaceGetPixel(main_pSurface, X, Y)                                \
    PX_SURFACECOLOR(main_pSurface, X, Y)
...
px_bool PX_SurfaceCreate(px_memorypool *mp,
                         px_int width,
                         px_int height,
                         _OUT px_surface *surface);
px_void PX_SurfaceUnlimit(px_surface *psurface);
px_void PX_SurfaceClear(px_surface *ps,
                        px_int left,
                        px_int top,
                        px_int right,
                        px_int bottom,

...
px_byte *PX_MemoryData(px_memory *memory) { return memory->buffer; }
...
px_void PX_MemoryRemove(px_memory *memory, px_int start, px_int end) {
    ...
    PX_memcpy(memory->buffer + start,
              memory->buffer + end + 1,
              memory->usedsize - end - 1);
...
px_bool PX_MemoryCopy(px_memory *memory,
                      const px_void *buffer,
                      px_int startoffset,
                      px_int size) {
    px_byte *old;
    px_int length, shl;
...
px_bool PX_CircularBufferInitialize(px_memorypool *mp,
                                    PX_CircularBuffer *pcbuffer,
                                    px_int size) {
...
px_void
PX_CircularBufferAdd(PX_CircularBuffer *pcbuffer, px_int pos, px_double v) {
...
px_void
PX_CircularBufferSet(PX_CircularBuffer *pcbuffer, px_int pos, px_double v) {
    pos = pos % pcbuffer->size;
    if (pos < 0) {
        pos += pcbuffer->size;
    }
...
px_void PX_FifoBufferFree(px_fifobuffer *pfifo) { PX_MemoryFree(pfifo); }
...
px_int PX_StackPop(px_stack *pstack, px_void *data, px_int size) {
    if (pstack->usedsize > sizeof(px_int)) {
        px_int rsize
            = *(px_int *) (pstack->buffer + pstack->usedsize - sizeof(px_int));
        ...
        PX_memcpy(data,
                  pstack->buffer + pstack->usedsize - sizeof(px_int) - rsize,
                  rsize);
        PX_MemoryRemove(pstack,
                        pstack->usedsize - sizeof(px_int) - rsize,
                        pstack->usedsize - 1);
        return rsize;
    }
    return 0;
...
px_void PX_StackFree(px_stack *pstack) { PX_MemoryFree(pstack); }
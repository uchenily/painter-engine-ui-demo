...
px_void PX_BitmapRender(px_surface *psurface,
                        void *BitmapBuffer,
                        px_int BufferSize,
                        px_int x,
                        px_int y) {
    ...
    ByteColor
        = ((px_char *) BitmapBuffer + PX_BitmapGetDataOffset(BitmapBuffer));

    // 32bit Color for A8R8G8B8(X8R8G8B8)
    if (bitCount == 32) {
        rgb32 = (PX_BITMAP_RGB32 *) ByteColor;
        if (Height * Width
            > BufferSize - (px_int) PX_BitmapGetDataOffset(BitmapBuffer)) {
            return;
        }
        if (Height > 0) {
            for (i = 0; i < Height; i++) {
    ...
    if (bitCount == 24) {
        ...
        if (Height > 0) {
            for (i = 0; i < Height; i++) {
                rgb24 = (PX_BITMAP_RGB24 *) (ByteColor
                                             + RowBytes * (Height - 1 - i));
                for (j = 0; j < Width; j++) {
            ...
            for (i = 0; i < -Height; i++) {
                ...
                for (j = 0; j < Width; j++) {
                    ...
                    PXcolor32._argb.g = rgb24[j].G;
                    PXcolor32._argb.b = rgb24[j].B;
                    if (j + x > 0 && i + y > 0 && j + x < psurface->width
                        && i + y < psurface->height)
                        PX_SurfaceSetPixel(psurface, j + x, i + y, PXcolor32);
                }
            }
...
px_bool
PX_BitmapBuild(px_surface *psurface, px_char *BitmapBuffer, px_int *size) {
    ...
    _outsize = psurface->height * psurface->width * 3
               + sizeof(PX_BITMAPFILEHEADER) + sizeof(PX_BITMAPINFOHEADER)
               + rowAppendSize * psurface->height;
    ...
    *size = _outsize;
    if (BitmapBuffer) {
        ...
        pdata = (PX_BITMAP_RGB24 *) (BitmapBuffer + sizeof(PX_BITMAPFILEHEADER)
                                     + sizeof(PX_BITMAPINFOHEADER));
        for (y = psurface->height - 1; y >= 0; y--) {
            for (x = 0; x < psurface->width; x++) {
                (*pdata).R
                    = (px_char) (PX_SURFACECOLOR(psurface, x, y)._argb.r
                                 * PX_SURFACECOLOR(psurface, x, y)._argb.a
                                 / 256);
                (*pdata).G
                    = (px_char) (PX_SURFACECOLOR(psurface, x, y)._argb.g
                                 * PX_SURFACECOLOR(psurface, x, y)._argb.a
                                 / 256);
                (*pdata).B
                    = (px_char) (PX_SURFACECOLOR(psurface, x, y)._argb.b
                                 * PX_SURFACECOLOR(psurface, x, y)._argb.a
                                 / 256);
                pdata++;
            }
